...
try {
  ...
  check(config.currentVersion === '448', 'config currentVersion V448');
  check(config.leagueId === 'fantapetillomantramanager' && config.slug === 'fantapetillomantramanager', 'identity clone corretta');
  check(config.sandbox?.enabled === true && config.sandbox?.firebase === 'disabled', 'sandbox/Firebase disabled in config');
  check(config.guardrails?.readyForFirebaseConfig === true, 'readyForFirebaseConfig tracciato');
  check(config.features?.admin === false && config.features?.teamArea === false, 'admin/teamArea disabilitati');

  ['index.html', 'competition.html', 'player.html'].forEach((page) => {
    const text = read(page);
    check(text.includes('fanta-petillo-sandbox-v448.js?v=448'), `${page} carica guard V448`);
    const versions = [...new Set((text.match(/\?v=\d+/g) || []).map((m) => m.slice(3)))];
    check(versions.length === 1 && versions[0] === '448', `${page} cache-buster V448`);
    check(!text.includes('/zonaorientale/') && !text.includes('silviobarra.com/zonaorientale'), `${page} senza URL pubblici ZonaOrientale`);
  });

  const guard = read('assets/js/core/fanta-petillo-sandbox-v448.js');
  check(guard.includes('MutationObserver') && guard.includes('noindex,nofollow'), 'guard V448 osserva DOM e aggiunge noindex');
  check(guard.includes('href="#admin"') && guard.includes('href="#teamarea"'), 'guard V448 copre entrypoint admin/teamarea');

  const firebase = read('assets/firebase.js');
  check(firebase.includes('Firebase disabled sandbox adapter'), 'Firebase stub attivo');
  check(firebase.includes('firebaseDisabled: true'), 'marker firebaseDisabled true');
  check(!firebase.includes('initializeApp') && !firebase.includes('fantapetillomantramanager.firebaseapp.com') && !firebase.includes('zonaorientale-d07af'), 'nessuna config Firebase reale nel clone V448');
