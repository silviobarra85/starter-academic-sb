...
if [[ "$versions" == "456" ]]; then pass "cache-buster clone V456"; else fail "cache-buster clone non allineati: $versions"; fi
if command -v node >/dev/null 2>&1; then
  ...
  if node "$SITE_ROOT/tools/audit-admin-card-visibility-v456.mjs" --quiet; then pass "audit selettore card Admin V456"; else fail "audit selettore card Admin V456"; fi
else
  fail "node non disponibile per audit runtime clone"
fi
if [[ "$failures" -gt 0 ]]; then exit 1; fi
printf 'Controlli clone fix click selettore Admin V456 passati.\n'
