...
PYJSON
  ...
  warn "python3 non disponibile per validazione JSON"
fi

if grep -q 'zonaorientale-d07af' "$SITE_ROOT/assets/firebase.js"; then
  fail "firebase ZonaOrientale presente nel clone"
else
  pass "Firebase ZonaOrientale assente dal clone"
fi

if grep -q '"adminOnboardingEnabled": true' "$SITE_ROOT/assets/league-config.json"; then
  pass "config onboarding Admin V451"
elif grep -q '"firebaseConnected": "admin-bootstrap-v450"' "$SITE_ROOT/assets/league-config.json"; then
  pass "config Admin bootstrap V450"
else
  fail "config Firebase clone non esplicita stato sicuro"
fi
