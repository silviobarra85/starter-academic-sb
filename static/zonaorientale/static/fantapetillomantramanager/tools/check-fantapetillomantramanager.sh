...
if [[ "$versions" == "468" ]]; then pass "cache-buster clone V468"; else fail "cache-buster clone non allineati: $versions"; fi
if command -v node >/dev/null 2>&1; then
  ...
  if node "$SITE_ROOT/tools/audit-standard-admin-cleanup-v468.mjs" --quiet; then pass "audit setup standard Admin V468"; else fail "audit setup standard Admin V468"; fi
...
printf 'Controlli clone setup standard Admin V468 passati.\n'
