...
check('runtime deploy almeno V438 in app.js', Number(app.match(/DEPLOY_EXPECTED_VERSION_V181\s*=\s*"(\d+)"/)?.[1] || 0) >= 438);
check('footer e cache-buster almeno V438 in index', index.includes('?v=467') && index.includes('V467 setup standard FantaPetillo'));
