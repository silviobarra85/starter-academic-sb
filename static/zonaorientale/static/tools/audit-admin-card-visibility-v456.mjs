...
check('CSS V456 collegato in index', index.includes('admin-card-visibility-v456.css?v=467'));
check('JS V456 collegato in index', index.includes('admin-card-visibility-v456.js?v=467'));
check('script V455 non piu collegato in index', !index.includes('admin-card-visibility-v455.js?v=467') && !index.includes('admin-card-visibility-v455.js?v=467'));
...
check('runtime almeno V456', Number(app.match(/DEPLOY_EXPECTED_VERSION_V181\s*=\s*"(\d+)"/)?.[1] || 0) >= 456);
check('footer V456 o successivo', Number((index.match(/data-league-footer-v445>[^<]*V(\d+)/) || [])[1] || 0) >= 456);
