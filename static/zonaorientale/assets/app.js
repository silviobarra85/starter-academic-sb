...
const DEPLOY_EXPECTED_VERSION_V181 = "205";
...
/* V197 - Generatore comunicati automatici.
   Pannello admin mobile-first che prepara bozze di comunicati usando solo i
   dati gia' caricati in state.raw da JSON/snapshot/Firebase. Non scrive su
   Firebase: puo' copiare il testo o inserirlo nel form Comunicati esistente,
   dove l'admin decide se salvarlo. */
...
function buildResultsAnnouncementDraftV197({ seasonId, competitionId, teamId, tone }) {
  ...
  const intro = tone === "celebrativo"
    ? `La ZonaOrientale si accende ancora: ecco il riepilogo aggiornato dei risultati della ${seasonLabel}.`
    : tone === "ironico"
...
function buildWinnerAnnouncementDraftV197({ seasonId, competitionId, teamId, tone }) {
  ...
  const body = `${opener}\n\n${podium ? `Podio ufficiale:\n${podium}\n\n` : ""}Complimenti alla societa' vincitrice e appuntamento ai prossimi impegni della ZonaOrientale.`;
...
function getCommunicationGeneratorOptionsHtmlV197(items, selectedId, getLabelFn) {
  return items.map((item) => `<option value="${escapeHtml(item.id)}" ${item.id === selectedId ? "selected" : ""}>${escapeHtml(getLabelFn(item))}</option>`).join("");
}
  ...
  return `
    <section class="panel communication-generator-v197" aria-labelledby="communicationGeneratorTitleV197">
      ...
      <div class="communication-generator-preview-v197">
        <label><span>Titolo bozza</span><input id="communicationDraftTitleV197" class="input" type="text" value="${escapeHtml(currentDraft.title || "")}" /></label>
        <label><span>Testo bozza</span><textarea id="communicationDraftBodyV197" class="input textarea" rows="9">${escapeHtml(currentDraft.body || "")}</textarea></label>
      </div>
      ...
      <p id="communicationGeneratorStatusV197" class="form-status">Bozza generata il ${escapeHtml(getCommunicationGeneratorNewsDateV197())}.</p>
    </section>`;
}
...
const renderAdminAreaBeforeV197 = renderAdminArea;
renderAdminArea = function renderAdminAreaV197() {
  const result = renderAdminAreaBeforeV197?.();
  renderCommunicationGeneratorPanelV197();
  return result;
};

const renderAdminLightGateBeforeV197 = typeof renderAdminLightGateV178 === "function" ? renderAdminLightGateV178 : null;
if (renderAdminLightGateBeforeV197) {
  renderAdminLightGateV178 = function renderAdminLightGateV197() {
...
const renderAdminHelpPanelBeforeV197 = renderAdminHelpPanelV185;
renderAdminHelpPanelV185 = function renderAdminHelpPanelV197() {
...
/* V205 - Final startup remains centralized here. */
