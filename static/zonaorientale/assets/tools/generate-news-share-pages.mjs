#!/usr/bin/env node
/* V259 - Genera pagine statiche per anteprime WhatsApp dei comunicati.
   ...
   Il comando legge assets/snapshots/seasons/*.json, crea/aggiorna:
     ...
     - index.html con meta Open Graph generici della home, mai dell'ultima news
   ...
   Nota V259: la home deve restare generica. Le anteprime dei comunicati devono
   uscire solo dai percorsi comunicato/share, non da index.html. */
...
const HOME_TITLE = "ZonaOrientale Salerno";
const HOME_DESCRIPTION = "Dashboard ufficiale della Lega Fantacalcio ZonaOrientale Salerno: rose, listoni, fantamercato, competizioni, albo d'oro, statistiche, archivio e comunicati.";
...
function replaceOrInsertMetaContent(html, attributeName, attributeValue, content) {
  const escapedValue = attributeValue.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  const pattern = new RegExp(`<meta\\s+${attributeName}="${escapedValue}"\\s+content="[^"]*"\\s*/?>`, "i");
  const replacement = `<meta ${attributeName}="${attributeValue}" content="${escapeHtml(content)}" />`;
  if (pattern.test(html)) return html.replace(pattern, replacement);
  return html.replace(/<title>[^<]*<\/title>\s*/i, (match) => `${match}\n  ${replacement}`);
}
...
function replaceOrInsertCanonical(html, href) {
  ...
  if (/<link\s+rel="canonical"\s+href="[^"]*"\s*\/?>/i.test(html)) {
    return html.replace(/<link\s+rel="canonical"\s+href="[^"]*"\s*\/?>/i, replacement);
  }
  return html.replace(/<meta name="description" content="[^"]*" \/>\s*/i, (match) => `${match}\n  ${replacement}`);
}

async function updateIndexHomeOpenGraph() {
  const indexPath = path.join(SITE_DIR, "index.html");
  let html = await fs.readFile(indexPath, "utf8");
  const homeUrl = normalizeBaseUrl(BASE_URL);
  html = replaceOrInsertMetaContent(html, "name", "description", HOME_DESCRIPTION);
  html = replaceOrInsertCanonical(html, homeUrl);
  html = replaceOrInsertMetaContent(html, "property", "og:type", "website");
  html = replaceOrInsertMetaContent(html, "property", "og:site_name", "ZonaOrientale Salerno");
  html = replaceOrInsertMetaContent(html, "property", "og:title", HOME_TITLE);
  html = replaceOrInsertMetaContent(html, "property", "og:description", HOME_DESCRIPTION);
  html = replaceOrInsertMetaContent(html, "property", "og:image", IMAGE_URL);
  html = replaceOrInsertMetaContent(html, "property", "og:url", homeUrl);
  html = replaceOrInsertMetaContent(html, "name", "twitter:card", "summary_large_image");
  html = replaceOrInsertMetaContent(html, "name", "twitter:title", HOME_TITLE);
  html = replaceOrInsertMetaContent(html, "name", "twitter:description", HOME_DESCRIPTION);
  html = replaceOrInsertMetaContent(html, "name", "twitter:image", IMAGE_URL);
...
async function main() {
  ...
  if (newsItems[0]) {
    ...
    await updateIndexHomeOpenGraph();
  }
  ...
  console.log("Home index.html aggiornata con anteprima generica V259.");
}
