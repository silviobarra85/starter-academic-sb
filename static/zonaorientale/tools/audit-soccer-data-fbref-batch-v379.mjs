...
const mapping = readJson(`assets/soccer-data/${manifest.currentMapping || 'fbref-player-map.v379.json'}`);
...
Number(String(manifest.meta?.version || '').replace(/^V/, '')) >= 379 ? pass(`manifest versione ${manifest.meta?.version} >= V379`) : fail('manifest precedente a V379');
...
confirmed.length >= 350 ? pass(`totale mapping confermati >= V379: ${confirmed.length}`) : fail(`mapping confermati totali: ${confirmed.length}, attesi almeno 350`);
const batch07 = mapping.players.filter((p) => p.matchStatus === 'confirmed' && p.mappedInRelease === 'V379' && p.reviewBatch === 'batch-07');
batch07.length === 50 ? pass('batch-07 contiene 50 mapping confermati') : fail(`mapping confermati batch-07: ${batch07.length}, attesi 50`);
...
const badStatus = mapping.players.filter((p) => p.mappedInRelease === 'V379' && p.reviewBatch !== 'batch-07');
badStatus.length === 0 ? pass('nessun mapping V379 fuori batch-07') : fail(`${badStatus.length} mapping V379 fuori batch-07`);
['window.ZonaOrientaleSoccerDataV371','window.ZonaOrientaleSoccerDataMappingAssistantV372','window.ZonaOrientaleSoccerDataFbrefBatchV373','window.ZonaOrientaleSoccerDataFbrefBatchV374','window.ZonaOrientaleSoccerDataFbrefBatchV375','window.ZonaOrientaleSoccerDataFbrefBatchV376','window.ZonaOrientaleSoccerDataFbrefBatchV377','window.ZonaOrientaleSoccerDataFbrefBatchV378','window.ZonaOrientaleSoccerDataFbrefBatchV379','fbref-player-map.v379.json','firebaseWrites: false','liveScraping: false'].forEach((token) => app.includes(token) ? pass(`app token presente: ${token}`) : fail(`app token mancante: ${token}`));
