...
const mapping = readJson(`assets/soccer-data/${manifest.currentMapping || 'fbref-player-map.v381.json'}`);
...
Number(String(manifest.meta?.version || '').replace(/^V/, '')) >= 381 ? pass(`manifest versione ${manifest.meta?.version} >= V381`) : fail('manifest precedente a V381');
...
confirmed.length >= 450 ? pass(`totale mapping confermati >= V381: ${confirmed.length}`) : fail(`mapping confermati totali: ${confirmed.length}, attesi almeno 450`);
const batch09 = mapping.players.filter((p) => p.matchStatus === 'confirmed' && p.mappedInRelease === 'V381' && p.reviewBatch === 'batch-09');
batch09.length === 50 ? pass('batch-09 contiene 50 mapping confermati') : fail(`mapping confermati batch-09: ${batch09.length}, attesi 50`);
