...
check('league-config versione V443+', Number(config.currentVersion) >= 443);
...
check('index carica loader V443', index.includes('league-config-v443.js?v=444'));
check('standalone caricano loader V443', competition.includes('league-config-v443.js?v=444') && player.includes('league-config-v443.js?v=444'));
check('cache-buster runtime V443+', index.includes(`assets/app.js?v=${config.currentVersion}`) && !index.includes('?v=442'));
check('footer V443+ aggiornato', index.includes(`V${config.currentVersion}`) && competition.includes(`V${config.currentVersion}`) && player.includes(`V${config.currentVersion}`));
check('DEPLOY_EXPECTED_VERSION V443+', app.includes(`DEPLOY_EXPECTED_VERSION_V181 = "${config.currentVersion}"`));
