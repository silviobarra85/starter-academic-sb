...
const mapping = readJson('assets/soccer-data/fbref-player-map.v382.json');
const app = fs.readFileSync(path.join(root, 'assets/app.js'), 'utf8');
const html = fs.readFileSync(path.join(root, 'index.html'), 'utf8');

manifest.currentMapping === 'fbref-player-map.v382.json' ? pass('manifest punta al mapping V382') : fail('manifest non punta a fbref-player-map.v382.json');
manifest.meta?.version === 'V382' ? pass('manifest versione V382') : fail('manifest non e V382');
...
confirmed.length === 500 ? pass('totale mapping confermati V382: 500') : fail(`mapping confermati totali: ${confirmed.length}, attesi 500`);
const batch10 = mapping.players.filter((p) => p.matchStatus === 'confirmed' && p.mappedInRelease === 'V382' && p.reviewBatch === 'batch-10');
batch10.length === 50 ? pass('batch-10 contiene 50 mapping confermati') : fail(`mapping confermati batch-10: ${batch10.length}, attesi 50`);
...
const badStatus = mapping.players.filter((p) => p.mappedInRelease === 'V382' && p.reviewBatch !== 'batch-10');
badStatus.length === 0 ? pass('nessun mapping V382 fuori batch-10') : fail(`${badStatus.length} mapping V382 fuori batch-10`);
['window.ZonaOrientaleSoccerDataV371','window.ZonaOrientaleSoccerDataMappingAssistantV372','window.ZonaOrientaleSoccerDataFbrefBatchV373','window.ZonaOrientaleSoccerDataFbrefBatchV374','window.ZonaOrientaleSoccerDataFbrefBatchV375','window.ZonaOrientaleSoccerDataFbrefBatchV376','window.ZonaOrientaleSoccerDataFbrefBatchV377','window.ZonaOrientaleSoccerDataFbrefBatchV378','window.ZonaOrientaleSoccerDataFbrefBatchV379','window.ZonaOrientaleSoccerDataFbrefBatchV380','window.ZonaOrientaleSoccerDataFbrefBatchV381','window.ZonaOrientaleSoccerDataFbrefBatchV382','fbref-player-map.v382.json','firebaseWrites: false','liveScraping: false'].forEach((token) => app.includes(token) ? pass(`app token presente: ${token}`) : fail(`app token mancante: ${token}`));
html.includes('Soccer Data') && html.includes('V382') ? pass('HTML Soccer Data aggiornato a V382') : fail('HTML Soccer Data non aggiornato a V382');
