...
const mapping = readJson(`assets/soccer-data/${manifest.currentMapping || 'fbref-player-map.v378.json'}`);
...
Number(String(manifest.meta?.version || '').replace(/^V/, '')) >= 378 ? pass(`manifest versione ${manifest.meta?.version} >= V378`) : fail('manifest precedente a V378');
...
confirmed.length >= 300 ? pass(`totale mapping confermati >= V378: ${confirmed.length}`) : fail(`mapping confermati totali: ${confirmed.length}, attesi almeno 300`);
const batch06 = mapping.players.filter((p) => p.matchStatus === 'confirmed' && p.mappedInRelease === 'V378' && p.reviewBatch === 'batch-06');
batch06.length === 50 ? pass('batch-06 contiene 50 mapping confermati') : fail(`mapping confermati batch-06: ${batch06.length}, attesi 50`);
