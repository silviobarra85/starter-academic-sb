...
const mapping = readJson(`assets/soccer-data/${manifest.currentMapping || 'fbref-player-map.v380.json'}`);
...
Number(String(manifest.meta?.version || '').replace(/^V/, '')) >= 380 ? pass(`manifest versione ${manifest.meta?.version} >= V380`) : fail('manifest precedente a V380');
...
confirmed.length >= 400 ? pass(`totale mapping confermati >= V380: ${confirmed.length}`) : fail(`mapping confermati totali: ${confirmed.length}, attesi almeno 400`);
const batch08 = mapping.players.filter((p) => p.matchStatus === 'confirmed' && p.mappedInRelease === 'V380' && p.reviewBatch === 'batch-08');
batch08.length === 50 ? pass('batch-08 contiene 50 mapping confermati') : fail(`mapping confermati batch-08: ${batch08.length}, attesi 50`);
