...
if (docsRoot) {
  ...
  if (fileExists(protectedDoc)) pass("FUNZIONALITA'.md presente e non richiesto per V369");
  ...
  for (const relativeDoc of requiredDocs) {
    ...
    if (fileExists(target)) pass(`doc V369 presente: ${relativeDoc}`);
    else fail(`doc V369 mancante: ${relativeDoc}`);
  }
} else {
