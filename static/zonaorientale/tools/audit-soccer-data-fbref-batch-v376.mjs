...
const mapping = readJson('assets/soccer-data/fbref-player-map.v376.json');
...
Number(String(manifest.meta?.version || '').replace(/^V/, '')) >= 376 ? pass(`manifest versione ${manifest.meta?.version} >= V376`) : fail('manifest precedente a V376');
...
confirmed.length === 200 ? pass('totale mapping confermati V376: 200') : fail(`mapping confermati totali: ${confirmed.length}, attesi 200`);
const batch04 = mapping.players.filter((p) => p.matchStatus === 'confirmed' && p.mappedInRelease === 'V376' && p.reviewBatch === 'batch-04');
batch04.length === 50 ? pass('batch-04 contiene 50 mapping confermati') : fail(`mapping confermati batch-04: ${batch04.length}, attesi 50`);
...
const badStatus = mapping.players.filter((p) => p.mappedInRelease === 'V376' && p.reviewBatch !== 'batch-04');
badStatus.length === 0 ? pass('nessun mapping V376 fuori batch-04') : fail(`${badStatus.length} mapping V376 fuori batch-04`);
['window.ZonaOrientaleSoccerDataV371','window.ZonaOrientaleSoccerDataMappingAssistantV372','window.ZonaOrientaleSoccerDataFbrefBatchV373','window.ZonaOrientaleSoccerDataFbrefBatchV374','window.ZonaOrientaleSoccerDataFbrefBatchV375','window.ZonaOrientaleSoccerDataFbrefBatchV376','fbref-player-map.v376.json','firebaseWrites: false','liveScraping: false'].forEach((token) => app.includes(token) ? pass(`app token presente: ${token}`) : fail(`app token mancante: ${token}`));
