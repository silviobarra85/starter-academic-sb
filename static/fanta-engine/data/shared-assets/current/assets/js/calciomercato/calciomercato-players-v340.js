/*
 * V340 - Helper associazione articoli Calciomercato -> giocatori Listone.
 * Refactor protetto: mantiene la normalizzazione punteggiatura V337 e aggiunge
 * una disambiguazione maiuscole/minuscole per i nomi composti da una sola parola.
 * Esempio: Giovane giocatore viene riconosciuto solo se nel testo compare come
 * nome proprio capitalizzato, non come aggettivo minuscolo.
 */

const PLAYER_STOP_WORDS_V340 = new Set([
...
const PLAYER_PUNCTUATION_RE_V340 = /[^a-z0-9]+/g;
const PLAYER_HTML_TAG_RE_V340 = /<[^>]*>/g;
...
function identityNormalize(value) {
  return String(value || "")
    ...
    .toLowerCase()
    .replace(/[’']/g, " ")
    .replace(PLAYER_PUNCTUATION_RE_V340, " ")
    .replace(/\s+/g, " ")
    .trim();
}
...
function createPlayerMatchNormalizer(normalizeValue) {
  ...
  return (value) => {
    ...
    return String(normalized || "")
      ...
      .replace(/[\u0300-\u036f]/g, "")
      .toLowerCase()
      .replace(PLAYER_HTML_TAG_RE_V340, " ")
      .replace(/[’'`´]/g, " ")
      .replace(PLAYER_PUNCTUATION_RE_V340, " ")
      .replace(/\s+/g, " ")
      .trim();
...
function buildPlayerAliasIndex(listone, options = {}) {
  ...
  players.forEach((player) => {
    ...
    if (surname && surname.length >= 5 && !PLAYER_STOP_WORDS_V340.has(surname)) {
  ...
  return players
    .map((player) => {
      ...
      const surname = tokens[tokens.length - 1] || "";
      if (surname && surname.length >= 5 && surnameCounts.get(surname) === 1 && !PLAYER_STOP_WORDS_V340.has(surname)) {
...
function runPlayerMatchingSmokeTestV340(normalizeValue) {
  const listone = {
    meta: { id: "smoke-v337" },
    players: [
      { playerName: "Kalulu" },
...
export function createCalciomercatoPlayerHelpersV340(dependencies = {}) {
  ...
  return {
    ...
    getListoneId,
    runSmokeTest: () => runPlayerMatchingSmokeTestV340(normalizeValue)
  };
}
