...
for (const [file, leagueId, name] of configs) {
  ...
  check(cfg.currentVersion === 493, `${file} currentVersion V493`);
...
for (const file of htmlFiles) {
  ...
  check(read(file).includes('?v=493'), `${file} cache-buster V493`);
}
for (const file of ['zonaorientale/index.html','zonaorientale/competition.html','zonaorientale/player.html','zonaorientale/static/index.html','zonaorientale/static/competition.html','zonaorientale/static/player.html']) {
  checkIncludes(file, 'ZonaOrientale Salerno · V493 · Ultimo aggiornamento 24/06/2026', `footer ZonaOrientale V493 in ${file}`);
}
for (const file of ['fantapetillomantramanager/index.html','fantapetillomantramanager/competition.html','fantapetillomantramanager/player.html']) {
  checkIncludes(file, 'FantaMantraManager · V493 · Ultimo aggiornamento 24/06/2026', `footer FantaMantraManager V493 in ${file}`);
}
...
const engineFiles = [
  ...
  'fanta-engine/tools/audit-runtime-regression-v493.mjs',
  'fanta-engine/tools/audit-multileague-contamination-v493.mjs',
  'fanta-engine/tools/audit-merge-readiness-v493.mjs',
];
...
const docsFiles = [
  ...
  'docs/fantapetillomantramanager/HANDOFF_V493_MERGE_READINESS.md',
  ...
  'docs/zonaorientale/HANDOFF_V493_MERGE_READINESS.md',
];
for (const file of docsFiles) check(existsRepo(file), `documentazione presente: ${file}`);
checkRepoIncludes('docs/fantapetillomantramanager/MERGE_BRANCH_CHECKLIST_V493.md', 'V480-V493', 'checklist FMM copre V480-V493');
checkRepoIncludes('docs/zonaorientale/MERGE_BRANCH_CHECKLIST_V493.md', 'V480-V493', 'checklist ZonaOrientale copre V480-V493');
check(!existsRepo("docs/zonaorientale/FUNZIONALITA'.md") || !readRepo("docs/zonaorientale/FUNZIONALITA'.md").includes('V493 - Merge readiness'), "FUNZIONALITA'.md non aggiornato da V493");

if (fail > 0) {
  console.error(`\nAudit merge readiness V493 fallito: ${ok} OK, ${fail} FAIL`);
  for (const item of failures) console.error(` - ${item}`);
  process.exit(1);
}
console.log(`\nAudit merge readiness V493 completato: ${ok} OK, ${fail} FAIL`);
