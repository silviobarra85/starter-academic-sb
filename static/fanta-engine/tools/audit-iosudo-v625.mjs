...
function fail(message) { console.error('[ioSudo V625][FAIL]', message); process.exit(1); }
...
if (!index.includes('../fanta-engine/css/iosudo-app-v625.css?v=625')) fail('ioSudo CSS V625 not linked');
if (!index.includes('../fanta-engine/js/apps/iosudo-app-v625.js?v=625')) fail('ioSudo JS V625 not linked');
if (!index.includes('data-iosudo-version="625"')) fail('ioSudo data version not bumped');
...
const app = read('static/fanta-engine/js/apps/iosudo-app-v625.js');
for (const token of [
...
const css = read('static/fanta-engine/css/iosudo-app-v625.css');
...
if (!sw.includes('iosudo-shell-v625')) fail('Service worker cache not V625');
if (!sw.includes('iosudo-app-v625.css?v=625')) fail('Service worker CSS not V625');
if (!sw.includes('iosudo-app-v625.js?v=625')) fail('Service worker JS not V625');
...
for (const league of ['zonaorientale','fantapetillomantramanager']) {
  ...
  if (cfg.currentVersion !== '625') fail('Wrong currentVersion in ' + league);
  if (cfg.features?.ioSudoGlobalPlayerCardClick !== 'V625') fail('Missing ioSudoGlobalPlayerCardClick V625 in ' + league);
  if (cfg.features?.ioSudoPlayerDetailFromGlobalView !== 'V625') fail('Missing ioSudoPlayerDetailFromGlobalView V625 in ' + league);
}
console.log('[ioSudo V625] Audit OK: card GIOCATORI cliccabili e dettaglio giocatore attivo.');
