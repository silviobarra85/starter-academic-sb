...
const css = 'static/fanta-engine/css/sudatori-section-v597.css';
const js = 'static/fanta-engine/js/sections/sudatori-section-v597.js';
...
ok('CSS V597 presente', exists(css), css);
ok('JS V597 presente', exists(js), js);
...
if (exists(js)) {
  ...
  ok('Versione runtime V597', /const VERSION = 'V597'/.test(source), 'VERSION V597');
...
if (exists(css)) {
  ...
  ok('CSS infortunati presente', /sudatori-injuries-v597/.test(source), 'sudatori-injuries-v597');
  ok('CSS stato fisico presente', /sudatori-physical-v597/.test(source), 'sudatori-physical-v597');
}
if (exists(manifestPath)) {
  const manifest = JSON.parse(read(manifestPath));
  ok('Manifest V597', manifest.version === 'V597', manifest.version);
  ok('Infortuni dichiarati', Number(manifest.injuries || 0) >= 6, String(manifest.injuries));
}
if (exists(dataPath)) {
  const data = JSON.parse(read(dataPath));
  ok('Meta V597', data.meta?.version === 'V597', data.meta?.version);
...
for (const league of ['zonaorientale', 'fantapetillomantramanager']) {
  ...
  if (exists(index)) {
    ...
    ok(`${league}: CSS Sudatori V597 referenziato`, html.includes('sudatori-section-v597.css?v=597'), index);
    ok(`${league}: JS Sudatori V597 referenziato`, html.includes('sudatori-section-v597.js?v=597'), index);
  }
  const cfg = `static/${league}/assets/league-config.json`;
  if (exists(cfg)) {
    const config = JSON.parse(read(cfg));
    ok(`${league}: currentVersion V597`, String(config.currentVersion) === '597', config.currentVersion);
  }
}
...
if (failed.length) {
  console.error(`\nAudit V597 fallito: ${failed.length} errori.`);
  process.exit(1);
}
console.log('\nAudit V597 superato.');
