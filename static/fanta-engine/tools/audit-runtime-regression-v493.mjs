...
for (const [cfgPath, leagueId, name] of configs) {
  ...
  check(cfg.currentVersion === 493, `${cfgPath} currentVersion V493`);
...
for (const file of htmlPages) {
  ...
  check(read(file).includes('?v=493'), `${file} cache-buster V493 presente`);
}
...
for (const file of [`${zonaRoot}/index.html`, `${zonaRoot}/competition.html`, `${zonaRoot}/player.html`, `${zonaNestedRoot}/index.html`, `${zonaNestedRoot}/competition.html`, `${zonaNestedRoot}/player.html`]) {
  checkIncludes(file, 'ZonaOrientale Salerno · V493 · Ultimo aggiornamento 24/06/2026', `footer V493 ZonaOrientale in ${file}`);
...
for (const file of [`${fmmRoot}/index.html`, `${fmmRoot}/competition.html`, `${fmmRoot}/player.html`]) {
  checkIncludes(file, 'FantaMantraManager · V493 · Ultimo aggiornamento 24/06/2026', `footer V493 FantaMantraManager in ${file}`);
...
for (const file of loaderFiles) {
  const text = read(file);
  check(text.includes("currentVersion: '493'"), `${file} currentVersion runtime V493`);
  check(text.includes('league-config.json?v=493'), `${file} fetch config V493`);
  check(text.includes("version: 'V493'"), `${file} marker runtime V493`);
...
if (fail > 0) {
  console.error(`\nAudit regressione runtime V493 fallito: ${ok} OK, ${fail} FAIL`);
  for (const item of failures) console.error(` - ${item}`);
  process.exit(1);
}
console.log(`\nAudit regressione runtime V493 completato: ${ok} OK, ${fail} FAIL`);
