...
for(const [p,id,name] of [['zonaorientale/assets/league-config.json','zonaorientale','ZonaOrientale Salerno'],['fantapetillomantramanager/assets/league-config.json','fantapetillomantramanager','FantaMantraManager']]){const c=readJson(p);check(c.leagueId===id,`${p} leagueId corretto`);check(c.name===name,`${p} nome corretto`);check(c.currentVersion===509,`${p} versione V509`);check(c.features?.dashboardRendererMigrationVersion==='V509',`${p} renderer migration V509`)}
for(const p of ['zonaorientale/index.html','zonaorientale/competition.html','zonaorientale/player.html']){noTerms(p,['FantaMantraManager','FantaPetillo','fantapetillomantramanager'],`${p} senza contaminazione FMM`);check(read(p).includes('ZonaOrientale Salerno · V509 · Ultimo aggiornamento 24/06/2026'),`${p} footer Zona V509`)}
...
for (const [file, token, label] of [
  ...
  ['fanta-engine/js/ui/dashboard-renderer-helpers-v505.js','DASHBOARD_RENDERER_HELPERS_VERSION_V505','Dashboard renderer helpers V505'],
...
check(read('fanta-engine/js/ui/dashboard-renderer-helpers-v509.js').includes('renderAdminCollapsiblePanelV509'), 'V509 contiene renderer Admin comune');
check(read('fanta-engine/js/ui/dashboard-renderer-helpers-v509.js').includes('renderPresidentDashboardMetricV509'), 'V509 contiene metriche Presidente comuni');
check(read('../docs/OVERLAY_ROADMAP.md').includes('V510 - Report centralizzazione'), 'roadmap overlay aggiornata');
if(fail>0){console.error(`\nAudit anti-contaminazione V509 fallito: ${ok} OK, ${fail} FAIL`);failures.forEach(f=>console.error(` - ${f}`));process.exit(1)}
console.log(`\nAudit anti-contaminazione V509 completato: ${ok} OK, ${fail} FAIL`);
