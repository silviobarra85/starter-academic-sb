...
check(zonaConfig.currentVersion === 488, 'currentVersion ZonaOrientale V488');
check(zonaNestedConfig.currentVersion === 488, 'currentVersion ZonaOrientale annidato V488');
check(fmmConfig.currentVersion === 488, 'currentVersion FantaMantraManager V488');
...
for (const file of zonaFooterPages) {
  check(read(file).includes('ZonaOrientale Salerno · V488 · Ultimo aggiornamento 24/06/2026'), `footer V488 ZonaOrientale in ${file}`);
}
...
for (const file of fmmFooterPages) {
  check(read(file).includes('FantaMantraManager · V488 · Ultimo aggiornamento 24/06/2026'), `footer V488 FantaMantraManager in ${file}`);
}
...
if (fail > 0) {
  console.error(`\nAudit anti-contaminazione V488 fallito: ${ok} OK, ${fail} FAIL`);
  for (const item of failures) console.error(` - ${item}`);
  process.exit(1);
}
console.log(`\nAudit anti-contaminazione V488 completato: ${ok} OK, ${fail} FAIL`);
