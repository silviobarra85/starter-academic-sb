...
function fail(message) {
  console.error('[ioSudo V611][FAIL]', message);
  process.exit(1);
}
...
if (!index.includes('../fanta-engine/css/iosudo-app-v611.css?v=611')) fail('ioSudo CSS not linked');
if (!index.includes('../fanta-engine/js/apps/iosudo-app-v611.js?v=611')) fail('ioSudo JS not linked');
...
const app = read('static/fanta-engine/js/apps/iosudo-app-v611.js');
...
const css = read('static/fanta-engine/css/iosudo-app-v611.css');
...
for (const league of ['zonaorientale', 'fantapetillomantramanager']) {
  ...
  if (!html.includes('data-iosudo-link-v611="true"')) fail('Missing ioSudo link in ' + league + ' index');
  ...
  if (cfg.currentVersion !== '611') fail('Wrong currentVersion in ' + league);
  if (!cfg.features || cfg.features.ioSudoPwa !== true) fail('Missing ioSudo feature flag in ' + league);
}

console.log('[ioSudo V611] Audit OK: player detail links, role colors, hidden search state, sticky team menu, shared data source and league links verified.');
