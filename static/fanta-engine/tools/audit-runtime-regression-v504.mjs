...
for (const file of ['zonaorientale/assets/app.js','fantapetillomantramanager/assets/app.js']) { const text=read(file); check(text.includes('dashboard-cards-engine-v504.js?v=504'),`${file} usa dashboard engine V504`); check(text.includes('safe-enforce'),`${file} default safe-enforce`); }
...
for (const file of ['zonaorientale/assets/js/sections/matchday-draw-tool-v473.js','fantapetillomantramanager/assets/js/sections/matchday-draw-tool-v473.js']) { const text=read(file); check(text.includes('matchday-draw-engine-v501.js?v=504'), `${file} usa tool engine V504 cache`); check(text.includes('initMatchdayDrawToolFallbackV473'), `${file} fallback locale V473`); }
check(exists('fanta-engine/tools/playwright-smoke-v503.mjs'), 'Playwright smoke V503 preservato');
if(fail>0){ console.error(`\nAudit regressione runtime V504 fallito: ${ok} OK, ${fail} FAIL`); failures.forEach(f=>console.error(` - ${f}`)); process.exit(1); }
console.log(`\nAudit regressione runtime V504 completato: ${ok} OK, ${fail} FAIL`);
