...
check(zonaConfig.currentVersion === 485, 'currentVersion ZonaOrientale V485');
check(zonaNestedConfig.currentVersion === 485, 'currentVersion ZonaOrientale annidato V485');
check(fmmConfig.currentVersion === 485, 'currentVersion FantaMantraManager V485');
...
for (const file of zonaFooterPages) {
  check(read(file).includes('ZonaOrientale Salerno · V485 · Ultimo aggiornamento 24/06/2026'), `footer V485 ZonaOrientale in ${file}`);
}
...
for (const file of fmmFooterPages) {
  check(read(file).includes('FantaMantraManager · V485 · Ultimo aggiornamento 24/06/2026'), `footer V485 FantaMantraManager in ${file}`);
}
...
if (fail > 0) {
  console.error(`\nAudit anti-contaminazione V485 fallito: ${ok} OK, ${fail} FAIL`);
  for (const item of failures) console.error(` - ${item}`);
  process.exit(1);
}
console.log(`\nAudit anti-contaminazione V485 completato: ${ok} OK, ${fail} FAIL`);
