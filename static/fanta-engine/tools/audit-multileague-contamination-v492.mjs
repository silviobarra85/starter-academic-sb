...
check(zonaConfig.currentVersion === 492, 'currentVersion ZonaOrientale V492');
check(zonaNestedConfig.currentVersion === 492, 'currentVersion ZonaOrientale annidato V492');
check(fmmConfig.currentVersion === 492, 'currentVersion FantaMantraManager V492');
...
for (const file of zonaFooterPages) {
  check(read(file).includes('ZonaOrientale Salerno · V492 · Ultimo aggiornamento 24/06/2026'), `footer V492 ZonaOrientale in ${file}`);
}
...
for (const file of fmmFooterPages) {
  check(read(file).includes('FantaMantraManager · V492 · Ultimo aggiornamento 24/06/2026'), `footer V492 FantaMantraManager in ${file}`);
}
...
if (fail > 0) {
  console.error(`\nAudit anti-contaminazione V492 fallito: ${ok} OK, ${fail} FAIL`);
  for (const item of failures) console.error(` - ${item}`);
  process.exit(1);
}
console.log(`\nAudit anti-contaminazione V492 completato: ${ok} OK, ${fail} FAIL`);
