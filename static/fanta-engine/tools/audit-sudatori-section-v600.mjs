...
const css = 'static/fanta-engine/css/sudatori-section-v600.css';
const js = 'static/fanta-engine/js/sections/sudatori-section-v600.js';
...
ok('CSS V600 presente', exists(css), css);
ok('JS V600 presente', exists(js), js);
...
if (exists(js)) {
  ...
  ok('Versione runtime V600', /const VERSION = 'V600'/.test(source), 'VERSION V600');
...
if (exists(css)) {
  ...
  ok('CSS infortunati presente', /sudatori-injuries-v600/.test(source), 'sudatori-injuries-v600');
  ok('CSS stato fisico presente', /sudatori-physical-v600/.test(source), 'sudatori-physical-v600');
  ok('CSS badge segnalazione campo rosso', /is-field-alert/.test(source), 'is-field-alert');
}
if (exists(manifestPath)) {
  const manifest = JSON.parse(read(manifestPath));
  ok('Manifest V600', manifest.version === 'V600', manifest.version);
  ok('Infortuni dichiarati', Number(manifest.injuries || 0) >= 6, String(manifest.injuries));
}
if (exists(dataPath)) {
  const data = JSON.parse(read(dataPath));
  ok('Meta V600', data.meta?.version === 'V600', data.meta?.version);
  ...
  const lazioPlayersAll = data.playersByTeam?.lazio || [];
  const genoaPlayers = data.playersByTeam?.genoa || [];
  const cagliariPlayers = data.playersByTeam?.cagliari || [];
  const udinesePlayersAll = data.playersByTeam?.udinese || [];
  ok('V600: Gila in rosa Milan', milanPlayers.some((p) => /^gila$/i.test(p.playerName || '')), milanPlayers.map((p) => p.playerName).filter((n) => /gila/i.test(n)).join(', '));
  ok('V600: Gila non resta in Lazio', !lazioPlayersAll.some((p) => /^gila$/i.test(p.playerName || '')), lazioPlayersAll.map((p) => p.playerName).filter((n) => /gila/i.test(n)).join(', '));
  ok('V600: Elias Havel in rosa Genoa', genoaPlayers.some((p) => /havel/i.test(p.playerName || '')), genoaPlayers.map((p) => p.playerName).filter((n) => /havel/i.test(n)).join(', '));
  ok('V600: Luvumbo non in rosa Cagliari', !cagliariPlayers.some((p) => /luvumbo/i.test(p.playerName || '')), cagliariPlayers.map((p) => p.playerName).filter((n) => /luvumbo/i.test(n)).join(', '));
  ok('V600: Omar Traore in rosa Udinese', udinesePlayersAll.some((p) => /omar\s+traore/i.test(p.playerName || '')), udinesePlayersAll.map((p) => p.playerName).filter((n) => /traore/i.test(n)).join(', '));
  ok('V600: Fiorentina-Real Madrid amichevole presente', (data.friendliesByTeam?.fiorentina || []).some((f) => /real madrid/i.test(f.event || '')), (data.friendliesByTeam?.fiorentina || []).map((f) => f.event).join(', '));
  ok('V600: Burnley-Torino amichevole presente', (data.friendliesByTeam?.torino || []).some((f) => /burnley/i.test(f.event || '')), (data.friendliesByTeam?.torino || []).map((f) => f.event).join(', '));
  ok('V600: TMW serale caricato', Number(data.meta?.tmwEveningNews || 0) >= 10 && (data.tmwEveningNews || []).length >= 10, String(data.meta?.tmwEveningNews));
  ok('V600: trattativa Andrej Vasovic Bologna', (data.teamTransferTalksByTeam?.bologna || []).some((x) => /vasovic/i.test(x.target || '')), (data.teamTransferTalksByTeam?.bologna || []).map((x) => x.target).join(', '));
  ok('V600: Davitashvili alternativa Atalanta', (data.teamTransferTalksByTeam?.atalanta || []).some((x) => /davitashvili/i.test(x.target || '')), (data.teamTransferTalksByTeam?.atalanta || []).map((x) => x.target).join(', '));
}
...
for (const league of ['zonaorientale', 'fantapetillomantramanager']) {
  ...
  if (exists(index)) {
    ...
    ok(`${league}: CSS Sudatori V600 referenziato`, html.includes('sudatori-section-v600.css?v=600'), index);
    ok(`${league}: JS Sudatori V600 referenziato`, html.includes('sudatori-section-v600.js?v=600'), index);
  }
  const cfg = `static/${league}/assets/league-config.json`;
  if (exists(cfg)) {
    const config = JSON.parse(read(cfg));
    ok(`${league}: currentVersion V600`, String(config.currentVersion) === '600', config.currentVersion);
  }
}
...
if (failed.length) {
  console.error(`\nAudit V600 fallito: ${failed.length} errori.`);
  process.exit(1);
}
console.log('\nAudit V600 superato.');
