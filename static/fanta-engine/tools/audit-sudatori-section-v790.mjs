...
check('V790 non cancella i dati ioSudo V782', manifest?.dataFile === 'sudatori-runtime.json' && manifest?.archiveDataFile === 'sudatori-data.json');
console.log(`Audit dati Sudatori V790: ${checks.filter((item) => item.ok).length}/${checks.length} controlli superati.`);
checks.forEach((item) => console.log(`${item.ok ? 'OK' : 'ERRORE'} - ${item.label}`));
if (failures.length) { console.error(`Audit dati Sudatori V790 fallito: ${failures.length} problemi.`); process.exit(1); }
