...
const fail = (msg) => { console.error('AUDIT V627 FAIL:', msg); process.exit(1); };
...
if (manifest.version !== 'V627' || data.meta.version !== 'V627') fail('versione manifest/data non V627');
...
if (manifest.articleSourceAuditToVerify < 60) fail('righe DA VERIFICARE non tracciate');
...
if (!manifest.genericLinksSuppressedV627 || manifest.genericLinksSuppressedV627 < 1) fail('nessun link generico soppresso: controllo V627 non applicato');

const js = read('static/fanta-engine/js/sections/sudatori-section-v627.js');
for (const token of ['itemArticleUrlV627','loadLeagueRosters','applyLiveRosters','fantasyRosterLabel','assets/rose/manifest.json','liveRosterFor','leagueBaseUrl']) if (!js.includes(token)) fail('token runtime rose mancante: ' + token);
for (const league of ['zonaorientale','fantapetillomantramanager']) {
  const html = read(`static/${league}/index.html`);
  if (!html.includes('sudatori-section-v627.css?v=627')) fail('CSS Sudatori V627 non linkato in ' + league);
  if (!html.includes('sudatori-section-v627.js?v=627')) fail('JS Sudatori V627 non linkato in ' + league);
  const cfg = json(`static/${league}/assets/league-config.json`);
  if (cfg.currentVersion !== '627') fail('currentVersion non 627 in ' + league);
  if (cfg.features?.sudatoriLiveRosters !== 'V627') fail('feature sudatoriLiveRosters non V627 in ' + league);
}
console.log('AUDIT V627 OK', JSON.stringify({teams:data.teams.length, players:manifest.players, friendlies:manifest.friendlies, talks:manifest.teamTransferTalks, officialIn, officialOut, sources:manifest.sources, injuries: manifest.injuries}));
