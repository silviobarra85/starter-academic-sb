...
check(zonaConfig.currentVersion === 485, 'currentVersion ZonaOrientale V486');
check(zonaNestedConfig.currentVersion === 485, 'currentVersion ZonaOrientale annidato V486');
check(fmmConfig.currentVersion === 485, 'currentVersion FantaMantraManager V486');
...
for (const file of zonaFooterPages) {
  check(read(file).includes('ZonaOrientale Salerno · V486 · Ultimo aggiornamento 24/06/2026'), `footer V486 ZonaOrientale in ${file}`);
}
...
for (const file of fmmFooterPages) {
  check(read(file).includes('FantaMantraManager · V486 · Ultimo aggiornamento 24/06/2026'), `footer V486 FantaMantraManager in ${file}`);
}
...
if (fail > 0) {
  console.error(`\nAudit anti-contaminazione V486 fallito: ${ok} OK, ${fail} FAIL`);
  for (const item of failures) console.error(` - ${item}`);
  process.exit(1);
}
console.log(`\nAudit anti-contaminazione V486 completato: ${ok} OK, ${fail} FAIL`);
