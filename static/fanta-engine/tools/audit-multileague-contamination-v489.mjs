...
check(zonaConfig.currentVersion === 489, 'currentVersion ZonaOrientale V489');
check(zonaNestedConfig.currentVersion === 489, 'currentVersion ZonaOrientale annidato V489');
check(fmmConfig.currentVersion === 489, 'currentVersion FantaMantraManager V489');
...
if (fail > 0) {
  console.error(`\nAudit anti-contaminazione V489 fallito: ${ok} OK, ${fail} FAIL`);
  for (const item of failures) console.error(` - ${item}`);
  process.exit(1);
}
console.log(`\nAudit anti-contaminazione V489 completato: ${ok} OK, ${fail} FAIL`);
