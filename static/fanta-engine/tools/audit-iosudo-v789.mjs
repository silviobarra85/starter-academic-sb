...
check('FantaMantraManager non riceve patch ZonaOrientale V789', !mantraApp.includes('ZonaOrientaleCanonicalFooterV789') && !mantraApp.includes('ZonaOrientaleTradeAnnouncementV789'));

const footerLabel = 'Fantacalcio - V789 - Aggiornato al 12/08/2026';
check('Footer canonico V789 presente in app', zonaApp.includes('const ZONAORIENTALE_RELEASE_V789') && zonaApp.includes(footerLabel));
check('Observer canonico unico V789 presente', zonaApp.includes('footerCanonicalObserverV789'));
check('Tutti i writer footer legacy delegano al canonico V789', (zonaApp.match(/ZonaOrientaleCanonicalFooterV789\?\.active/g) || []).length >= 20);
...
check('Writer V694 non puo scrivere V694 nel runtime V789', v694Block.includes('ZonaOrientaleCanonicalFooterV789?.active') && v694Block.indexOf('ZonaOrientaleCanonicalFooterV789?.active') < v694Block.indexOf('footerVersionV694'));
check('Home parte gia con footer V789', zonaIndex.includes(footerLabel));
check('Competition parte gia con footer V789', zonaCompetition.includes(footerLabel));
check('Player parte gia con footer V789', zonaPlayer.includes(footerLabel));
check('Entrypoint app cache-buster V789', zonaIndex.includes('./assets/app.js?v=789') && release?.entrypoint === 'assets/app.js?v=789');
check('Core league config usa un solo cache-buster V789 in home', zonaIndex.includes('league-config-v443.js?v=789') && !zonaIndex.includes('league-config-v443.js?v=761'));
check('Core league config usa V789 in competition/player', zonaCompetition.includes('league-config-v443.js?v=789') && zonaPlayer.includes('league-config-v443.js?v=789'));
...
check('Config runtime V789', zonaConfig?.currentVersion === '789' && zonaConfig?.branding?.footerLastUpdated === '12/08/2026' && zonaConfig?.branding?.footerVersion === 789);
check('Fallback config e fetch V789', zonaConfigJs.includes("currentVersion: '789'") && zonaConfigJs.includes("league-config.json?v=789"));
...
check('V789 ascolta evento auth realmente emesso dal sito', zonaApp.includes('"fanta:auth-state-v760"'));
...
check('Pulsante Scambio/Vendita apre il pannello collassato', zonaApp.includes('openZonaTradeV789') && zonaApp.includes('panel.classList.toggle("is-collapsed-v432", !expanded)'));
...
check('Release metadata V789', release?.version === '789' && release?.entrypoint === 'assets/app.js?v=789');

console.log(`Audit V789 footer/scambio/rose: ${checks.filter((item) => item.ok).length}/${checks.length} controlli superati.`);
for (const item of checks) console.log(`${item.ok ? 'OK' : 'ERRORE'} - ${item.label}${item.details && !item.ok ? ` (${item.details})` : ''}`);
if (failures.length) {
  console.error(`Audit V789 fallito: ${failures.length} problemi.`);
  process.exit(1);
}
