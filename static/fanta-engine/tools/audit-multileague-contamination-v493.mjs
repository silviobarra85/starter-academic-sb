...
check(zonaConfig.currentVersion === 493, 'currentVersion ZonaOrientale V493');
check(zonaNestedConfig.currentVersion === 493, 'currentVersion ZonaOrientale annidato V493');
check(fmmConfig.currentVersion === 493, 'currentVersion FantaMantraManager V493');
...
for (const file of zonaFooterPages) {
  check(read(file).includes('ZonaOrientale Salerno · V493 · Ultimo aggiornamento 24/06/2026'), `footer V493 ZonaOrientale in ${file}`);
}
...
for (const file of fmmFooterPages) {
  check(read(file).includes('FantaMantraManager · V493 · Ultimo aggiornamento 24/06/2026'), `footer V493 FantaMantraManager in ${file}`);
}
...
if (fail > 0) {
  console.error(`\nAudit anti-contaminazione V493 fallito: ${ok} OK, ${fail} FAIL`);
  for (const item of failures) console.error(` - ${item}`);
  process.exit(1);
}
console.log(`\nAudit anti-contaminazione V493 completato: ${ok} OK, ${fail} FAIL`);
