...
const required = [
  ...
  'static/iosudo/sw.js',
  'static/fanta-engine/js/apps/iosudo-app-v628.js',
  'static/fanta-engine/css/iosudo-app-v628.css',
  'static/zonaorientale/assets/league-config.json',
  'static/fantapetillomantramanager/assets/league-config.json'
];
function fail(message) { console.error('[ioSudo V628][FAIL]', message); process.exit(1); }
...
if (!index.includes('../fanta-engine/css/iosudo-app-v628.css?v=628')) fail('ioSudo CSS V628 not linked');
if (!index.includes('../fanta-engine/js/apps/iosudo-app-v628.js?v=628')) fail('ioSudo JS V628 not linked');
if (!index.includes('data-iosudo-version="628"')) fail('ioSudo data version not bumped');
...
const app = read('static/fanta-engine/js/apps/iosudo-app-v628.js');
...
const css = read('static/fanta-engine/css/iosudo-app-v628.css');
...
if (!sw.includes('iosudo-shell-v628')) fail('Service worker cache not V628');
if (!sw.includes('iosudo-app-v628.css?v=628')) fail('Service worker CSS not V628');
if (!sw.includes('iosudo-app-v628.js?v=628')) fail('Service worker JS not V628');
...
for (const league of ['zonaorientale','fantapetillomantramanager']) {
  ...
  if (cfg.currentVersion !== '628') fail('Wrong currentVersion in ' + league);
  if (cfg.features?.ioSudoGlobalPlayerCardClick !== 'V628') fail('Missing ioSudoGlobalPlayerCardClick V628 in ' + league);
  if (cfg.features?.ioSudoPlayerDetailFromGlobalView !== 'V628') fail('Missing ioSudoPlayerDetailFromGlobalView V628 in ' + league);
}
console.log('[ioSudo V628] Audit OK: card GIOCATORI cliccabili, dettaglio giocatore e fonti articolo puntuali attive.');
