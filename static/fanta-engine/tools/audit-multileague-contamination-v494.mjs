...
check(zonaConfig.currentVersion === 494, 'currentVersion ZonaOrientale V494');
check(zonaNestedConfig.currentVersion === 494, 'currentVersion ZonaOrientale annidato V494');
check(fmmConfig.currentVersion === 494, 'currentVersion FantaMantraManager V494');
...
for (const file of zonaFooterPages) {
  check(read(file).includes('ZonaOrientale Salerno · V494 · Ultimo aggiornamento 24/06/2026'), `footer V494 ZonaOrientale in ${file}`);
}
...
for (const file of fmmFooterPages) {
  check(read(file).includes('FantaMantraManager · V494 · Ultimo aggiornamento 24/06/2026'), `footer V494 FantaMantraManager in ${file}`);
}
...
if (fail > 0) {
  console.error(`\nAudit anti-contaminazione V494 fallito: ${ok} OK, ${fail} FAIL`);
  for (const item of failures) console.error(` - ${item}`);
  process.exit(1);
}
console.log(`\nAudit anti-contaminazione V494 completato: ${ok} OK, ${fail} FAIL`);
