...
for (const [cfgPath, leagueId, name] of [['zonaorientale/assets/league-config.json','zonaorientale','ZonaOrientale Salerno'],['fantapetillomantramanager/assets/league-config.json','fantapetillomantramanager','FantaMantraManager']]) {
  ...
  check(cfg.currentVersion===498, `${cfgPath} currentVersion V498`);
...
for (const file of htmlFiles){ check(exists(file), `${file} presente`); checkRefs(file, `${file}`); check(read(file).includes('?v=498'), `${file} cache-buster V498`); }
for (const file of ['zonaorientale/index.html','zonaorientale/competition.html','zonaorientale/player.html']){ check(read(file).includes('ZonaOrientale Salerno · V498 · Ultimo aggiornamento 24/06/2026'), `footer ZonaOrientale V498 in ${file}`); noTerms(file,['FantaMantraManager','FantaPetillo','fantapetillomantramanager'],`${file} senza contaminazione FMM`); }
for (const file of ['fantapetillomantramanager/index.html','fantapetillomantramanager/competition.html','fantapetillomantramanager/player.html']){ check(read(file).includes('FantaMantraManager · V498 · Ultimo aggiornamento 24/06/2026'), `footer FMM V498 in ${file}`); noTerms(file,['ZonaOrientale','zonaorientale'],`${file} senza contaminazione ZonaOrientale`); }
noTerms('fantapetillomantramanager/news.html',['news-ujE2CqJMjzkYhhjzZZHD'],'news FMM senza ID storico ZonaOrientale');

for (const file of ['zonaorientale/assets/js/core/league-config-v443.js','fantapetillomantramanager/assets/js/core/league-config-v443.js']){ const text=read(file); check(text.includes("currentVersion: '498'"),`${file} runtime V498`); check(text.includes('league-config.json?v=498'),`${file} fetch config V498`);
  check(text.includes('uiComponentsEngineV496'),`${file} traccia UI components V498`); check(!text.includes('audit clone FantaPetillo'),`${file} senza clone footer`); }
...
if(fail>0){ console.error(`\nAudit regressione runtime V498 fallito: ${ok} OK, ${fail} FAIL`); failures.forEach(f=>console.error(` - ${f}`)); process.exit(1); }
console.log(`\nAudit regressione runtime V498 completato: ${ok} OK, ${fail} FAIL`);
