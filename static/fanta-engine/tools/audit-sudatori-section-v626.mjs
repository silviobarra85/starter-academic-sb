...
const fail = (msg) => { console.error('AUDIT V626 FAIL:', msg); process.exit(1); };
...
if (manifest.version !== 'V626' || data.meta.version !== 'V626') fail('versione manifest/data non V626');
...
if (manifest.extraSourcesAddedV626 !== 5) fail('extraSourcesAddedV626 atteso 5');
if (manifest.extraOfficialRowsAddedV626 !== 20) fail('extraOfficialRowsAddedV626 atteso 20');
if (manifest.extraTalkRowsAddedV626 !== 11) fail('extraTalkRowsAddedV626 atteso 11');
...
const js = read('static/fanta-engine/js/sections/sudatori-section-v626.js');
...
for (const league of ['zonaorientale','fantapetillomantramanager']) {
  ...
  if (!html.includes('sudatori-section-v626.css?v=626')) fail('CSS Sudatori V626 non linkato in ' + league);
  if (!html.includes('sudatori-section-v626.js?v=626')) fail('JS Sudatori V626 non linkato in ' + league);
  const cfg = json(`static/${league}/assets/league-config.json`);
  if (cfg.currentVersion !== '626') fail('currentVersion non 626 in ' + league);
  if (cfg.features?.sudatoriLiveRosters !== 'V626') fail('feature sudatoriLiveRosters non V626 in ' + league);
}
console.log('AUDIT V626 OK', JSON.stringify({teams:data.teams.length, players:manifest.players, friendlies:manifest.friendlies, talks:manifest.teamTransferTalks, officialIn, officialOut, sources:manifest.sources, injuries: manifest.injuries}));
