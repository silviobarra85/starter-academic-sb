...
for(const p of ['zonaorientale/index.html','zonaorientale/competition.html','zonaorientale/player.html']){noTerms(p,['FantaMantraManager','FantaPetillo','fantapetillomantramanager'],`${p} senza contaminazione FMM`);check(read(p).includes('ZonaOrientale Salerno · V502 · Ultimo aggiornamento 24/06/2026'),`${p} footer Zona V502`)}
...
check(read('fanta-engine/js/ui/components-v496.js').includes('UI_VERSION_V496'),'UI engine V502 presente');
...
check(read('fanta-engine/js/tools/matchday-draw-engine-v501.js').includes('MATCHDAY_DRAW_ENGINE_VERSION_V501'),'Tool engine V502 presente');
...
if(fail>0){console.error(`\nAudit anti-contaminazione V502 fallito: ${ok} OK, ${fail} FAIL`);failures.forEach(f=>console.error(` - ${f}`));process.exit(1)}
...
console.log(`\nAudit anti-contaminazione V502 completato: ${ok} OK, ${fail} FAIL`);
