...
for(const [p,id,name] of [['zonaorientale/assets/league-config.json','zonaorientale','ZonaOrientale Salerno'],['fantapetillomantramanager/assets/league-config.json','fantapetillomantramanager','FantaMantraManager']]){const c=readJson(p);check(c.leagueId===id,`${p} leagueId corretto`);check(c.name===name,`${p} nome corretto`);check(c.currentVersion===508,`${p} versione V508`);check(c.features?.formValidatorsVersion==='V506',`${p} form validators V506`);check(c.features?.playwrightHardeningVersion==='V508',`${p} Playwright hardening V508`)}
for(const p of ['zonaorientale/index.html','zonaorientale/competition.html','zonaorientale/player.html']){noTerms(p,['FantaMantraManager','FantaPetillo','fantapetillomantramanager'],`${p} senza contaminazione FMM`);check(read(p).includes('ZonaOrientale Salerno · V508 · Ultimo aggiornamento 24/06/2026'),`${p} footer Zona V508`)}
...
check(read('fanta-engine/tools/playwright-smoke-v508.mjs').includes('EXPECTED_VERSION = \'V508\''), 'Playwright V508 versione corretta');
check(read('fanta-engine/tools/playwright-smoke-v508.mjs').includes('mobile') && read('fanta-engine/tools/playwright-smoke-v508.mjs').includes('desktop'), 'Playwright V508 mobile desktop');
check(read('fanta-engine/tools/playwright-smoke-v508.mjs').includes('toMarkdownReport'), 'Playwright V508 report markdown');

check(read('fanta-engine/tools/create-league-v507.mjs').includes('validate-league-config-v507.mjs'), 'create-league V507 usa validator');
check(read('fanta-engine/tools/validate-league-config-v507.mjs').includes('reservedFirebaseProjects'), 'validator V507 blocca riuso Firebase noto');
check(read('fanta-engine/data/league-template-hardening-v507.json').includes('non aggiorna netlify.toml automaticamente'), 'manifest V507 non auto-edita Netlify');

if(fail>0){console.error(`\nAudit anti-contaminazione V508 fallito: ${ok} OK, ${fail} FAIL`);failures.forEach(f=>console.error(` - ${f}`));process.exit(1)}
console.log(`\nAudit anti-contaminazione V508 completato: ${ok} OK, ${fail} FAIL`);
