...
const css = 'static/fanta-engine/css/sudatori-section-v603.css';
const js = 'static/fanta-engine/js/sections/sudatori-section-v603.js';
...
ok('CSS V603 presente', exists(css), css);
ok('JS V603 presente', exists(js), js);
...
if (exists(js)) {
  ...
  ok('Versione runtime V603', /const VERSION = 'V603'/.test(source), 'VERSION V603');
  ...
  ok('V603: helper market signal presente', /function\s+renderMarketSignal\s*\(/.test(source), 'renderMarketSignal');
  ok('V603: marketNotesFor risolve note anche per nome esteso/team', /function\s+marketNotesFor\s*\(/.test(source) && /noteTeamMatchesPlayer/.test(source), 'marketNotesFor team-aware');
  ok('V603: badge unico NUOVO RUMOR CONFERMATO', /marketBadgeInfo/.test(source) && /label: 'NUOVO'/.test(source) && /label: 'RUMOR'/.test(source) && /label: 'CONFERMATO'/.test(source), 'marketBadgeInfo');
  ok('V603: colonna Mercato renderizza HTML badge', /<td>\$\{mercato\}<\/td>/.test(source), 'mercato HTML');
}
if (exists(css)) {
  const source = read(css);
  ok('CSS infortunati presente', /sudatori-injuries-v603/.test(source), 'sudatori-injuries-v603');
  ok('CSS badge segnalazione campo rosso', /is-field-alert/.test(source), 'is-field-alert');
  ok('CSS V603 badge mercato presente', /sudatori-market-flag-v603/.test(source), 'sudatori-market-flag-v603');
  ok('CSS V603 badge NUOVO presente', /is-new/.test(source), 'is-new');
  ok('CSS V603 badge RUMOR presente', /is-rumor/.test(source), 'is-rumor');
  ok('CSS V603 badge CONFERMATO presente', /is-confirmed/.test(source), 'is-confirmed');
}
if (exists(manifestPath)) {
  const manifest = JSON.parse(read(manifestPath));
  ok('Manifest V603', manifest.version === 'V603', manifest.version);
...
if (exists(dataPath)) {
  ...
  ok('Meta V603', data.meta?.version === 'V603', data.meta?.version);
  ...
  ok('V603: Atalanta-Atalanta Under 23 presente', (data.friendliesByTeam?.atalanta || []).some((f) => /under 23/i.test(f.event || '')), (data.friendliesByTeam?.atalanta || []).map((f) => f.event).join(', '));
  ok('V603: Real Madrid-Fiorentina presente', (data.friendliesByTeam?.fiorentina || []).some((f) => /real madrid/i.test(f.event || '')), (data.friendliesByTeam?.fiorentina || []).map((f) => f.event).join(', '));
  ok('V603: Burnley-Torino presente', (data.friendliesByTeam?.torino || []).some((f) => /burnley/i.test(f.event || '')), (data.friendliesByTeam?.torino || []).map((f) => f.event).join(', '));
  ok('V603: Lecce ha 4 amichevoli aggiornate', (data.friendliesByTeam?.lecce || []).length === 4, (data.friendliesByTeam?.lecce || []).map((f) => f.event).join(', '));
  ok('V603: Udinese senza amichevoli ma ritiro Lienz', (data.friendliesByTeam?.udinese || []).length === 0 && /lienz/i.test((data.teams || []).find((t) => t.id === 'udinese')?.retreatPlace || ''), JSON.stringify({ friendlies: data.friendliesByTeam?.udinese, team: (data.teams || []).find((t) => t.id === 'udinese') }));

  const rumors = data.transfermarktRumors || [];
  const rumorNames = rumors.map((x) => `${x.teamName}:${x.playerName}:${x.type}`).join(' | ');
  ok('V603: 25 rumors Transfermarkt caricati', rumors.length >= 25, String(rumors.length));
  ok('V603: Ederson rumor uscita Atalanta', rumors.some((x) => /atalanta/i.test(x.teamName || '') && /ederson/i.test(x.playerName || '') && /uscita/i.test(x.type || '')), rumorNames);
  ok('V603: Davinson Sanchez rumor Como', rumors.some((x) => /como/i.test(x.teamName || '') && /davinson/i.test(x.playerName || '')), rumorNames);
  ok('V603: Sergi Dominguez rumor Lazio', rumors.some((x) => /lazio/i.test(x.teamName || '') && /sergi/i.test(x.playerName || '')), rumorNames);
  const noteFor = (name) => data.marketNotesByPlayer?.[String(name || '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().replace(/[^a-z0-9]+/g, ' ').trim()] || [];
  ok('V603: Bremer ha rumor Transfermarkt anche se stato base puo essere In rosa', noteFor('Bremer').some((x) => /transfermarkt/i.test(`${x.source || ''} ${x.note || ''}`) && /uscita/i.test(x.status || x.type || '')), JSON.stringify(noteFor('Bremer')));
  ok('V603: Dodo ha rumor Transfermarkt nella lista note giocatore', noteFor('Dodo').some((x) => /transfermarkt/i.test(`${x.source || ''} ${x.note || ''}`) && /uscita/i.test(x.status || x.type || '')), JSON.stringify(noteFor('Dodo')));
  ...
  ok('V603: Gaetano Atalanta marcato nuovo acquisto', !!gaetano && gaetano.newAcquisition === true && /acquisto/i.test(gaetano.marketStatus || ''), JSON.stringify(gaetano && { name: gaetano.playerName, status: gaetano.marketStatus, flag: gaetano.flag, newAcquisition: gaetano.newAcquisition }));
  ok('V603: nota mercato Gaetano risolta anche con chiave breve', noteFor('Gaetano').some((x) => /atalanta/i.test(x.team || '') && /arrivo/i.test(x.note || '')), JSON.stringify(noteFor('Gaetano')));
  ok('V603: Giovane Napoli assegnato a Real Pisistrius', !!giovaneNapoli && /real pisistrius/i.test(`${giovaneNapoli.fantasyRoster || ''} ${giovaneNapoli.listone?.fantasyRoster || ''}`), JSON.stringify(giovaneNapoli && { name: giovaneNapoli.playerName, fantasyRoster: giovaneNapoli.fantasyRoster, listone: giovaneNapoli.listone?.fantasyRoster, method: giovaneNapoli.listoneMatchMethod }));
  ok('V603: Giovane Atalanta non eredita Real Pisistrius', !!giovaneAtalanta && !/real pisistrius/i.test(`${giovaneAtalanta.fantasyRoster || ''} ${giovaneAtalanta.listone?.fantasyRoster || ''}`), JSON.stringify(giovaneAtalanta && { name: giovaneAtalanta.playerName, fantasyRoster: giovaneAtalanta.fantasyRoster, listone: giovaneAtalanta.listone?.fantasyRoster, method: giovaneAtalanta.listoneMatchMethod }));

  ok('V603: Ederson nella card trattative Atalanta', (data.teamTransferTalksByTeam?.atalanta || []).some((x) => /ederson/i.test(x.target || '')), (data.teamTransferTalksByTeam?.atalanta || []).map((x) => x.target).join(', '));
  ok('V603: Davinson Sanchez nella card trattative Como', (data.teamTransferTalksByTeam?.como || []).some((x) => /davinson/i.test(x.target || '')), (data.teamTransferTalksByTeam?.como || []).map((x) => x.target).join(', '));
  ok('V603: Ben Nelson nella card trattative Torino', (data.teamTransferTalksByTeam?.torino || []).some((x) => /ben nelson/i.test(x.target || '')), (data.teamTransferTalksByTeam?.torino || []).map((x) => x.target).join(', '));
  ...
  ok('V603: Atta resta in rosa Fiorentina', fiorentinaPlayers.some((p) => /atta/i.test(p.playerName || '')), fiorentinaPlayers.map((p) => p.playerName).filter((n) => /atta/i.test(n)).join(', '));
  ok('V603: Gila resta in rosa Milan', milanPlayers.some((p) => /^gila$/i.test(p.playerName || '')), milanPlayers.map((p) => p.playerName).filter((n) => /gila/i.test(n)).join(', '));
  ok('V603: Luvumbo non in Cagliari', !cagliariPlayers.some((p) => /luvumbo/i.test(p.playerName || '')), cagliariPlayers.map((p) => p.playerName).filter((n) => /luvumbo/i.test(n)).join(', '));
}
...
for (const league of ['zonaorientale', 'fantapetillomantramanager']) {
  ...
  if (exists(index)) {
    ...
    ok(`${league}: CSS Sudatori V603 referenziato`, html.includes('sudatori-section-v603.css?v=603'), index);
    ok(`${league}: JS Sudatori V603 referenziato`, html.includes('sudatori-section-v603.js?v=603'), index);
  }
  const cfg = `static/${league}/assets/league-config.json`;
  if (exists(cfg)) {
    const config = JSON.parse(read(cfg));
    ok(`${league}: currentVersion V603`, String(config.currentVersion) === '603', config.currentVersion);
    ok(`${league}: feature Sudatori V603`, config.features?.sudatoriStandaloneSectionVersion === 'V603', config.features?.sudatoriStandaloneSectionVersion);
  }
}
...
if (failed.length) {
  console.error(`\nAudit V603 fallito: ${failed.length} errori.`);
  process.exit(1);
}
console.log('\nAudit V603 superato.');
