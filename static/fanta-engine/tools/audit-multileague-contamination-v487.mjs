...
check(zonaConfig.currentVersion === 487, 'currentVersion ZonaOrientale V487');
check(zonaNestedConfig.currentVersion === 487, 'currentVersion ZonaOrientale annidato V487');
check(fmmConfig.currentVersion === 487, 'currentVersion FantaMantraManager V487');
...
for (const file of zonaFooterPages) {
  check(read(file).includes('ZonaOrientale Salerno · V487 · Ultimo aggiornamento 24/06/2026'), `footer V487 ZonaOrientale in ${file}`);
}
...
for (const file of fmmFooterPages) {
  check(read(file).includes('FantaMantraManager · V487 · Ultimo aggiornamento 24/06/2026'), `footer V487 FantaMantraManager in ${file}`);
}
...
if (fail > 0) {
  console.error(`\nAudit anti-contaminazione V487 fallito: ${ok} OK, ${fail} FAIL`);
  for (const item of failures) console.error(` - ${item}`);
  process.exit(1);
}
console.log(`\nAudit anti-contaminazione V487 completato: ${ok} OK, ${fail} FAIL`);
