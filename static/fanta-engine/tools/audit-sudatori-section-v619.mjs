...
const fail = (msg) => { console.error('AUDIT V619 FAIL:', msg); process.exit(1); };
const read = (rel) => {
...
if (manifest.version !== 'V619' || data.meta.version !== 'V619') fail('versione manifest/data non V619');
...
const js = read('static/fanta-engine/js/sections/sudatori-section-v619.js');
...
for (const league of ['zonaorientale','fantapetillomantramanager']) {
  ...
  if (!html.includes('sudatori-section-v619.css?v=619')) fail('CSS Sudatori V619 non linkato in ' + league);
  if (!html.includes('sudatori-section-v619.js?v=619')) fail('JS Sudatori V619 non linkato in ' + league);
  const cfg = json(`static/${league}/assets/league-config.json`);
  if (cfg.currentVersion !== '619') fail('currentVersion non V619 in ' + league);
  if (cfg.features?.sudatoriLiveRosters !== 'V619') fail('feature sudatoriLiveRosters non V619 in ' + league);
}

console.log('AUDIT V619 OK', JSON.stringify({teams:data.teams.length, players:manifest.players, friendlies:manifest.friendlies, talks:manifest.teamTransferTalks, officialIn, officialOut, liveRosterRuntime: true}));
