...
for(const p of ['zonaorientale/index.html','zonaorientale/competition.html','zonaorientale/player.html']){noTerms(p,['FantaMantraManager','FantaPetillo','fantapetillomantramanager'],`${p} senza contaminazione FMM`);check(read(p).includes('ZonaOrientale Salerno · V500 · Ultimo aggiornamento 24/06/2026'),`${p} footer Zona V500`)}
...
check(read('fanta-engine/js/ui/components-v496.js').includes('UI_VERSION_V496'),'UI engine V500 presente');
...
if(fail>0){console.error(`\nAudit anti-contaminazione V500 fallito: ${ok} OK, ${fail} FAIL`);failures.forEach(f=>console.error(` - ${f}`));process.exit(1)}
...
console.log(`\nAudit anti-contaminazione V500 completato: ${ok} OK, ${fail} FAIL`);
