...
for(const p of ['zonaorientale/index.html','zonaorientale/competition.html','zonaorientale/player.html']){noTerms(p,['FantaMantraManager','FantaPetillo','fantapetillomantramanager'],`${p} senza contaminazione FMM`);check(read(p).includes('ZonaOrientale Salerno · V497 · Ultimo aggiornamento 24/06/2026'),`${p} footer Zona V497`)}
...
check(read('fanta-engine/js/ui/components-v496.js').includes('UI_VERSION_V496'),'UI engine V497 presente');
...
if(fail>0){console.error(`\nAudit anti-contaminazione V497 fallito: ${ok} OK, ${fail} FAIL`);failures.forEach(f=>console.error(` - ${f}`));process.exit(1)}
console.log(`\nAudit anti-contaminazione V497 completato: ${ok} OK, ${fail} FAIL`);

check(read('fanta-engine/js/core/feature-card-registry-v497.js').includes('FEATURE_CARD_REGISTRY_VERSION_V497'),'feature card registry V497 presente');
