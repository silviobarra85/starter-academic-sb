...
for(const p of ['zonaorientale/index.html','zonaorientale/competition.html','zonaorientale/player.html']){noTerms(p,['FantaMantraManager','FantaPetillo','fantapetillomantramanager'],`${p} senza contaminazione FMM`);check(read(p).includes('ZonaOrientale Salerno · V503 · Ultimo aggiornamento 24/06/2026'),`${p} footer Zona V503`)}
...
check(read('fanta-engine/js/ui/components-v496.js').includes('UI_VERSION_V496'),'UI engine V503 presente');
...
check(read('fanta-engine/js/ui/dashboard-cards-engine-v500.js').includes('DASHBOARD_CARDS_ENGINE_VERSION_V500'),'Dashboard cards engine V500 presente');
check(!read('fanta-engine/js/ui/dashboard-cards-engine-v500.js').includes('ZonaOrientale Salerno'),'Dashboard cards engine senza brand Zona hardcoded');
check(!read('fanta-engine/js/ui/dashboard-cards-engine-v500.js').includes('FantaMantraManager'),'Dashboard cards engine senza brand FMM hardcoded');
...
check(read('fanta-engine/js/tools/matchday-draw-engine-v501.js').includes('MATCHDAY_DRAW_ENGINE_VERSION_V501'),'Tool engine V503 presente');
...
check(read('fanta-engine/tools/playwright-smoke-v503.mjs').includes('FANTA_BASE_URL'), 'Playwright smoke V503 senza base URL hardcoded obbligatorio');
...
if(fail>0){console.error(`\nAudit anti-contaminazione V503 fallito: ${ok} OK, ${fail} FAIL`);failures.forEach(f=>console.error(` - ${f}`));process.exit(1)}
...
console.log(`\nAudit anti-contaminazione V503 completato: ${ok} OK, ${fail} FAIL`);
