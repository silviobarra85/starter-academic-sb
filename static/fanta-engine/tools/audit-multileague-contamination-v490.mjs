...
check(zonaConfig.currentVersion === 490, 'currentVersion ZonaOrientale V490');
check(zonaNestedConfig.currentVersion === 490, 'currentVersion ZonaOrientale annidato V490');
check(fmmConfig.currentVersion === 490, 'currentVersion FantaMantraManager V490');
...
for (const file of zonaFooterPages) {
  check(read(file).includes('ZonaOrientale Salerno · V490 · Ultimo aggiornamento 24/06/2026'), `footer V490 ZonaOrientale in ${file}`);
}
...
for (const file of fmmFooterPages) {
  check(read(file).includes('FantaMantraManager · V490 · Ultimo aggiornamento 24/06/2026'), `footer V490 FantaMantraManager in ${file}`);
}
...
if (fail > 0) {
  console.error(`\nAudit anti-contaminazione V490 fallito: ${ok} OK, ${fail} FAIL`);
  for (const item of failures) console.error(` - ${item}`);
  process.exit(1);
}
console.log(`\nAudit anti-contaminazione V490 completato: ${ok} OK, ${fail} FAIL`);
