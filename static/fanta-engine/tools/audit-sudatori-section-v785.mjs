...
function readJson(relativePath) {
  ...
  try {
    ...
    failures.push(`JSON non valido: ${relativePath} (${error.message})`);
    return null;
  }
}
...
check('ioSudo V785 non cancella i dati V782', manifest?.dataFile === 'sudatori-runtime.json' && manifest?.archiveDataFile === 'sudatori-data.json');

console.log(`Audit dati Sudatori V785: ${checks.filter((item) => item.ok).length}/${checks.length} controlli superati.`);
checks.forEach((item) => console.log(`${item.ok ? 'OK' : 'ERRORE'} - ${item.label}`));

if (failures.length) {
  console.error(`Audit dati Sudatori V785 fallito: ${failures.length} problemi.`);
  process.exit(1);
}
