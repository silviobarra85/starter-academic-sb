...
check(zonaConfig.currentVersion === 483, 'currentVersion ZonaOrientale V483');
check(zonaNestedConfig.currentVersion === 483, 'currentVersion ZonaOrientale annidato V483');
check(fmmConfig.currentVersion === 483, 'currentVersion FantaMantraManager V483');
...
for (const file of zonaFooterPages) {
  check(read(file).includes('ZonaOrientale Salerno · V483 · Ultimo aggiornamento 24/06/2026'), `footer V483 ZonaOrientale in ${file}`);
}
...
for (const file of fmmFooterPages) {
  check(read(file).includes('FantaMantraManager · V483 · Ultimo aggiornamento 24/06/2026'), `footer V483 FantaMantraManager in ${file}`);
}
...
if (fail > 0) {
  console.error(`\nAudit anti-contaminazione V483 fallito: ${ok} OK, ${fail} FAIL`);
  for (const item of failures) console.error(` - ${item}`);
  process.exit(1);
}
console.log(`\nAudit anti-contaminazione V483 completato: ${ok} OK, ${fail} FAIL`);
