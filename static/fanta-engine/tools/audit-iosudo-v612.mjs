...
const required = [
  ...
  'static/fanta-engine/js/apps/iosudo-app-v612.js',
  'static/fanta-engine/css/iosudo-app-v612.css',
...
function fail(message) {
  console.error('[ioSudo V612][FAIL]', message);
  process.exit(1);
}
...
if (!index.includes('../fanta-engine/css/iosudo-app-v612.css?v=612')) fail('ioSudo CSS not linked');
if (!index.includes('../fanta-engine/js/apps/iosudo-app-v612.js?v=612')) fail('ioSudo JS not linked');
...
const app = read('static/fanta-engine/js/apps/iosudo-app-v612.js');
...
const css = read('static/fanta-engine/css/iosudo-app-v612.css');
...
for (const league of ['zonaorientale', 'fantapetillomantramanager']) {
  ...
  if (!html.includes('data-iosudo-link-v612="true"')) fail('Missing ioSudo link in ' + league + ' index');
  ...
  if (cfg.currentVersion !== '612') fail('Wrong currentVersion in ' + league);
  if (!cfg.features || cfg.features.ioSudoPwa !== true) fail('Missing ioSudo feature flag in ' + league);
}

console.log('[ioSudo V612] Audit OK: player detail links, role colors, team-card stripes, hidden search state, sticky team menu, shared data source and league links verified.');
