...
const fail = (msg) => { console.error('AUDIT V617 FAIL:', msg); process.exit(1); };
const sum = (obj) => Object.values(obj || {}).reduce((a,b)=>a+(Array.isArray(b)?b.length:0),0);
const norm = (v) => String(v || '').toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g,'').replace(/[^a-z0-9]+/g,' ').trim();
if (manifest.version !== 'V617' || data.meta.version !== 'V617') fail('versione manifest/data non V617');
...
console.log('AUDIT V617 OK', JSON.stringify({teams:data.teams.length, players:manifest.players, friendlies:manifest.friendlies, talks:manifest.teamTransferTalks, officialIn, officialOut, injuries:manifest.injuries, filteredOfficialTalks:manifest.filteredOfficialTalks}));
