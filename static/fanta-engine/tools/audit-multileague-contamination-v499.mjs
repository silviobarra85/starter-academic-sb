...
for(const p of ['zonaorientale/index.html','zonaorientale/competition.html','zonaorientale/player.html']){noTerms(p,['FantaMantraManager','FantaPetillo','fantapetillomantramanager'],`${p} senza contaminazione FMM`);check(read(p).includes('ZonaOrientale Salerno · V499 · Ultimo aggiornamento 24/06/2026'),`${p} footer Zona V499`)}
...
check(read('fanta-engine/js/ui/components-v496.js').includes('UI_VERSION_V496'),'UI engine V499 presente');
...
if(fail>0){console.error(`\nAudit anti-contaminazione V499 fallito: ${ok} OK, ${fail} FAIL`);failures.forEach(f=>console.error(` - ${f}`));process.exit(1)}
...
console.log(`\nAudit anti-contaminazione V499 completato: ${ok} OK, ${fail} FAIL`);
