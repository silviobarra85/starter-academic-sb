...
check(zonaConfig.currentVersion === 491, 'currentVersion ZonaOrientale V491');
check(zonaNestedConfig.currentVersion === 491, 'currentVersion ZonaOrientale annidato V491');
check(fmmConfig.currentVersion === 491, 'currentVersion FantaMantraManager V491');
...
for (const file of zonaFooterPages) {
  check(read(file).includes('ZonaOrientale Salerno · V491 · Ultimo aggiornamento 24/06/2026'), `footer V491 ZonaOrientale in ${file}`);
}
...
for (const file of fmmFooterPages) {
  check(read(file).includes('FantaMantraManager · V491 · Ultimo aggiornamento 24/06/2026'), `footer V491 FantaMantraManager in ${file}`);
}
...
if (fail > 0) {
  console.error(`\nAudit anti-contaminazione V491 fallito: ${ok} OK, ${fail} FAIL`);
  for (const item of failures) console.error(` - ${item}`);
  process.exit(1);
}
console.log(`\nAudit anti-contaminazione V491 completato: ${ok} OK, ${fail} FAIL`);
