...
function fail(message) {
  console.error('[ioSudo V609][FAIL]', message);
  process.exit(1);
}
...
if (!index.includes('../fanta-engine/css/iosudo-app-v609.css?v=609')) fail('ioSudo CSS not linked');
if (!index.includes('../fanta-engine/js/apps/iosudo-app-v609.js?v=609')) fail('ioSudo JS not linked');
if (!index.includes('manifest.webmanifest')) fail('ioSudo webmanifest not linked');

const app = read('static/fanta-engine/js/apps/iosudo-app-v609.js');
...
for (const league of ['zonaorientale', 'fantapetillomantramanager']) {
  ...
  if (!html.includes('data-iosudo-link-v609="true"')) fail('Missing ioSudo link in ' + league + ' index');
  ...
  if (cfg.currentVersion !== '609') fail('Wrong currentVersion in ' + league);
  if (!cfg.features || cfg.features.ioSudoPwa !== true) fail('Missing ioSudo feature flag in ' + league);
}

console.log('[ioSudo V609] Audit OK: PWA shell, shared Sudatori data source, league links and feature flags verified.');
