...
for (const [cfgPath, leagueId, name] of configs) {
  ...
  check(cfg.currentVersion === 492, `${cfgPath} currentVersion V492`);
...
for (const file of htmlPages) {
  ...
  check(read(file).includes('?v=492'), `${file} cache-buster V492 presente`);
}
...
for (const file of [`${zonaRoot}/index.html`, `${zonaRoot}/competition.html`, `${zonaRoot}/player.html`, `${zonaNestedRoot}/index.html`, `${zonaNestedRoot}/competition.html`, `${zonaNestedRoot}/player.html`]) {
  checkIncludes(file, 'ZonaOrientale Salerno · V492 · Ultimo aggiornamento 24/06/2026', `footer V492 ZonaOrientale in ${file}`);
...
for (const file of [`${fmmRoot}/index.html`, `${fmmRoot}/competition.html`, `${fmmRoot}/player.html`]) {
  checkIncludes(file, 'FantaMantraManager · V492 · Ultimo aggiornamento 24/06/2026', `footer V492 FantaMantraManager in ${file}`);
...
for (const file of loaderFiles) {
  const text = read(file);
  check(text.includes("currentVersion: '492'"), `${file} currentVersion runtime V492`);
  check(text.includes('league-config.json?v=492'), `${file} fetch config V492`);
  check(text.includes("version: 'V492'"), `${file} marker runtime V492`);
...
for (const file of [
  ...
  'fanta-engine/tools/audit-runtime-regression-v492.mjs',
]) check(exists(file), `audit/tool presente: ${file}`);

if (fail > 0) {
  console.error(`\nAudit regressione runtime V492 fallito: ${ok} OK, ${fail} FAIL`);
  for (const item of failures) console.error(` - ${item}`);
  process.exit(1);
}
console.log(`\nAudit regressione runtime V492 completato: ${ok} OK, ${fail} FAIL`);
