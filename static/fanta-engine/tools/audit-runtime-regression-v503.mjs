...
for (const file of htmlFiles){ check(exists(file), `${file} presente`); checkRefs(file, `${file}`); check(read(file).includes('?v=503'), `${file} cache-buster V503`); }
for (const file of ['zonaorientale/index.html','zonaorientale/competition.html','zonaorientale/player.html']){ check(read(file).includes('ZonaOrientale Salerno · V503 · Ultimo aggiornamento 24/06/2026'), `footer ZonaOrientale V503 in ${file}`); noTerms(file,['FantaMantraManager','FantaPetillo','fantapetillomantramanager'],`${file} senza contaminazione FMM`); }
for (const file of ['fantapetillomantramanager/index.html','fantapetillomantramanager/competition.html','fantapetillomantramanager/player.html']){ check(read(file).includes('FantaMantraManager · V503 · Ultimo aggiornamento 24/06/2026'), `footer FMM V503 in ${file}`); noTerms(file,['ZonaOrientale','zonaorientale'],`${file} senza contaminazione ZonaOrientale`); }
noTerms('fantapetillomantramanager/news.html',['news-ujE2CqJMjzkYhhjzZZHD'],'news FMM senza ID storico ZonaOrientale');

for (const file of ['zonaorientale/assets/js/core/league-config-v443.js','fantapetillomantramanager/assets/js/core/league-config-v443.js']){ const text=read(file); check(text.includes("currentVersion: '503'"),`${file} runtime V503`); check(text.includes('league-config.json?v=503'),`${file} fetch config V503`);
...
for (const file of ['fanta-engine/js/core/unified-section-registry-v480.js','fanta-engine/js/core/league-presentation-v481.js','fanta-engine/js/core/data-paths-v490.js','fanta-engine/js/ui/components-v496.js','fanta-engine/js/core/feature-card-registry-v497.js','fanta-engine/js/ui/dashboard-cards-engine-v500.js','fanta-engine/js/tools/matchday-draw-engine-v501.js','fanta-engine/data/tool-engine-v501.json','fanta-engine/data/shared-assets/v485/assets/listoni/manifest.json','fanta-engine/data/shared-assets/v485/assets/calciomercato/links.json','fanta-engine/data/shared-css-assets-v487.json','fanta-engine/data/shared-js-modules-v491.json']) check(exists(file),`motore asset presente ${file}`);
...
for (const file of ['zonaorientale/assets/js/sections/matchday-draw-tool-v473.js','fantapetillomantramanager/assets/js/sections/matchday-draw-tool-v473.js']) { const text=read(file); check(text.includes('matchday-draw-engine-v501.js?v=503'), `${file} usa tool engine V503`); check(text.includes('initMatchdayDrawToolFallbackV473'), `${file} fallback locale V473`); }
...
check(exists('fanta-engine/tools/playwright-smoke-v503.mjs'), 'Playwright smoke V503 presente');
...
for (const cfgPath of ['zonaorientale/assets/league-config.json','fantapetillomantramanager/assets/league-config.json']) {
  ...
  check(cfg.features?.browserSmokeTestsVersion === 'V503', `${cfgPath} browser smoke version V503`);
}

if(fail>0){ console.error(`\nAudit regressione runtime V503 fallito: ${ok} OK, ${fail} FAIL`); failures.forEach(f=>console.error(` - ${f}`)); process.exit(1); }
console.log(`\nAudit regressione runtime V503 completato: ${ok} OK, ${fail} FAIL`);
