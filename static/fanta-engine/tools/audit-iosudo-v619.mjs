...
const required = [
  ...
  'static/fanta-engine/js/apps/iosudo-app-v619.js',
  'static/fanta-engine/css/iosudo-app-v619.css',
...
function fail(message) {
  console.error('[ioSudo V619][FAIL]', message);
  process.exit(1);
}
function read(rel) {
...
if (!index.includes('../fanta-engine/css/iosudo-app-v619.css?v=619')) fail('ioSudo CSS V619 not linked');
if (!index.includes('../fanta-engine/js/apps/iosudo-app-v619.js?v=619')) fail('ioSudo JS V619 not linked');
if (!index.includes('data-iosudo-version="619"')) fail('ioSudo data version not bumped');

const app = read('static/fanta-engine/js/apps/iosudo-app-v619.js');
for (const token of [
...
const css = read('static/fanta-engine/css/iosudo-app-v619.css');
for (const cls of [
...
if (!sw.includes('iosudo-shell-v619')) fail('Service worker cache not V619');
...
for (const league of ['zonaorientale', 'fantapetillomantramanager']) {
  ...
  if (!html.includes('data-iosudo-link-v619="true"')) fail('Missing ioSudo link in ' + league + ' index');
  ...
  if (cfg.currentVersion !== '619') fail('Wrong currentVersion in ' + league);
  if (cfg.features?.ioSudoLiveRosters !== 'V619') fail('Missing ioSudoLiveRosters feature flag in ' + league);
}
...
console.log('[ioSudo V619] Audit OK:', JSON.stringify({ liveRosterFile: latest.file, rosterPlayers: rosterPlayers.length, directMatches }));
