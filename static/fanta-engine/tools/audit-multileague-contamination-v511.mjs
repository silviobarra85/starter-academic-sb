...
for(const [p,id,name] of [['zonaorientale/assets/league-config.json','zonaorientale','ZonaOrientale Salerno'],['fantapetillomantramanager/assets/league-config.json','fantapetillomantramanager','FantaMantraManager']]){const c=readJson(p);check(c.leagueId===id,`${p} leagueId corretto`);check(c.name===name,`${p} nome corretto`);check(c.currentVersion===511,`${p} versione V511`);check(c.features?.navigationActionsEngineVersion==='V510',`${p} navigation actions V510 mantenuto`);check(c.features?.navigationDataRefresh===true,`${p} navigation data refresh V511`)}
for(const p of ['zonaorientale/index.html','zonaorientale/competition.html','zonaorientale/player.html']){noTerms(p,['FantaMantraManager','FantaPetillo','fantapetillomantramanager'],`${p} senza contaminazione FMM`);check(read(p).includes('ZonaOrientale Salerno · V511 · Ultimo aggiornamento 24/06/2026'),`${p} footer Zona V511`)}
...
check(read('../docs/OVERLAY_ROADMAP.md').includes('V512 - Report centralizzazione'), 'roadmap overlay aggiornata al prossimo step');

for (const file of ['zonaorientale/assets/app.js','fantapetillomantramanager/assets/app.js']) {
  const text = read(file);
  check(text.includes('navigation-data-refresh-v511.js?v=511'), `${file} navigation data refresh V511 presente`);
  check(text.includes('staticFirstFallback'), `${file} contiene fallback static-first V511`);
}
check(exists('fanta-engine/js/core/navigation-data-refresh-v511.js'), 'engine navigation data refresh V511 presente');

if(fail>0){console.error(`\nAudit anti-contaminazione V511 fallito: ${ok} OK, ${fail} FAIL`);failures.forEach(f=>console.error(` - ${f}`));process.exit(1)}
console.log(`\nAudit anti-contaminazione V511 completato: ${ok} OK, ${fail} FAIL`);
