...
const css = read('static/fanta-engine/css/player-tables-mobile-v572.css');
assert(css.includes('V572 - Mobile player tables reset'), 'CSS V572 presente');
...
for (const league of ['zonaorientale', 'fantapetillomantramanager']) {
  ...
  assert(index.includes('player-tables-mobile-v572.css?v=572'), `${league}: CSS V572 caricato`);
  assert(index.includes('V572'), `${league}: footer HTML V572`);
  assert(config.currentVersion === '572', `${league}: currentVersion V572`);
  assert(config.playerTablesMobileV572?.status === 'enabled', `${league}: metadata V572 in config`);
  assert(!('tableColumnResizerV571' in config), `${league}: metadata resize V571 rimossa`);
  assert(jsConfig.includes("currentVersion: '572'"), `${league}: fallback JS V572`);
}

if (fail.length) {
  console.error('Audit V572 fallito:');
  for (const item of fail) console.error(`- ${item}`);
  process.exit(1);
}
console.log('Audit V572 OK - tabelle giocatori mobile pulite e resize rimosso.');
