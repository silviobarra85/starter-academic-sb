...
const js = readText('static/fanta-engine/js/apps/iosudo-app-v717.js');
const css = readText('static/fanta-engine/css/iosudo-app-v717.css');
const sw = readText('static/iosudo/sw.js');
function assertOk(cond, msg) { if (!cond) { console.error('Audit V717 FAILED:', msg); process.exit(1); } }
...
assertOk(manifest.version === 'V717', 'manifest non aggiornato a V717');
assertOk(data.meta?.version === 'V717', 'data meta non aggiornata a V717');
assertOk(sw.includes('iosudo-shell-v717'), 'service worker non usa cache V717');
assertOk(sw.includes('iosudo-app-v717.js?v=717'), 'service worker non cachea JS V717');
assertOk(sw.includes('iosudo-app-v717.css?v=717'), 'service worker non cachea CSS V717');
...
assertOk(css.includes('V717'), 'CSS non marcato V717');
...
assertOk(Array.isArray(data.confirmedDuplicateAliasesV717) && data.confirmedDuplicateAliasesV717.length === 10, 'alias confermati V717 non archiviati');
assertOk(Array.isArray(data.duplicateNameCandidatesV717) && data.duplicateNameCandidatesV717.length === 10, 'lista 10 nuovi potenziali duplicati V717 assente');
...
for (const name of ['Iker Bravo','Jordan Zemura','Martin Payero']) {
  const row = (data.teamTransferTalksByTeam?.udinese || []).find((t) => canon(t.playerName || t.target) === canon(name) && t.updateFlag === 'V717');
  assertOk(Boolean(row && /watford/i.test(String(row.status || '') + ' ' + String(row.note || '')) && String(row.url || '').includes('calciomercato-news-trattative-oggi-18-luglio')), 'Trattativa Udinese-Watford V717 mancante: ' + name);
}
const atalantaU23 = (data.friendliesByTeam?.atalanta || []).find((item) => /under 23|u23/i.test(String(item.event || item.playerName || '')));
assertOk(Boolean(atalantaU23 && String(atalantaU23.url || '').includes('preseason-2026-27') && atalantaU23.hasPlayerStats !== true), 'Atalanta-U23 V717 assente o con tabellino improprio');
...
console.log('Audit ioSudo V717 OK', JSON.stringify({ players: manifest.players, official: manifest.officialMoves, talks: manifest.teamTransferTalks, injuries: manifest.injuries, friendlies: manifest.friendlies, sources: manifest.sources, activeOfficialRumors: manifest.activeOfficialRumors, duplicateExactPlayers: manifest.duplicateExactPlayers, duplicatePlayerIds: manifest.duplicatePlayerIds, confirmedAliases: manifest.confirmedAliasesV717, duplicateCandidates: manifest.duplicateCandidatesV717 }));
