...
const activeOfficialRumors=(data.activeOfficialRumorsAuditV748||[]).length;
if (activeOfficialRumors) throw new Error('Rumor attivi su ufficialita: '+activeOfficialRumors);
console.log('Audit ioSudo V748 OK');
console.log(JSON.stringify({players:ids.length, friendlyMatchDetails:friendlyDetails.length, friendlyPlayerStats:friendlyDetails.reduce((a,m)=>a+(m.players||[]).length,0), duplicateIds:idDups, exactDups, activeOfficialRumors},null,2));
