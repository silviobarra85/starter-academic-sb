...
const css = 'static/fanta-engine/css/sudatori-section-v605.css';
const js = 'static/fanta-engine/js/sections/sudatori-section-v605.js';
...
ok('CSS V605 presente', exists(css), css);
ok('JS V605 presente', exists(js), js);
...
if (exists(js)) {
  ...
  ok('Versione runtime V605', /const VERSION = 'V605'/.test(source), 'VERSION V605');
  ...
  ok('V605: helper market signal presente', /function\s+renderMarketSignal\s*\(/.test(source), 'renderMarketSignal');
  ok('V605: marketNotesFor risolve note anche per nome esteso/team', /function\s+marketNotesFor\s*\(/.test(source) && /noteTeamMatchesPlayer/.test(source), 'marketNotesFor team-aware');
  ok('V605: badge unico NUOVO RUMOR CONFERMATO', /marketBadgeInfo/.test(source) && /label: 'NUOVO'/.test(source) && /label: 'RUMOR'/.test(source) && /label: 'CONFERMATO'/.test(source), 'marketBadgeInfo');
  ok('V605: testo neutro non genera RUMOR', /neutralMarketText/.test(source) && !/rumor\|trattativ\|mercato\|interess/.test(source), 'neutralMarketText + no keyword mercato');
  ok('V605: colonna Mercato renderizza HTML badge', /<td>\$\{mercato\}<\/td>/.test(source), 'mercato HTML');
}
if (exists(css)) {
  const source = read(css);
  ok('CSS infortunati presente', /sudatori-injuries-v605/.test(source), 'sudatori-injuries-v605');
  ok('CSS badge segnalazione campo rosso', /is-field-alert/.test(source), 'is-field-alert');
  ok('CSS V605 badge mercato presente', /sudatori-market-flag-v605/.test(source), 'sudatori-market-flag-v605');
  ok('CSS V605 badge NUOVO presente', /is-new/.test(source), 'is-new');
  ok('CSS V605 badge RUMOR presente', /is-rumor/.test(source), 'is-rumor');
  ok('CSS V605 badge CONFERMATO presente', /is-confirmed/.test(source), 'is-confirmed');
}
if (exists(manifestPath)) {
  const manifest = JSON.parse(read(manifestPath));
  ok('Manifest V605', manifest.version === 'V605', manifest.version);
...
if (exists(dataPath)) {
  ...
  ok('Meta V605', data.meta?.version === 'V605', data.meta?.version);
  ...
  ok('V605: Atalanta-Atalanta Under 23 presente', (data.friendliesByTeam?.atalanta || []).some((f) => /under 23/i.test(f.event || '')), (data.friendliesByTeam?.atalanta || []).map((f) => f.event).join(', '));
  ok('V605: Real Madrid-Fiorentina presente', (data.friendliesByTeam?.fiorentina || []).some((f) => /real madrid/i.test(f.event || '')), (data.friendliesByTeam?.fiorentina || []).map((f) => f.event).join(', '));
  ok('V605: Burnley-Torino presente', (data.friendliesByTeam?.torino || []).some((f) => /burnley/i.test(f.event || '')), (data.friendliesByTeam?.torino || []).map((f) => f.event).join(', '));
  ok('V605: Lecce ha 4 amichevoli aggiornate', (data.friendliesByTeam?.lecce || []).length === 4, (data.friendliesByTeam?.lecce || []).map((f) => f.event).join(', '));
  ok('V605: Udinese senza amichevoli ma ritiro Lienz', (data.friendliesByTeam?.udinese || []).length === 0 && /lienz/i.test((data.teams || []).find((t) => t.id === 'udinese')?.retreatPlace || ''), JSON.stringify({ friendlies: data.friendliesByTeam?.udinese, team: (data.teams || []).find((t) => t.id === 'udinese') }));

  const rumors = data.transfermarktRumors || [];
  const rumorNames = rumors.map((x) => `${x.teamName}:${x.playerName}:${x.type}`).join(' | ');
  ok('V605: almeno 45 rumors Transfermarkt pagine 1-20 caricati', rumors.length >= 45, String(rumors.length));
  ok('V605: pagine Transfermarkt 1-20 tracciate', (data.transfermarktRumorPages || []).length === 20, String((data.transfermarktRumorPages || []).length));
  ok('V605: Ederson rumor uscita Atalanta', rumors.some((x) => /atalanta/i.test(x.teamName || '') && /ederson/i.test(x.playerName || '') && /uscita/i.test(x.type || '')), rumorNames);
  ok('V605: Davinson Sanchez rumor Como', rumors.some((x) => /como/i.test(x.teamName || '') && /davinson/i.test(x.playerName || '')), rumorNames);
  ok('V605: Sergi Dominguez rumor Lazio', rumors.some((x) => /lazio/i.test(x.teamName || '') && /sergi/i.test(x.playerName || '')), rumorNames);
  const noteFor = (name) => data.marketNotesByPlayer?.[String(name || '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().replace(/[^a-z0-9]+/g, ' ').trim()] || [];
  ok('V605: Bremer ha rumor Transfermarkt anche se stato base puo essere In rosa', noteFor('Bremer').some((x) => /transfermarkt/i.test(`${x.source || ''} ${x.note || ''}`) && /uscita/i.test(x.status || x.type || '')), JSON.stringify(noteFor('Bremer')));
  ok('V605: Dodo ha rumor Transfermarkt nella lista note giocatore', noteFor('Dodo').some((x) => /transfermarkt/i.test(`${x.source || ''} ${x.note || ''}`) && /uscita/i.test(x.status || x.type || '')), JSON.stringify(noteFor('Dodo')));
  ...
  ok('V605: giocatore neutro mostra CONFERMATO, non RUMOR', !!neutralPlayer && testBadge(neutralPlayer, noteFor('Carnesecchi')) === 'CONFERMATO', JSON.stringify(neutralPlayer && { name: neutralPlayer.playerName, marketStatus: neutralPlayer.marketStatus, marketDetail: neutralPlayer.marketDetail, badge: testBadge(neutralPlayer, noteFor('Carnesecchi')) }));
  ok('V605: Bremer mostra RUMOR da Transfermarkt', !!bremer && testBadge(bremer, noteFor('Bremer')) === 'RUMOR', JSON.stringify(bremer && { name: bremer.playerName, marketStatus: bremer.marketStatus, badge: testBadge(bremer, noteFor('Bremer')), notes: noteFor('Bremer').slice(0, 2) }));
  ...
  ok('V605: Gaetano Atalanta marcato nuovo acquisto', !!gaetano && gaetano.newAcquisition === true && /acquisto/i.test(gaetano.marketStatus || ''), JSON.stringify(gaetano && { name: gaetano.playerName, status: gaetano.marketStatus, flag: gaetano.flag, newAcquisition: gaetano.newAcquisition }));
  ok('V605: nota mercato Gaetano risolta anche con chiave breve', noteFor('Gaetano').some((x) => /atalanta/i.test(x.team || '') && /arrivo/i.test(x.note || '')), JSON.stringify(noteFor('Gaetano')));
  ok('V605: Giovane Napoli assegnato a Real Pisistrius', !!giovaneNapoli && /real pisistrius/i.test(`${giovaneNapoli.fantasyRoster || ''} ${giovaneNapoli.listone?.fantasyRoster || ''}`), JSON.stringify(giovaneNapoli && { name: giovaneNapoli.playerName, fantasyRoster: giovaneNapoli.fantasyRoster, listone: giovaneNapoli.listone?.fantasyRoster, method: giovaneNapoli.listoneMatchMethod }));
  ok('V605: Giovane Atalanta non eredita Real Pisistrius', !!giovaneAtalanta && !/real pisistrius/i.test(`${giovaneAtalanta.fantasyRoster || ''} ${giovaneAtalanta.listone?.fantasyRoster || ''}`), JSON.stringify(giovaneAtalanta && { name: giovaneAtalanta.playerName, fantasyRoster: giovaneAtalanta.fantasyRoster, listone: giovaneAtalanta.listone?.fantasyRoster, method: giovaneAtalanta.listoneMatchMethod }));

  ok('V605: Ederson nella card trattative Atalanta', (data.teamTransferTalksByTeam?.atalanta || []).some((x) => /ederson/i.test(x.target || '')), (data.teamTransferTalksByTeam?.atalanta || []).map((x) => x.target).join(', '));
  ok('V605: Davinson Sanchez nella card trattative Como', (data.teamTransferTalksByTeam?.como || []).some((x) => /davinson/i.test(x.target || '')), (data.teamTransferTalksByTeam?.como || []).map((x) => x.target).join(', '));
  ok('V605: Ben Nelson nella card trattative Torino', (data.teamTransferTalksByTeam?.torino || []).some((x) => /ben nelson/i.test(x.target || '')), (data.teamTransferTalksByTeam?.torino || []).map((x) => x.target).join(', '));
  ...
  ok('V605: Atta resta in rosa Fiorentina', fiorentinaPlayers.some((p) => /atta/i.test(p.playerName || '')), fiorentinaPlayers.map((p) => p.playerName).filter((n) => /atta/i.test(n)).join(', '));
  ok('V605: Gila resta in rosa Milan', milanPlayers.some((p) => /^gila$/i.test(p.playerName || '')), milanPlayers.map((p) => p.playerName).filter((n) => /gila/i.test(n)).join(', '));
  ok('V605: Luvumbo non in Cagliari', !cagliariPlayers.some((p) => /luvumbo/i.test(p.playerName || '')), cagliariPlayers.map((p) => p.playerName).filter((n) => /luvumbo/i.test(n)).join(', '));
}
...
for (const league of ['zonaorientale', 'fantapetillomantramanager']) {
  ...
  if (exists(index)) {
    ...
    ok(`${league}: CSS Sudatori V605 referenziato`, html.includes('sudatori-section-v605.css?v=605'), index);
    ok(`${league}: JS Sudatori V605 referenziato`, html.includes('sudatori-section-v605.js?v=605'), index);
  }
  const cfg = `static/${league}/assets/league-config.json`;
  if (exists(cfg)) {
    const config = JSON.parse(read(cfg));
    ok(`${league}: currentVersion V605`, String(config.currentVersion) === '605', config.currentVersion);
    ok(`${league}: feature Sudatori V605`, config.features?.sudatoriStandaloneSectionVersion === 'V605', config.features?.sudatoriStandaloneSectionVersion);
  }
}
...
if (failed.length) {
  console.error(`\nAudit V605 fallito: ${failed.length} errori.`);
  process.exit(1);
}
console.log('\nAudit V605 superato.');
