...
for (const [cfgPath, leagueId, name] of [['zonaorientale/assets/league-config.json','zonaorientale','ZonaOrientale Salerno'],['fantapetillomantramanager/assets/league-config.json','fantapetillomantramanager','FantaMantraManager']]) {
  ...
  check(cfg.currentVersion===495, `${cfgPath} currentVersion V495`);
...
for (const file of htmlFiles){ check(exists(file), `${file} presente`); checkRefs(file, `${file}`); check(read(file).includes('?v=495'), `${file} cache-buster V495`); }
for (const file of ['zonaorientale/index.html','zonaorientale/competition.html','zonaorientale/player.html']){ check(read(file).includes('ZonaOrientale Salerno · V495 · Ultimo aggiornamento 24/06/2026'), `footer ZonaOrientale V495 in ${file}`); noTerms(file,['FantaMantraManager','FantaPetillo','fantapetillomantramanager'],`${file} senza contaminazione FMM`); }
for (const file of ['fantapetillomantramanager/index.html','fantapetillomantramanager/competition.html','fantapetillomantramanager/player.html']){ check(read(file).includes('FantaMantraManager · V495 · Ultimo aggiornamento 24/06/2026'), `footer FMM V495 in ${file}`); noTerms(file,['ZonaOrientale','zonaorientale'],`${file} senza contaminazione ZonaOrientale`); }
noTerms('fantapetillomantramanager/news.html',['news-ujE2CqJMjzkYhhjzZZHD'],'news FMM senza ID storico ZonaOrientale');

for (const file of ['zonaorientale/assets/js/core/league-config-v443.js','fantapetillomantramanager/assets/js/core/league-config-v443.js']){ const text=read(file); check(text.includes("currentVersion: '495'"),`${file} runtime V495`); check(text.includes('league-config.json?v=495'),`${file} fetch config V495`); check(!text.includes('audit clone FantaPetillo'),`${file} senza clone footer`); }

for (const file of ['fanta-engine/js/core/unified-section-registry-v480.js','fanta-engine/js/core/league-presentation-v481.js','fanta-engine/js/core/data-paths-v490.js','fanta-engine/data/shared-assets/v485/assets/listoni/manifest.json','fanta-engine/data/shared-assets/v485/assets/calciomercato/links.json','fanta-engine/data/shared-css-assets-v487.json','fanta-engine/data/shared-js-modules-v491.json']) check(exists(file),`motore asset presente ${file}`);
...
if(fail>0){ console.error(`\nAudit regressione runtime V495 fallito: ${ok} OK, ${fail} FAIL`); failures.forEach(f=>console.error(` - ${f}`)); process.exit(1); }
console.log(`\nAudit regressione runtime V495 completato: ${ok} OK, ${fail} FAIL`);
