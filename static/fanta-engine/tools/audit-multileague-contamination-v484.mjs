...
check(zonaConfig.currentVersion === 484, 'currentVersion ZonaOrientale V484');
check(zonaNestedConfig.currentVersion === 484, 'currentVersion ZonaOrientale annidato V484');
check(fmmConfig.currentVersion === 484, 'currentVersion FantaMantraManager V484');
...
for (const file of zonaFooterPages) {
  check(read(file).includes('ZonaOrientale Salerno · V484 · Ultimo aggiornamento 24/06/2026'), `footer V484 ZonaOrientale in ${file}`);
}
...
for (const file of fmmFooterPages) {
  check(read(file).includes('FantaMantraManager · V484 · Ultimo aggiornamento 24/06/2026'), `footer V484 FantaMantraManager in ${file}`);
}
...
if (fail > 0) {
  console.error(`\nAudit anti-contaminazione V484 fallito: ${ok} OK, ${fail} FAIL`);
  for (const item of failures) console.error(` - ${item}`);
  process.exit(1);
}
console.log(`\nAudit anti-contaminazione V484 completato: ${ok} OK, ${fail} FAIL`);
