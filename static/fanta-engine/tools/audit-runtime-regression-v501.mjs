...
for (const [cfgPath, leagueId, name] of [['zonaorientale/assets/league-config.json','zonaorientale','ZonaOrientale Salerno'],['fantapetillomantramanager/assets/league-config.json','fantapetillomantramanager','FantaMantraManager']]) {
  ...
  check(cfg.currentVersion===501, `${cfgPath} currentVersion V501`);
...
for (const file of htmlFiles){ check(exists(file), `${file} presente`); checkRefs(file, `${file}`); check(read(file).includes('?v=501'), `${file} cache-buster V501`); }
for (const file of ['zonaorientale/index.html','zonaorientale/competition.html','zonaorientale/player.html']){ check(read(file).includes('ZonaOrientale Salerno · V501 · Ultimo aggiornamento 24/06/2026'), `footer ZonaOrientale V501 in ${file}`); noTerms(file,['FantaMantraManager','FantaPetillo','fantapetillomantramanager'],`${file} senza contaminazione FMM`); }
for (const file of ['fantapetillomantramanager/index.html','fantapetillomantramanager/competition.html','fantapetillomantramanager/player.html']){ check(read(file).includes('FantaMantraManager · V501 · Ultimo aggiornamento 24/06/2026'), `footer FMM V501 in ${file}`); noTerms(file,['ZonaOrientale','zonaorientale'],`${file} senza contaminazione ZonaOrientale`); }
noTerms('fantapetillomantramanager/news.html',['news-ujE2CqJMjzkYhhjzZZHD'],'news FMM senza ID storico ZonaOrientale');

for (const file of ['zonaorientale/assets/js/core/league-config-v443.js','fantapetillomantramanager/assets/js/core/league-config-v443.js']){ const text=read(file); check(text.includes("currentVersion: '501'"),`${file} runtime V501`); check(text.includes('league-config.json?v=501'),`${file} fetch config V501`);
...
for (const file of ['zonaorientale/assets/js/sections/matchday-draw-tool-v473.js','fantapetillomantramanager/assets/js/sections/matchday-draw-tool-v473.js']) { const text=read(file); check(text.includes('matchday-draw-engine-v501.js?v=501'), `${file} usa tool engine V501`); check(text.includes('initMatchdayDrawToolFallbackV473'), `${file} fallback locale V473`); }
...
if(fail>0){ console.error(`\nAudit regressione runtime V501 fallito: ${ok} OK, ${fail} FAIL`); failures.forEach(f=>console.error(` - ${f}`)); process.exit(1); }
console.log(`\nAudit regressione runtime V501 completato: ${ok} OK, ${fail} FAIL`);
