...
const fail = (msg) => { console.error('AUDIT V622 FAIL:', msg); process.exit(1); };
...
if (manifest.version !== 'V622' || data.meta.version !== 'V622') fail('versione manifest/data non V622');
...
if ((data.retreatFriendliesControlSourcesV622||[]).length !== manifest.extraControlFriendliesSourcesAddedV622) fail('fonti controllo ritiri V622 non coerenti');
if (manifest.extraFriendliesAddedV622 !== 4) fail('amichevoli Udinese V622 attese: 4');
if (!String(data.teams.find(t=>t.id==='udinese')?.notes || '').includes('Bwin')) fail('nota Udinese Bwin mancante');
...
if (talks < 178) fail('troppe poche trattative aggregate: ' + talks);
const js = read('static/fanta-engine/js/sections/sudatori-section-v622.js');
...
for (const league of ['zonaorientale','fantapetillomantramanager']) {
  ...
  if (!html.includes('sudatori-section-v622.css?v=622')) fail('CSS Sudatori V622 non linkato in ' + league);
  if (!html.includes('sudatori-section-v622.js?v=622')) fail('JS Sudatori V622 non linkato in ' + league);
  const cfg = json(`static/${league}/assets/league-config.json`);
  if (cfg.currentVersion !== '622') fail('currentVersion non 622 in ' + league);
  if (cfg.features?.sudatoriLiveRosters !== 'V622') fail('feature sudatoriLiveRosters non V622 in ' + league);
}
console.log('AUDIT V622 OK', JSON.stringify({teams:data.teams.length, players:manifest.players, friendlies:manifest.friendlies, talks:manifest.teamTransferTalks, officialIn, officialOut, sources:manifest.sources}));
