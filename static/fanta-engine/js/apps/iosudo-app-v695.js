(function () {
  ...
  function fastPlayerDedupKeysV695(player) {
  ...
  function dedupeFastPlayerRowsV695(rows) {
    const seen = new Map();
    const out = [];
    (rows || []).forEach(function (row) {
      const keys = fastPlayerDedupKeysV695(row && row.player);
      const hit = keys.find(function (key) { return seen.has(key); });
      if (hit) {
        const existing = seen.get(hit);
        if (existing && existing.player && row && row.player) {
          if (!existing.player.listone && row.player.listone) existing.player.listone = row.player.listone;
          if (!existing.player.fantasyRoster && row.player.fantasyRoster) existing.player.fantasyRoster = row.player.fantasyRoster;
          existing.player._iosudoMarketRows = uniqueMarketItems([].concat(existing.player._iosudoMarketRows || [], row.player._iosudoMarketRows || []), 'dedupe-market');
        }
        return;
      }
      keys.forEach(function (key) { seen.set(key, row); });
      out.push(row);
    });
    return out;
  }
  ...
  function collectFastPlayerRows() {
    ...
    const dedupedRowsV695 = dedupeFastPlayerRowsV695(rows);
    rows.length = 0;
    dedupedRowsV695.forEach(function (row) { rows.push(row); });
...
/* V695: aggiornamento dati globale v41; RUMOR/UFFICIALITA raggruppate e header con data+ora. */

/* V695: aggiornamento globale v42; deduplica GIOCATORI, fix SOS in Rosa, ufficialita/rinnovi in Rosa. */
