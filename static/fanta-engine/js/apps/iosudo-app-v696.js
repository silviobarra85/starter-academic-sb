(function () {
  ...
  function samePersonNameV696(a, b, allowSurnameOnly) {
    const ia = nameInfoV696(a);
    const ib = nameInfoV696(b);
  ...
  function samePersonRoleV696(a, b, allowSurnameOnly) {
    if (!a || !b) return false;
    ...
    return samePersonNameV696(a.playerName || a.originalName || a.disambiguatedName, b.playerName || b.originalName || b.disambiguatedName, allowSurnameOnly);
  }

  function mergeFastPlayerRowV696(target, source) {
    ...
    a._iosudoMarketRows = uniqueMarketItems([].concat(a._iosudoMarketRows || [], b._iosudoMarketRows || [], b.virtualMarketRows || []), 'dedupe-v696-market');
    const aInfo = nameInfoV696(a.playerName || '');
    const bInfo = nameInfoV696(b.playerName || '');
  ...
  function findUniqueFastRowForListoneV696(item, rows) {
    ...
    const strict = (rows || []).filter(function (row) {
      ...
      if (!samePersonRoleV696(p, probe, true)) return false;
    ...
    const loose = (rows || []).filter(function (row) {
      const p = row && row.player;
      return p && !p.listoneOnlyPlayer && !p.virtualMarketPlayer && samePersonRoleV696(p, probe, true);
    });
  ...
  function fastPlayerDedupKeysV696(player) {
  ...
  function dedupeFastPlayerRowsV696(rows) {
    const out = [];
  ...
  function collectFastPlayerRows() {
    ...
    if (state.latestListone && state.latestListone.active) {
      ...
      (state.latestListone.players || []).forEach(function (item) {
        ...
        const matchedRow = findUniqueFastRowForListoneV696(item, rows);
    ...
    const dedupedRowsV696 = dedupeFastPlayerRowsV696(rows);
    rows.length = 0;
    dedupedRowsV696.forEach(function (row) { rows.push(row); });
...
/* V696: aggiornamento dati globale v41; RUMOR/UFFICIALITA raggruppate e header con data+ora. */

/* V696: deduplica conservativa GIOCATORI/Rosa da lista duplicati utente; matching listone per persona anche se la squadra del listone e arretrata. */
