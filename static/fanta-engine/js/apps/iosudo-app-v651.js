(function () {
  ...
  function fastPlayerBadge(player) {
    ...
    if (text === 'NUOVO') return { text: text, cls: 'iosudo-badge-new' };
  ...
  function fastCurrentTeamText(player) {
    ...
    return String(player && (player.realTeamCurrent || player.realTeam || player.teamName || player.realTeamOriginal) || listTeam || '').trim() || '-';
  }
  ...
  function prepareFastPlayerRow(player) {
    ...
    const row = {
      ...
      _iosudoFastSource: (listonePresent ? 'Listone' : 'Dati ioSudo'),
  ...
  function bindPlayerDetail() {
    // V651: gestione delegata sul contenitore focus, senza listener ricreati a ogni dettaglio.
  }
  ...
  function bindTeamPanel() {
    // V651: niente listener per-tab/per-card sul pannello. Gestione delegata una sola volta in init().
  }
