(function () {
  ...
  // V781: le uscite ufficiali sono sempre lette dal bucket della squadra di origine, senza filtri sulla lega di destinazione.
  ...
  const state = {
    ...
    marketRowResolvedPlayerCache: new Map(),
    playerByIdIndex: new Map(),
    playerSourceIndex: new Map(),
    playerSourceKeyIndex: new Map(),
    catalogStatsV757: null,
    detailRenderingId: '',
  ...
  function playerNameCandidates(player) {
    ...
    if (player && player.canonicalFullName && player.canonicalFullName !== raw) names.push({ value: player.canonicalFullName, teamOnly: false });
    (player && (player.nameAliasesV757 || player.nameAliasesV755) || []).forEach(function (value) { names.push({ value: value, teamOnly: true }); });
  ...
  async function loadLatestListone() {
    state.latestListone = { loaded: false, active: false, entries: 0, map: new Map(), globalMap: new Map(), idMap: new Map(), players: [], source: '', label: '' };
    try {
      ...
      players.forEach(function (player, index) {
        ...
        if (value.fantacalcioId) idMap.set(String(value.fantacalcioId), value);
        rosterNameKeys(playerName).forEach(function (nameKey) {
          addRosterCandidate(map, nameKey + '::' + realTeam + '::' + role, value);
          addRosterCandidate(map, nameKey + '::' + realTeam, value);
          addRosterCandidate(globalMap, nameKey + '::' + role, value);
      ...
      state.latestListone = { loaded: true, active: players.length > 0, entries: players.length, map: map, globalMap: globalMap, idMap: idMap, players: players.map(function (player, index) { const playerName = player && (player.playerName || player.name); const realTeam = teamKey(player.realTeam || player.realTeamOriginal || player.teamName || ''); const role = roleKeyForRoster(player.classicRole || player.role || player.rosterRole || ''); return Object.assign({}, player, { playerName: playerName, realTeam: realTeam, role: role, listoneFile: entry.file, sourceIndex: index }); }).filter(function (player) { return player.playerName; }), source: entry.file, label: entry.label || payload?.meta?.label || entry.id || '' };
    } catch (error) {
  ...
  function latestListoneFor(player) {
    ...
    if (state.latestListone && state.latestListone.active && state.latestListone.idMap && player.fantacalcioId) {
      const byId = state.latestListone.idMap.get(String(player.fantacalcioId));
      if (byId && listoneMatchesProtectedPessinaV768(player, byId)) return byId;
    }
    if (player.listone && (!state.latestListone || !state.latestListone.loaded)) return player.listone;
    if (!state.latestListone || !state.latestListone.active) return player.listone || null;
    ...
    for (let c = 0; c < candidates.length; c += 1) {
      ...
      for (let k = 0; k < keys.length; k += 1) {
        ...
        for (let t = 0; t < teams.length; t += 1) {
          ...
          if (acceptedListoneCandidateV768(player, byTeamRole)) return byTeamRole;
          const byTeam = uniqueListoneCandidate(state.latestListone.map.get(nameKey + '::' + team), player);
          if (acceptedListoneCandidateV768(player, byTeam)) return byTeam;
        }
        if (!item.teamOnly) {
          const byGlobalRole = uniqueListoneCandidate(state.latestListone.globalMap.get(nameKey + '::' + role), player);
          if (acceptedListoneCandidateV768(player, byGlobalRole)) return byGlobalRole;
          const globalList = state.latestListone.globalMap.get(nameKey) || [];
          if (globalList.length === 1 && acceptedListoneCandidateV768(player, globalList[0])) return globalList[0];
        }
      }
    }
    return acceptedListoneCandidateV768(player, looseListoneForPlayer(player));
  }
  ...
  function looseListoneForPlayer(player) {
    ...
    candidates.forEach(function (item) {
      rosterNameKeys(item.value).forEach(function (nameKey) {
        ...
        rows.forEach(function (entry) {
          const entryTeam = teamKey(entry.realTeam || entry.realTeamOriginal || entry.teamName || '');
          if (item.teamOnly && teams.size && !teams.has(entryTeam)) return;
    ...
    if (pool.length === 1) {
      const only = pool[0];
      const onlyTeam = teamKey(only.realTeam || only.realTeamOriginal || only.teamName || '');
      const onlyRole = roleKeyForRoster(only.classicRole || only.role || only.rosterRole || '');
      const teamMatches = !teams.size || teams.has(onlyTeam);
      const roleMatches = !role || !onlyRole || role === onlyRole;
      if (!teamMatches && !roleMatches) return null;
      return only;
    }
    if (role) {
  ...
  function setupLeagueChrome() {
    ...
    document.title = 'ioSudo V781 - ' + league.name;
  }
  ...
  function teamCounters(teamId, summary) {
    ...
    const talksOther = Array.isArray(currentSummary.talksOther) ? currentSummary.talksOther.length : 0;
    ...
    return {
      ...
      talksOther: talksOther,
      rumors: talksIncoming + talksOutgoing + talksOther,
  ...
  function emptyPlayerSourceBucketV757() {
    return { talks: [], officialIn: [], officialOut: [], injuries: [], formations: [], friendlies: [] };
  }

  function canonicalIdsForItemV757(item) {
    if (!item) return [];
    const ids = Array.isArray(item.canonicalPlayerIds) ? item.canonicalPlayerIds.slice() : [];
    if (item.canonicalPlayerId) ids.push(item.canonicalPlayerId);
    return Array.from(new Set(ids.filter(Boolean).map(String)));
  }

  function sourceBucketForPlayerV757(player) {
    const id = String(player && player.id || '');
    return id && state.playerSourceIndex ? state.playerSourceIndex.get(id) || null : null;
  }

  function addIndexedSourceV757(playerId, key, item) {
    const id = String(playerId || '');
    if (!id || !state.playerByIdIndex.has(id) || !item) return;
    const bucket = state.playerSourceIndex.get(id) || emptyPlayerSourceBucketV757();
    if (!state.playerSourceIndex.has(id)) state.playerSourceIndex.set(id, bucket);
    const uniqueKey = id + '::' + key + '::' + marketItemKey(item, key);
    if (state.playerSourceKeyIndex.has(uniqueKey)) return;
    state.playerSourceKeyIndex.set(uniqueKey, true);
    bucket[key].push(item);
  }

  function indexItemForCanonicalPlayersV757(item, key) {
    canonicalIdsForItemV757(item).forEach(function (playerId) {
      addIndexedSourceV757(playerId, key, item);
    });
  }

  const PROTECTED_PESSINA_V768 = Object.freeze({
    'bologna-pessina-mas': Object.freeze({ id: 'bologna-pessina-mas', teamId: 'bologna', teamName: 'Bologna', playerName: 'Massimo Pessina', role: 'P' }),
    'monza-pessina-mas': Object.freeze({ id: 'monza-pessina-mas', teamId: 'monza', teamName: 'Monza', playerName: 'Matteo Pessina', role: 'C' })
  });

  function normalizedPessinaIdentityV768(item) {
    if (!item || typeof item !== 'object') return null;
    const ids = [];
    if (item.id) ids.push(String(item.id));
    if (item.canonicalPlayerId) ids.push(String(item.canonicalPlayerId));
    if (Array.isArray(item.canonicalPlayerIds)) item.canonicalPlayerIds.forEach(function (id) { if (id) ids.push(String(id)); });
    const direct = ids.map(function (id) { return PROTECTED_PESSINA_V768[id]; }).find(Boolean);
    if (direct) return direct;
    ...
    if (name.indexOf('pessina') === -1) return null;
    if (team === 'bol') return PROTECTED_PESSINA_V768['bologna-pessina-mas'];
    if (team === 'mon') return PROTECTED_PESSINA_V768['monza-pessina-mas'];
    return null;
  }


  function protectedPessinaIdentityForPlayerV768(player) {
    return normalizedPessinaIdentityV768(player);
  }

  function listoneMatchesProtectedPessinaV768(player, entry) {
    const identity = protectedPessinaIdentityForPlayerV768(player);
    if (!identity) return true;
    if (!entry || typeof entry !== 'object') return false;
    const entryTeam = teamKey(entry.realTeam || entry.realTeamOriginal || entry.teamName || '');
    const entryRole = roleKeyForRoster(entry.classicRole || entry.role || entry.rosterRole || '');
    const expectedTeam = teamKey(identity.teamId || identity.teamName || '');
    return entryTeam === expectedTeam && entryRole === identity.role;
  }

  function acceptedListoneCandidateV768(player, entry) {
    return entry && listoneMatchesProtectedPessinaV768(player, entry) ? entry : null;
  }

  function protectedRoleForPlayerV768(player) {
    const identity = protectedPessinaIdentityForPlayerV768(player);
    return identity ? identity.role : '';
  }

  function enforceProtectedPessinaIdentitiesV768(root) {
    if (!root || typeof root !== 'object') return { inspected: 0, corrected: 0 };
    const stack = [root];
    const seen = new Set();
    let inspected = 0;
    let corrected = 0;
    while (stack.length) {
      ...
      seen.add(item);
      inspected += 1;
      const identity = normalizedPessinaIdentityV768(item);
      if (identity) {
        const before = JSON.stringify([item.id, item.playerName, item.target, item.name, item.teamId, item.teamName, item.role, item.classicRole]);
        if (item.id && PROTECTED_PESSINA_V768[item.id]) item.id = identity.id;
        if (item.playerName != null) item.playerName = identity.playerName;
        if (item.target != null) item.target = identity.playerName;
        if (item.name != null && norm(item.name).indexOf('pessina') !== -1) item.name = identity.playerName;
        if (item.originalName != null && norm(item.originalName).indexOf('pessina') !== -1) item.originalName = identity.playerName;
        ...
        if (identity.id === 'monza-pessina-mas') {
          if (item.formationPosition === 'P' || item.formationPosition === 'POR') item.formationPosition = 'CC';
          if (item.formationSourcePosition === 'P' || item.formationSourcePosition === 'POR') item.formationSourcePosition = 'CC';
          if (item.formationLine === 'P' || item.formationLine === 'POR') item.formationLine = 'CC';
          if (item.position === 'P' || item.position === 'POR') item.position = 'CC';
        ...
        if (Array.isArray(item.nameAliasesV755)) {
          item.nameAliasesV755 = Array.from(new Set(item.nameAliasesV755.filter(function (alias) {
            return norm(alias) !== 'pessina';
          }).concat([identity.playerName])));
        }
        item.identityOverrideV768 = identity.teamName + ': identita protetta per nome completo, squadra e ruolo';
        const after = JSON.stringify([item.id, item.playerName, item.target, item.name, item.teamId, item.teamName, item.role, item.classicRole]);
    ...
    return { inspected: inspected, corrected: corrected };
  }


  const PROTECTED_BORDON_V770 = Object.freeze({
    'lazio-filipe-bordon': Object.freeze({ id: 'lazio-filipe-bordon', teamId: 'lazio', teamName: 'Lazio', playerName: 'Filipe Bordon', role: 'D' }),
    'lazio-ricardo-bordon': Object.freeze({ id: 'lazio-ricardo-bordon', teamId: 'lazio-u20', teamName: 'Lazio U20', playerName: 'Ricardo Bordon', role: 'D' })
  });

  function enforceProtectedBordonIdentitiesV770(root) {
    if (!root || typeof root !== 'object') return { inspected: 0, corrected: 0 };
    const stack = [root]; const seen = new Set(); let inspected = 0; let corrected = 0;
    while (stack.length) {
      const item = stack.pop();
      if (!item || typeof item !== 'object' || seen.has(item)) continue;
      seen.add(item); inspected += 1;
      const name = norm(item.playerName || item.target || item.name || item.originalName || item.disambiguatedName || '');
      const team = norm(item.teamName || item.realTeam || item.realTeamOriginal || '');
      let identity = null;
      if (name === 'ricardo bordon' || (name === 'bordon' && team.indexOf('u20') !== -1)) identity = PROTECTED_BORDON_V770['lazio-ricardo-bordon'];
      else if (name === 'filipe bordon' || name === 'f bordon' || (name === 'bordon' && team.indexOf('lazio') !== -1)) identity = PROTECTED_BORDON_V770['lazio-filipe-bordon'];
      if (identity) {
        const before = JSON.stringify([item.id,item.canonicalPlayerId,item.playerName,item.target,item.teamId,item.teamName,item.role]);
        if (item.id && /bordon/.test(String(item.id))) item.id = identity.id;
        if (item.canonicalPlayerId && /bordon/.test(String(item.canonicalPlayerId))) item.canonicalPlayerId = identity.id;
        if (Array.isArray(item.canonicalPlayerIds) && item.canonicalPlayerIds.some(function(id){ return /bordon/.test(String(id)); })) item.canonicalPlayerIds = [identity.id];
        if (item.playerName != null) item.playerName = identity.playerName;
        if (item.target != null) item.target = identity.playerName;
        if (item.canonicalFullName != null) item.canonicalFullName = identity.playerName;
        if (item.disambiguatedName != null) item.disambiguatedName = identity.playerName;
        if (item.role != null) item.role = identity.role;
        if (item.teamName != null && identity.id === 'lazio-ricardo-bordon') item.teamName = identity.teamName;
        item.identityOverrideV770 = 'Fratelli Bordon protetti: nome completo e ID distinti';
        const after = JSON.stringify([item.id,item.canonicalPlayerId,item.playerName,item.target,item.teamId,item.teamName,item.role]);
        if (before !== after) corrected += 1;
      }
      Object.keys(item).forEach(function(key){ const value=item[key]; if (value && typeof value === 'object') stack.push(value); });
    }
    return { inspected: inspected, corrected: corrected };
  }

  function buildPlayerCatalogIndexesV757() {
    const rosterPlayers = Object.values(state.data && state.data.playersByTeam || {}).reduce(function (acc, rows) {
      return acc.concat(rows || []);
    }, []);
    const rosterById = new Map(rosterPlayers.filter(Boolean).map(function (player) {
      return [String(player.id || ''), player];
    }));
    const directory = Array.isArray(state.data && state.data.playerDirectory) && state.data.playerDirectory.length
      ? state.data.playerDirectory
      : rosterPlayers;
    const allPlayers = [];
    const seen = new Set();
    directory.forEach(function (entry) {
      ...
      seen.add(id);
      const player = rosterById.get(id) || entry;
      if (player !== entry) Object.assign(player, entry);
      allPlayers.push(player);
    });
    state.allPlayers = allPlayers;
    state.data.playerDirectory = allPlayers;
    state.playerByIdIndex = new Map(allPlayers.map(function (player) { return [String(player.id), player]; }));
    state.playerSourceIndex = new Map();
    state.playerSourceKeyIndex = new Map();
    allPlayers.forEach(function (player) {
      state.playerSourceIndex.set(String(player.id), emptyPlayerSourceBucketV757());
    });

    Object.values(state.data.marketSummaryByTeam || {}).forEach(function (summary) {
      (summary.officialIncoming || []).forEach(function (item) { indexItemForCanonicalPlayersV757(item, 'officialIn'); });
      (summary.officialOutgoing || []).forEach(function (item) { indexItemForCanonicalPlayersV757(item, 'officialOut'); });
      (summary.officialRenewals || []).forEach(function (item) { indexItemForCanonicalPlayersV757(item, 'officialIn'); });
      (summary.talksIncoming || []).forEach(function (item) { indexItemForCanonicalPlayersV757(item, 'talks'); });
      (summary.talksOutgoing || []).forEach(function (item) { indexItemForCanonicalPlayersV757(item, 'talks'); });
      (summary.talksOther || []).forEach(function (item) { indexItemForCanonicalPlayersV757(item, 'talks'); });
    });
    Object.values(state.data.injuriesByTeam || {}).forEach(function (items) {
      (items || []).forEach(function (item) { indexItemForCanonicalPlayersV757(item, 'injuries'); });
    });
    Object.values(state.data.formationsByTeam || {}).forEach(function (items) {
      (items || []).forEach(function (item) { indexItemForCanonicalPlayersV757(item, 'formations'); });
    });
    Object.entries(state.data.friendlyPlayerStatsByMatch || {}).forEach(function (entry) {
      const matchKey = entry[0];
      const match = entry[1] || {};
      (match.players || []).forEach(function (row) {
        ...
        indexItemForCanonicalPlayersV757(item, 'friendlies');
      });
    });

    state.catalogStatsV757 = {
      players: allPlayers.length,
      rosterPlayers: rosterPlayers.length,
      indexedPlayers: state.playerSourceIndex.size,
      indexedSourceRows: state.playerSourceKeyIndex.size,
      lookupMode: 'player-id-map-o1',
      detailMode: 'preindexed-sources-no-global-scan'
    };
    window.ioSudoDiagnosticsV776 = Object.freeze(Object.assign({ version: 'V776', sosFlagMode: 'active-injury-index-only', sourceBadgeMode: 'catalog-membership-listone-first', anagraficaVisible: false, identityMode: 'full-name-team-scoped', musahCurrentTeam: 'Milan', giovaneProtected: true, pessinaProtected: true, bordonProtected: true, protectedBordonAuditV770: state.protectedBordonAuditV770 || null, protectedIdentityAuditV768: state.protectedIdentityAuditV768 || null }, state.catalogStatsV757));
  }
  ...
  function isRealFriendlyMatch(item, team) {
    ...
    const compactEvent = norm(event);
    if (compactEvent.indexOf('basilea juventus') !== -1 && compactEvent.indexOf('bologna arminia') !== -1 && compactEvent.indexOf('atalanta') !== -1 && compactEvent.indexOf('u23') !== -1) return false;
  ...
  function friendlyResultForItem(item) {
    const detail = friendlyStatsStore(friendlyDetailKey(item));
    return item && (item.result || item.score || item.finalScore) || detail && (detail.result || detail.score || detail.finalScore) || '';
  }

  function friendlySourceForItem(item) {
    const detail = friendlyStatsStore(friendlyDetailKey(item));
    return sourcesHtml(item || {}) || detail && sourcesHtml(detail) || '';
  }
  ...
  function friendlyStatRow(row) {
    ...
    const playerNameHtml = linkedPlayer
      ...
      : '<strong>' + escapeHtml(upperPlayerName(row.playerName || 'Giocatore')) + '</strong>';
  ...
  function renderFriendlyDetail(matchKey, teamId) {
    ...
    const result = friendlyResultForItem(item);
    ...
    els.focus.innerHTML = '<div class="iosudo-panel-header iosudo-friendly-detail-header"><div>'
      ...
      + '<div class="iosudo-friendly-detail-card iosudo-friendly-detail-card-compact">'
      + '<p><strong>Partita:</strong> ' + escapeHtml(item.event || 'Amichevole') + '</p>'
      + (result ? '<p><strong>Risultato:</strong> <span class="iosudo-result-badge">' + escapeHtml(result) + '</span></p>' : '')
      + '<p><strong>Data:</strong> ' + escapeHtml(when) + '</p>'
      + friendlySourceForItem(item)
      + '</div>'
      ...
      + '<h3 class="iosudo-friendly-tabellino-title">Tabellino giocatori</h3>'
      + statRows;
    els.focus.classList.remove('hidden');
    navigateHashV757('friendly=' + encodeURIComponent(key) + (state.activeTeamId ? '&team=' + encodeURIComponent(state.activeTeamId) : ''));
  }

  function playerById(playerId, teamId) {
    const id = String(playerId || '');
    if (!id) return null;
    if (state.playerByIdIndex && state.playerByIdIndex.has(id)) return state.playerByIdIndex.get(id);
  ...
  function itemMatchesPlayer(item, player) {
    ...
    if (item.canonicalPlayerId && player.id && String(item.canonicalPlayerId) === String(player.id)) return true;
    const candidate = item && (item.playerName || item.target || item.name || item.title);
    if (!candidate) return false;
    return playerNameCandidates(player).some(function (entry) { return sameName(candidate, entry.value); });
  }

  function friendlyStatsForPlayer(player) {
    const bucket = sourceBucketForPlayerV757(player);
    const rows = bucket ? bucket.friendlies.slice() : [];
  ...
  function talksForPlayer(player) {
    const bucket = sourceBucketForPlayerV757(player);
    return uniqueMarketItems(bucket ? bucket.talks : [], 'talks');
  }

  function officialIncomingForPlayer(player) {
    const bucket = sourceBucketForPlayerV757(player);
    return uniqueMarketItems(bucket ? bucket.officialIn : [], 'officialIncoming');
  }

  function officialOutgoingForPlayer(player) {
    const bucket = sourceBucketForPlayerV757(player);
    return uniqueMarketItems(bucket ? bucket.officialOut : [], 'officialOutgoing');
  }

  function itemIsRenewal(item) {
    if (!item) return false;
    if (item.isRenewal === false) return false;
    if (item.isRenewal === true) return true;
    const text = norm([
    ...
    if (text.indexOf('primo contratto professionistico') !== -1) return false;
    return /(rinnovo|rinnova|prolungamento|prolunga|nuovo contratto)/.test(text);
  }

  function officialRenewalsForPlayer(player) {
    return uniqueMarketItems(
      officialIncomingForPlayer(player).concat(officialOutgoingForPlayer(player))
        .filter(itemIsRenewal),
      'officialRenewal'
    );
  }

  function officialBadgeForItems(items) {
    const renewal = (items || []).some(itemIsRenewal);
    return renewal
      ? { text: 'RINNOVO', cls: 'iosudo-badge-renewal' }
      : { text: 'UFFICIALE', cls: 'iosudo-badge-new' };
  }

  function injuriesForPlayer(player) {
    const bucket = sourceBucketForPlayerV757(player);
    return uniqueMarketItems(bucket ? bucket.injuries : [], 'injury');
  }
  ...
  function isPhysicalIssue(value) {
    ...
    if (text.indexOf('nessun infortunio') !== -1) return false;
    if (text.indexOf('nessuna indisponibilita') !== -1) return false;
    if (text.indexOf('monitoraggio chiuso') !== -1) return false;
  ...
  function validatedSosFlagV769(player) {
    if (!player) return false;
    // V769: il flag esplicito e affidabile solo quando e stato ricostruito
    // dall'indice delle segnalazioni SOS attive. Un campo di stato non vuoto
    // non deve mai trasformarsi automaticamente in un badge SOS.
    if (player.sosFlagActiveV769 === true) return true;
    if (player.sosFlagActiveV769 === false) return false;
    const status = player.physicalStatus || player.injuryStatus || player.formationPhysicalStatus;
    return player.sosFantaFlag === true && isPhysicalIssue(status);
  }

  function playerHasSos(player) {
    if (injuriesForPlayer(player).length > 0) return true;
    return validatedSosFlagV769(player);
  }

  function marketBadgeForPlayer(player) {
    if ((player && player.hasRenewal) || officialRenewalsForPlayer(player).length) {
      return { text: 'RINNOVO', cls: 'iosudo-badge-renewal' };
    }
    if (player.newAcquisition || officialIncomingForPlayer(player).filter(function (item) { return !itemIsRenewal(item); }).length) {
  ...
  function upperPlayerName(value) {
    return String(value || 'Giocatore').toLocaleUpperCase('it-IT');
  }

  function displayPlayerName(player, fallbackName) {
    return String(player && (player.canonicalFullName || player.playerName || player.disambiguatedName) || fallbackName || 'Giocatore').trim();
  }

  function normalizeNameSource(value) {
    const text = norm(value || '');
    if (text.indexOf('listone') !== -1) return 'LISTONE';
    if (text.indexOf('ufficial') !== -1) return 'UFFICIALITÀ';
    if (text.indexOf('tratt') !== -1 || text.indexOf('rumor') !== -1 || text.indexOf('mercato') !== -1) return 'TRATTATIVA';
    if (text.indexOf('sos') !== -1 || text.indexOf('infortun') !== -1) return 'SOS';
    if (text.indexOf('amichevol') !== -1 || text.indexOf('tabellino') !== -1) return 'AMICHEVOLE';
    if (text.indexOf('formazione') !== -1 || text.indexOf('xi') !== -1) return 'FORMAZIONE';
    // V760: ANAGRAFICA/ALIAS descrive solo come e stato ricostruito il nome,
    // non la sorgente con cui il giocatore entra nel catalogo pubblico.
    if (text.indexOf('anagraf') !== -1 || text.indexOf('alias') !== -1) return 'ROSA';
    return 'ROSA';
  }

  function playerSourceKinds(player) {
    if (!player) return [];
    const values = [];
    [player.sourceKindsV757, player.sourceKindsV756, player.sourceKinds].forEach(function (items) {
      (Array.isArray(items) ? items : []).forEach(function (item) {
        const kind = normalizeNameSource(item);
        if (kind && values.indexOf(kind) === -1) values.push(kind);
      });
    });
    return values;
  }

  function playerHasListoneSource(player) {
    if (!player) return false;
    if (player.listoneOnlyPlayer || player.listone || player.fantacalcioId) return true;
    if (normalizeNameSource(player.roleSource) === 'LISTONE') return true;
    return playerSourceKinds(player).indexOf('LISTONE') !== -1;
  }

  function playerNameSourceKind(player) {
    if (!player) return 'ROSA';
    // V760: la presenza nel listone ha sempre priorita sul metodo interno
    // usato per espandere il nome abbreviato.
    if (playerHasListoneSource(player)) return 'LISTONE';

    const kinds = playerSourceKinds(player);
    if (kinds.indexOf('ROSA') !== -1) return 'ROSA';
    if (player.virtualMarketPlayer) return marketRowsAreOfficial(player.virtualMarketRows) ? 'UFFICIALITÀ' : 'TRATTATIVA';
    if (kinds.indexOf('TRATTATIVA') !== -1) return 'TRATTATIVA';
    if (kinds.indexOf('UFFICIALITÀ') !== -1) return 'UFFICIALITÀ';
    if (kinds.indexOf('SOS') !== -1) return 'SOS';
    if (kinds.indexOf('FORMAZIONE') !== -1) return 'FORMAZIONE';
    if (kinds.indexOf('AMICHEVOLE') !== -1) return 'AMICHEVOLE';

    const fallback = normalizeNameSource(player.nameSource || player.rosterSource || 'ROSA');
    return fallback === 'LISTONE' ? 'LISTONE' : fallback;
  }

  function sourceBadgeHtml(kind) {
    const source = normalizeNameSource(kind);
    const cls = source.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-z0-9]+/g, '-');
    return '<span class="iosudo-badge iosudo-badge-source iosudo-badge-source-' + escapeHtml(cls) + '">SORGENTE: ' + escapeHtml(source) + '</span>';
  }

  function roleBadgeHtml(role) {
    const key = roleKeyForRoster(role) || '?';
    return '<span class="iosudo-badge iosudo-badge-role iosudo-badge-role-' + escapeHtml(key.toLowerCase()) + '">' + escapeHtml(key) + '</span>';
  }

  function playerIdentityHtml(player, fallbackName) {
    const role = displayRoleForPlayer(player);
    return roleBadgeHtml(role) + '<span class="iosudo-player-name-text">' + escapeHtml(upperPlayerName(displayPlayerName(player, fallbackName))) + '</span>' + sourceBadgeHtml(playerNameSourceKind(player));
  }

  function knownPlayerForItem(item) {
    if (!item) return null;
    if (item.canonicalPlayerId) {
      const byId = playerById(item.canonicalPlayerId, item.teamId || '');
      if (byId) return byId;
    }
    const name = item.playerName || item.target || item.name || item.title;
    if (!name) return null;
    const teamId = item.teamId || '';
    const local = teamId ? teamPlayers(teamId).filter(function (player) { return itemMatchesPlayer(item, player); }) : [];
    if (local.length === 1) return local[0];
    const global = state.allPlayers.filter(function (player) { return itemMatchesPlayer(item, player); });
    return global.length === 1 ? global[0] : null;
  }

  function itemIdentityHtml(item, sourceKind) {
    const player = knownPlayerForItem(item);
    const role = player ? displayRoleForPlayer(player) : item && (item.role || item.roleBrief);
    const name = player ? displayPlayerName(player) : item && (item.playerName || item.target) || 'Giocatore';
    return roleBadgeHtml(role) + '<span class="iosudo-player-name-text">' + escapeHtml(upperPlayerName(name)) + '</span>' + sourceBadgeHtml(sourceKind);
  }
  ...
  function liveSummaryCount(key) {
    ...
    if (key === 'players') {
      // V757: allinea il KPI alla vista GIOCATORI effettivamente renderizzata.
      // Il manifest può contare anche profili tecnici/righe che la vista compatta deduplica.
      try {
        const rows = playerRowsForView();
        if (Array.isArray(rows)) return rows.length;
      } catch (error) {}
      return state.allPlayers.length || summaryCount('players') || 0;
    }
  ...
  function groupedMarketRows(kind) {
    ...
    (state.data.teams || []).forEach(function (team) {
      ...
      const keys = kind === 'official'
        ...
        : [['talksIncoming', 'Entrata'], ['talksOutgoing', 'Uscita'], ['talksOther', 'Permanenza/Rinnovo']];
  ...
  function displayRoleForPlayer(player) {
    if (!player) return '-';
    const protectedRole = protectedRoleForPlayerV768(player);
    if (protectedRole) return protectedRole;
    const listone = latestListoneFor(player) || player.listone;
  ...
  function selectUniqueMarketIdentity(candidates, role) {
    if (candidates.length === 1) return candidates[0];
    const roleFiltered = candidates.filter(function (player) { return roleMatchesMarket(player, role); });
    return roleFiltered.length === 1 ? roleFiltered[0] : null;
  }

  function marketRowResolutionKey(row) {
    const item = row && row.item || row || {};
    const teamId = row && row.team && row.team.id || item.teamId || '';
    return marketItemKey(item, row && row.key || 'market') + '::' + String(teamId);
  }

  function findPlayerForMarketRow(row) {
    const cacheKey = marketRowResolutionKey(row);
    if (cacheKey && state.marketRowResolvedPlayerCache.has(cacheKey)) {
      const cachedId = state.marketRowResolvedPlayerCache.get(cacheKey);
      return cachedId ? playerById(cachedId) : null;
    }
    const name = marketPlayerName(row);
    if (!name) {
      if (cacheKey) state.marketRowResolvedPlayerCache.set(cacheKey, '');
      return null;
    }
    const teamId = row && row.team && row.team.id || row && row.item && row.item.teamId || '';
    const role = roleKeyForRoster(marketPlayerRole(row));
    const localMatches = (teamId ? teamPlayers(teamId) : []).filter(function (player) {
      return marketNameMatchesPlayer(row, player);
    });
    let resolved = selectUniqueMarketIdentity(localMatches, role);
    ...
    if (!resolved) {
      ...
      if (origin) {
        ...
        const byOrigin = state.allPlayers.filter(function (player) {
          return marketNameMatchesPlayer(row, player) && playerTeamKeys(player).indexOf(originKey) !== -1;
        });
        resolved = selectUniqueMarketIdentity(byOrigin, role);
      }
    }

    if (!resolved) {
      const globalMatches = state.allPlayers.filter(function (player) { return marketNameMatchesPlayer(row, player); });
      resolved = selectUniqueMarketIdentity(globalMatches, role);
    }
    if (cacheKey) state.marketRowResolvedPlayerCache.set(cacheKey, resolved && resolved.id || '');
  ...
  function marketRowMatchesKnownPlayer(row, player) {
    if (!row || !player) return false;
    const resolved = findPlayerForMarketRow(row);
    return Boolean(resolved && resolved.id && player.id && String(resolved.id) === String(player.id));
  }
  ...
  function sameListonePerson(listone, player) {
    ...
    if (!listoneMatchesProtectedPessinaV768(player, listone)) return false;
  ...
  function applyListoneToPlayer(player, item) {
    ...
    if (!listoneMatchesProtectedPessinaV768(player, item)) return;
    player.listone = item;
    player.fantacalcioId = String(item.fantacalcioId || player.fantacalcioId || '');
    player.role = roleKeyForRoster(item.classicRole || item.role || item.rosterRole || '') || player.role;
    player.roleSource = 'LISTONE';
  ...
  function invalidatePlayerRowsCache() {
    ...
    state.marketRowResolvedPlayerCache = new Map();
  }
  ...
  function fastPlayerStatusText(player) {
    ...
    if (player && player.hasRenewal) return 'RINNOVO';
  ...
  function fastPlayerBadge(player) {
    ...
    if (text === 'RINNOVO') return { text: text, cls: 'iosudo-badge-renewal' };
  ...
  function collectFastPlayerRows() {
    ...
    state.virtualPlayers = new Map();
    const rows = (state.allPlayers || []).filter(Boolean).map(function (player) {
      applyLiveRosterMatchToPlayer(player);
      return prepareFastPlayerRow(player);
    });
    rows.sort(function (a, b) {
      const freshDiff = (b.updatedTime || 0) - (a.updatedTime || 0);
  ...
  function globalPlayerItem(row) {
    ...
    const html = '<article class="iosudo-list-row iosudo-compact-row iosudo-player-global-row ' + escapeHtml(roleClass(displayRole)) + '">'
      ...
      + '<h4 class="iosudo-player-identity">' + playerIdentityHtml(player) + badgeHtml(badge) + sos + '</h4>'
      + '<p><strong>' + escapeHtml(displayRole) + '</strong> · Squadra reale: ' + escapeHtml(currentTeam) + ' · Fantasy: ' + escapeHtml(fantasy) + '</p>'
      + '<p>Ultimo aggiornamento: ' + escapeHtml(updated) + '</p>'
      + '</button></article>';
  ...
  function globalOfficialItem(row) {
    ...
    const officialBadge = officialBadgeForItems(items);
    return '<article class="iosudo-list-row iosudo-compact-row iosudo-market-row-compact iosudo-official-group-card"><h4>'
      + itemIdentityHtml(Object.assign({}, items[0], { playerName: row.playerName || marketPrimaryName(items[0]) }), 'UFFICIALITÀ')
      + ' <span class="iosudo-badge ' + escapeHtml(officialBadge.cls) + '">' + escapeHtml(officialBadge.text) + '</span></h4>'
  ...
  function globalRumorItem(row) {
    ...
    return '<article class="iosudo-list-row iosudo-compact-row iosudo-market-row-compact iosudo-rumor-group-card"><h4>'
      + itemIdentityHtml(Object.assign({}, items[0], { playerName: row.playerName || marketPrimaryName(items[0]) }), 'TRATTATIVA')
  ...
  function globalMarketItem(row, badgeText) {
    ...
    const singleOfficialBadge = badgeText === 'UFFICIALE' ? officialBadgeForItems([item]) : null;
    const effectiveBadgeText = singleOfficialBadge ? singleOfficialBadge.text : badgeText;
    const badgeCls = singleOfficialBadge ? singleOfficialBadge.cls : 'iosudo-badge-rumor';
    const html = '<article class="iosudo-list-row iosudo-compact-row iosudo-market-row-compact"><h4>'
      + itemIdentityHtml(Object.assign({}, item, { playerName: name }), badgeText === 'UFFICIALE' ? 'UFFICIALITÀ' : 'TRATTATIVA') + ' <span class="iosudo-badge ' + badgeCls + '">' + escapeHtml(effectiveBadgeText) + '</span></h4>'
  ...
  function teamOfficialCard(row, direction) {
    ...
    const officialBadge = officialBadgeForItems(row.items || [item]);
    return '<article class="iosudo-list-row iosudo-team-market-card iosudo-team-market-card-compact"><h4>'
      + '<span class="iosudo-player-identity">' + itemIdentityHtml(Object.assign({}, item, { playerName: row.playerName || marketPrimaryName(item) }), 'UFFICIALITÀ') + '</span>'
      + '<span class="iosudo-badge ' + escapeHtml(officialBadge.cls) + '">' + escapeHtml(officialBadge.text) + '</span></h4>'
  ...
  function teamTalkCard(row, direction) {
    ...
    return '<article class="iosudo-list-row iosudo-team-market-card iosudo-team-market-card-compact"><h4>'
      + '<span class="iosudo-player-identity">' + itemIdentityHtml(Object.assign({}, item, { playerName: row.playerName || marketPrimaryName(item) }), 'TRATTATIVA') + '</span>'
      + '<span class="iosudo-badge iosudo-badge-rumor">RUMOR</span></h4>'
  ...
  function renderTeamMarketPanel(team, summary) {
    ...
    const talksOther = groupItemsByPlayer(summary.talksOther || [], 'talksOther', team);
    return '<div class="iosudo-tab-panel iosudo-team-market-panel">'
      ...
      + renderMarketSubsection('PERMANENZE E RINNOVI', talksOther, function (row) { return teamTalkCard(row, 'status'); }, 'Nessuna voce di permanenza o rinnovo.', false)
      + '</div>';
  }
  ...
  function globalSosItem(row) {
    ...
    return '<article class="iosudo-list-row iosudo-compact-row"><h4>'
      + itemIdentityHtml(item, 'SOS') + '<span class="iosudo-badge iosudo-badge-sos">SOS</span></h4>'
  ...
  function globalFriendlyItem(row) {
    ...
    const when = [formatDate(item.date) || 'Data da confermare', item.time].filter(Boolean).join(' · ');
    const result = friendlyResultForItem(item);
    return '<article class="iosudo-list-row iosudo-compact-row iosudo-friendly-card iosudo-friendly-card-compact">'
      + '<button class="iosudo-friendly-row-button" type="button"' + friendlyDetailActionAttrs(item) + ' aria-label="Apri tabellino amichevole ' + escapeHtml(item.event || '') + '">'
  ...
  function playerCard(player) {
    ...
    return '<article class="iosudo-player-card ' + escapeHtml(roleClass(displayRole)) + '">'
      ...
      + '<h3 class="iosudo-player-identity">' + playerIdentityHtml(player) + '</h3>'
  ...
  function renderPitch(teamId) {
    ...
    const rows = order.map(function (line) {
      ...
      return '<div class="iosudo-pitch-row">' + items.map(function (item) {
        ...
        const type = playerId ? ' type="button" aria-label="Apri dettaglio di ' + escapeHtml(upperPlayerName(item.playerName || '')) + '"' : '';
        return '<' + tag + ' class="iosudo-pitch-player ' + escapeHtml(roleCss) + '"' + type + attrs + '>'
          ...
          + '<span class="iosudo-pitch-name">' + escapeHtml(upperPlayerName(item.playerName || '')) + '</span>'
          + fantasyHtml
          + risk
          + '</' + tag + '>';
      }).join('') + '</div>';
  ...
  function marketItem(item) {
    ...
    return '<article class="iosudo-list-row"><h4 class="iosudo-player-identity">' + itemIdentityHtml(item, /ufficial/i.test(String(item.status || '')) ? 'UFFICIALITÀ' : 'TRATTATIVA') + '</h4>'
      + (detail ? '<p>' + escapeHtml(detail) + '</p>' : '')
      + notes
      + sourcesHtml(item)
      + '</article>';
  ...
  function playerContextMarketItem(item) {
    const status = [item.status || item.directionLabel, item.origin, item.formula].filter(Boolean).join(' - ');
    const notes = item.note ? '<p>' + escapeHtml(item.note) + '</p>' : '';
    return '<article class="iosudo-list-row iosudo-player-context-market-row">'
      + (status ? '<p><strong>Stato:</strong> ' + escapeHtml(status) + '</p>' : '')
      + notes
      + sourcesHtml(item)
      + '</article>';
  }
  ...
  function injuryItem(item) {
    return '<article class="iosudo-list-row"><h4 class="iosudo-player-identity">' + itemIdentityHtml(item, 'SOS') + '<span class="iosudo-badge iosudo-badge-sos">SOS</span></h4>'
  ...
  function friendlyItem(item) {
    const result = friendlyResultForItem(item);
    const when = [formatDate(item.date) || 'Data da confermare', item.time].filter(Boolean).join(' · ');
    return '<article class="iosudo-list-row iosudo-friendly-card iosudo-friendly-card-compact">'
      + '<button class="iosudo-friendly-row-button" type="button"' + friendlyDetailActionAttrs(item) + ' aria-label="Apri tabellino amichevole ' + escapeHtml(item.event || '') + '">'
      + '<h4>' + escapeHtml(item.event || 'Amichevole') + (result ? ' <span class="iosudo-result-badge">' + escapeHtml(result) + '</span>' : '') + '</h4>'
      + '<p>' + escapeHtml(when) + '</p>'
      + '</button>'
  ...
  function playerLatestNewsTime(player) {
    if (!player) return 0;
    const times = [itemUpdatedTime(player)];
    try { times.push(marketLatestTime(talksForPlayer(player))); } catch (err) {}
    try { times.push(marketLatestTime(officialIncomingForPlayer(player))); } catch (err) {}
    try { times.push(marketLatestTime(officialOutgoingForPlayer(player))); } catch (err) {}
    try { times.push(Math.max.apply(Math, injuriesForPlayer(player).map(function (item) { return itemUpdatedTime(item); }).concat([0]))); } catch (err) {}
    return Math.max.apply(Math, times.concat([0]));
  }

  function teamPlayersList(teamId) {
    return teamPlayers(teamId).slice().sort(function (a, b) {
      // V757: nelle rose il ruolo e il criterio principale: P, D, C, A.
      const ar = roleOrder(a.role);
      const br = roleOrder(b.role);
      if (ar !== br) return ar - br;
      // Dentro lo stesso ruolo restano prima i giocatori con aggiornamenti recenti.
      const bt = playerLatestNewsTime(b);
      const at = playerLatestNewsTime(a);
      if (bt || at) {
        const diff = bt - at;
        if (diff) return diff;
      }
  ...
  function formationDetailItem(item) {
    return '<article class="iosudo-list-row"><h4 class="iosudo-player-identity">' + itemIdentityHtml(item, 'FORMAZIONE') + '</h4>'
  ...
  function detailBadgeForPlayer(player, detail) {
    const officialItems = (detail.officialIn || []).concat(detail.officialOut || []);
    if ((player && player.hasRenewal) || officialItems.some(itemIsRenewal)) {
      return { text: 'RINNOVO', cls: 'iosudo-badge-renewal' };
    }
    if (player.newAcquisition || (detail.officialIn && detail.officialIn.some(function (item) { return !itemIsRenewal(item); }))) {
      return { text: 'NUOVO', cls: 'iosudo-badge-new' };
    }
  ...
  function detailHasSos(player, detail) {
    if (detail && detail.injuries && detail.injuries.length) return true;
    return validatedSosFlagV769(player);
  }
  ...
  function playerDetailModel(player) {
    ...
    const bucket = sourceBucketForPlayerV757(player) || emptyPlayerSourceBucketV757();
    const model = {
      officialIn: uniqueMarketItems(bucket.officialIn, 'officialIncoming'),
      officialOut: uniqueMarketItems(bucket.officialOut, 'officialOutgoing'),
      talks: uniqueMarketItems(bucket.talks, 'talks'),
      injuries: uniqueMarketItems(bucket.injuries, 'injury'),
      formationItems: bucket.formations.slice(),
      friendlies: bucket.friendlies.slice().sort(function (a, b) { return itemUpdatedTime(b) - itemUpdatedTime(a); })
    };
  ...
  function renderPlayerDetailUnsafeV757(playerId, teamId) {
    ...
    els.focus.innerHTML = '<div class="iosudo-panel-header"><div>'
      ...
      + '<h2 class="iosudo-card-title iosudo-player-identity">' + playerIdentityHtml(player) + '</h2>'
      ...
      + detailSection('Trattative e rumors', talks, playerContextMarketItem)
    ...
    if (els.app) els.app.classList.add('is-team-open');
    bindPlayerDetail();
    navigateHashV757('team=' + encodeURIComponent(state.activeTeamId) + '&player=' + encodeURIComponent(player.id));
  }

  function renderPlayerDetail(playerId, teamId) {
    const renderKey = String(playerId || '') + '::' + String(teamId || '');
    if (!playerId || state.detailRenderingId === renderKey) return;
    state.detailRenderingId = renderKey;
    try {
      return renderPlayerDetailUnsafeV757(playerId, teamId);
    } catch (error) {
      console.error('[ioSudo V781] errore dettaglio giocatore', error);
      if (els.focus) {
        els.focus.innerHTML = '<p class="iosudo-empty">Impossibile aprire questa scheda. Il resto dell’app resta disponibile.</p>';
        els.focus.classList.remove('hidden');
      }
      setStatus('Errore isolato nella scheda giocatore.');
    } finally {
      state.detailRenderingId = '';
    }
  }
  ...
  function renderTeamPanel(teamId) {
    ...
    els.focus.classList.remove('hidden');
    bindTeamPanel();
    navigateHashV757('team=' + encodeURIComponent(teamId) + '&tab=' + encodeURIComponent(tab));
  }
  ...
  function navigateHashV757(value, replace) {
    const hash = '#' + String(value || '').replace(/^#/, '');
    if (window.location.hash === hash) return;
    const url = window.location.pathname + window.location.search + hash;
    if (replace) history.replaceState({ iosudo: true }, '', url);
    else history.pushState({ iosudo: true }, '', url);
  }
  ...
  async function loadData() {
    ...
    const protectedIdentityAuditV768 = enforceProtectedPessinaIdentitiesV768(state.data);
    state.protectedIdentityAuditV768 = protectedIdentityAuditV768;
    state.protectedBordonAuditV770 = enforceProtectedBordonIdentitiesV770(state.data);
    buildPlayerCatalogIndexesV757();
  ...
  function updateInstallButton() {
    ...
    els.installBtn.textContent = deferredInstallPrompt ? 'Installa' : 'Installa app';
    els.installBtn.title = deferredInstallPrompt ? 'Installa ioSudo' : 'Installa ioSudo dalla schermata Home del browser';
  }

  function manualInstallMessage() {
    const isIos = /iphone|ipad|ipod/i.test(window.navigator.userAgent || '');
    if (isIos) return 'Per installare ioSudo: apri il menu Condividi di Safari e scegli “Aggiungi a schermata Home”.';
    return 'Per installare ioSudo: apri il menu del browser e scegli “Installa app” oppure “Aggiungi a schermata Home”.';
  }
...
/* V742: Excel V107; Liberali/Saelemaekers confermati, Saelemaekers disambiguato da Chukwueze, Milan-Milan Futuro 7-0, nuovi rumor mercato, timestamp header. */

/* V742: Excel V107; Liberali/Saelemaekers confermati, Saelemaekers disambiguato da Chukwueze, Milan-Milan Futuro 7-0, nuovi rumor mercato, timestamp header. */

/* V742: Excel V107; Liberali/Saelemaekers confermati, Saelemaekers disambiguato da Chukwueze, Milan-Milan Futuro 7-0, nuovi rumor mercato, timestamp header. */

/* V742: Excel V109; disambiguazioni V737 protette, nuovi tabellini 19/07 (Genoa, Lecce, Venezia, Fiorentina), De Gea rientrato, Baldanzi SOS prudenziale. */

/* V742: duplicati confermati fusi; amichevoli compatte con risultato/data/link; click apre tabellino; matchKey/risultati tabellini riparati. */

/* V746: Excel V118 fuso; nessuna nuova fonte, duplicati V742 mantenuti, header timestamp e candidati duplicati aggiornati. */

/* V746: Excel V120 integrato; dedupliche Cittadini/Maldini/Dominguez/Pellegrini/Fofana; disambiguazioni Miranda-Moro-Kone-Vasquez-Perez; Lazio-U20 ore 18:00. */

/* V757: catalogo Excel v146, Adams protetti e indici dettaglio O(1). */



/* V760: badge sorgente basato sulla presenza nel catalogo; LISTONE prioritario, ANAGRAFICA solo interna. */
/* V768: ruolo Pessina protetto prima di qualsiasi fallback da listone. */

/* V768: fix strutturale matching listone e header/versione sincronizzati. */

/* V769: badge SOS derivato esclusivamente dall'indice delle segnalazioni attive; rimossi i falsi positivi da stato fisico generico. */

/* V770: Excel V155; Filipe e Ricardo Bordon protetti con ID distinti; SOS V769 preservato e aggiornato a 37 righe attive. */

/* V774: Excel V158; catalogo ricostruito, duplicati verificati, header sincronizzato. */

/* V774: Excel V158; deduplica globale e header sincronizzato. */

/* V774: probabili XI V159, 20 squadre e 220 slot indicizzati per ID canonico. */

/* V774: Excel V161, ufficialità Zerbin/Hasa, SOS Nkunku, Obaretin/Garofalo e fonti deduplicate. */

/* V776: Excel V163, mercato 25/07, SOS Parma/Yildiz, Markmann e controllo duplicati. */

/* V781: Excel V167; rumor runtime ripristinati con bucket incoming/outgoing/other e rinnovo Vergara, Quintero e mercato V167. */
