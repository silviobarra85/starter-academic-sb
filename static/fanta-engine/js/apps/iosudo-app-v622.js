(function () {
  ...
  function teamKey(value) {
    ...
    return TEAM_ALIAS_V622.get(key) || key;
  }
  ...
  async function loadLeagueRosters() {
    ...
    try {
      ...
      console.warn('[ioSudo V622] Rose live non caricate: uso snapshot Sudatori incorporato.', error);
    }
  }
  ...
  async function loadLatestListone() {
    ...
    try {
      ...
      console.warn('[ioSudo V622] Listone piu recente non caricato.', error);
    }
  }
  ...
  function collectPlayerRows() {
    return state.allPlayers.map(function (player) {
      return { player: player, item: player, updatedTime: playerLastUpdateTime(player) };
    }).sort(function (a, b) {
