(function () {
  ...
  function teamKey(value) {
    ...
    return TEAM_ALIAS_V626.get(key) || key;
  }
  ...
  async function loadLeagueRosters() {
    ...
    try {
      ...
      console.warn('[ioSudo V626] Rose live non caricate: uso snapshot Sudatori incorporato.', error);
    }
  }
  ...
  async function loadLatestListone() {
    ...
    try {
      ...
      console.warn('[ioSudo V626] Listone piu recente non caricato.', error);
    }
  }
  ...
  function currentRealTeamForMarketRow(row) {
    ...
    if (/Outgoing$|talksOutgoing|officialOutgoing/.test(key) || direction.indexOf('uscita') !== -1 || direction.indexOf('outgoing') !== -1) {
      return team.name || item.teamName || origin || '';
    }
  ...
  function attachMarketRowsForPlayer(player) {
    ...
    const rows = [].concat(collectMarketRows('rumor'), collectMarketRows('official')).filter(function (row) {
      return findPlayerForMarketRow(row) === player;
    });
    if (rows.length) player._iosudoMarketRows = rows;
  }
  ...
  function globalPlayerItem(row) {
    ...
    const currentTeam = currentRealTeamText(player);
    return '<article class="iosudo-list-row iosudo-compact-row iosudo-player-global-row ' + escapeHtml(roleClass(player.role)) + '">'
      ...
      + '<p><strong>' + escapeHtml(player.role || '-') + '</strong> · Squadra reale: ' + escapeHtml(currentTeam) + ' · Fantasy: ' + escapeHtml(fantasy) + '</p>'
  ...
  function renderPlayerDetail(playerId, teamId) {
    ...
    const updatedInfo = '<p>Ultimo aggiornamento: ' + escapeHtml(playerLastUpdateText(player)) + '</p>';
    ...
    els.focus.innerHTML = '<div class="iosudo-panel-header"><div>'
      ...
      + '<p class="iosudo-card-subtitle">' + escapeHtml(displayRealTeam) + ' - ' + escapeHtml(player.role || '-') + '</p>'
