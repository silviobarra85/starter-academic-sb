(function () {
  ...
  function setupLeagueChrome() {
    ...
    document.title = 'ioSudo V764 - ' + league.name;
  }
  ...
  const PROTECTED_PESSINA_V764 = Object.freeze({
  ...
  function normalizedPessinaIdentityV764(item) {
    ...
    const direct = ids.map(function (id) { return PROTECTED_PESSINA_V764[id]; }).find(Boolean);
    if (direct) return direct;
    ...
    if (team === 'bol') return PROTECTED_PESSINA_V764['bologna-pessina-mas'];
    if (team === 'mon') return PROTECTED_PESSINA_V764['monza-pessina-mas'];
    return null;
  }

  function enforceProtectedPessinaIdentitiesV764(root) {
    ...
    while (stack.length) {
      ...
      seen.add(item);
      inspected += 1;
      const identity = normalizedPessinaIdentityV764(item);
      if (identity) {
        ...
        if (item.id && PROTECTED_PESSINA_V764[item.id]) item.id = identity.id;
  ...
  function buildPlayerCatalogIndexesV757() {
    ...
    window.ioSudoDiagnosticsV764 = Object.freeze(Object.assign({ version: 'V764', sourceBadgeMode: 'catalog-membership-listone-first', anagraficaVisible: false, identityMode: 'full-name-team-scoped', musahCurrentTeam: 'Milan', giovaneProtected: true, pessinaProtected: true, protectedIdentityAuditV764: state.protectedIdentityAuditV764 || null }, state.catalogStatsV757));
  }
  ...
  function renderPlayerDetail(playerId, teamId) {
    ...
    try {
      ...
      console.error('[ioSudo V764] errore dettaglio giocatore', error);
      if (els.focus) {
  ...
  async function loadData() {
    ...
    const protectedIdentityAuditV764 = enforceProtectedPessinaIdentitiesV764(state.data);
    state.protectedIdentityAuditV764 = protectedIdentityAuditV764;
...
/* V764: identita Pessina protette e versione visibile nell header. */
