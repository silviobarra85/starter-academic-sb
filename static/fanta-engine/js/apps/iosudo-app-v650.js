(function () {
  ...
  const state = {
    ...
    visibleCaps: { players: 60, rumor: 80, official: 80, sos: 80, friendlies: 80 },
    renderTimer: 0
  };
  ...
  async function loadLeagueRosters() {
    ...
    try {
      ...
      console.warn('[ioSudo V650] Rose live non caricate: uso snapshot Sudatori incorporato.', error);
    }
  }
  ...
  async function loadLatestListone() {
    ...
    try {
      ...
      console.warn('[ioSudo V650] Listone piu recente non caricato.', error);
    }
  }
  ...
  function scheduleRenderResults() {
    ...
    state.renderTimer = window.setTimeout(function () {
      state.renderTimer = 0;
      renderResults();
    }, state.quickView === 'players' ? 220 : 90);
  }

  function defaultVisibleCap(view) {
    if (view === 'players') return 60;
    if (view === 'rumor' || view === 'official') return 80;
    return 100;
  }

  function capStep(view) {
    return view === 'players' ? 60 : 100;
  }
  ...
  function bindPlayerDetail() {
    // V650: gestione delegata sul contenitore focus, senza listener ricreati a ogni dettaglio.
  }
  ...
  function bindTeamPanel() {
    // V650: niente listener per-tab/per-card sul pannello. Gestione delegata una sola volta in init().
  }
