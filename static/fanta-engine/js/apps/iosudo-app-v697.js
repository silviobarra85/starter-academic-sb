(function () {
  ...
  function liveSummaryCount(key) {
    ...
    if (key === 'players') return playerRowsForView().length || state.allPlayers.length || summaryCount('players') || 0;
  ...
  function samePersonNameV697(a, b, allowSurnameOnly) {
    const ia = nameInfoV697(a);
    const ib = nameInfoV697(b);
  ...
  function samePersonRoleV697(a, b, allowSurnameOnly) {
    if (!a || !b) return false;
    ...
    return samePersonNameV697(a.playerName || a.originalName || a.disambiguatedName, b.playerName || b.originalName || b.disambiguatedName, allowSurnameOnly);
  }

  function mergeFastPlayerRowV697(target, source) {
    ...
    a._iosudoMarketRows = uniqueMarketItems([].concat(a._iosudoMarketRows || [], b._iosudoMarketRows || [], b.virtualMarketRows || []), 'dedupe-v697-market');
    const aInfo = nameInfoV697(a.playerName || '');
    const bInfo = nameInfoV697(b.playerName || '');
  ...
  function findUniqueFastRowForListoneV697(item, rows) {
    ...
    const strict = (rows || []).filter(function (row) {
      ...
      if (!samePersonRoleV697(p, probe, true)) return false;
    ...
    const loose = (rows || []).filter(function (row) {
      const p = row && row.player;
      return p && !p.listoneOnlyPlayer && !p.virtualMarketPlayer && samePersonRoleV697(p, probe, true);
    });
  ...
  function fastPlayerDedupKeysV697(player) {
  ...
  function dedupeFastPlayerRowsV697(rows) {
    const out = [];
  ...
  function collectFastPlayerRows() {
    ...
    if (state.latestListone && state.latestListone.active) {
      ...
      (state.latestListone.players || []).forEach(function (item) {
        ...
        const matchedRow = findUniqueFastRowForListoneV697(item, rows);
        ...
        const dedupKey = fastListoneDedupKey(probe);
        if (!dedupKey || existingKeys.has(dedupKey)) return;
    ...
    const dedupedRowsV697 = dedupeFastPlayerRowsV697(rows);
    rows.length = 0;
    dedupedRowsV697.forEach(function (row) { rows.push(row); });
  ...
  async function loadData() {
    ...
    setStatus('Dati aggiornati al ' + formatDateTime(updatedWithTime, updated) + ' - versione ' + (state.manifest.version || state.data.meta.version || 'corrente') + (state.liveRoster && state.liveRoster.active ? ' - rose live ' + state.liveRoster.source : '') + (state.latestListone && state.latestListone.active ? ' - listone ' + state.latestListone.source : ''));
    renderSummary();
    renderResults();
    parseHash();
  }
...
/* V697: aggiornamento dati globale v41; RUMOR/UFFICIALITA raggruppate e header con data+ora. */

/* V697: deduplica conservativa GIOCATORI/Rosa da lista duplicati utente; matching listone per persona anche se la squadra del listone e arretrata. */

/* V697: aggiornamento dati globale v44; mantiene deduplica V696 e header data+ora. */
