(function () {
  ...
  function playerNameCandidates(player) {
    ...
    (player && player.nameAliasesV755 || []).forEach(function (value) { names.push({ value: value, teamOnly: true }); });
  ...
  function playerNameSourceKind(player) {
    ...
    if (player.listoneOnlyPlayer || player.listone || player.fantacalcioId || normalizeNameSource(player.nameSource) === 'LISTONE') return 'LISTONE';
    if (player.virtualMarketPlayer) return marketRowsAreOfficial(player.virtualMarketRows) ? 'UFFICIALITÀ' : 'TRATTATIVA';
    return normalizeNameSource(player.nameSource || player.rosterSource || 'ROSA');
  }
  ...
  function liveSummaryCount(key) {
    ...
    if (key === 'players') {
      // V755: allinea il KPI alla vista GIOCATORI effettivamente renderizzata.
  ...
  function collectFastPlayerRows() {
    ...
    rows.sort(function (a, b) {
      // V755: i giocatori con rumor/notizie/SOS/ufficialità più recenti devono stare più in alto.
  ...
  function teamPlayersList(teamId) {
    return teamPlayers(teamId).slice().sort(function (a, b) {
      // V755: nelle rose il ruolo e il criterio principale: P, D, C, A.
...
/* V755: Excel V126 integrato; Palma rinnovo Udinese; dedupe Atalanta-Atalanta U23; Kostić/Kostic disambiguati. */

/* V755: Excel V128 integrato; SOS Napoli aggiornati; trattative non ufficiali V128; dedupe amichevoli confermata. */
