(function () {
  ...
  function latestListoneFor(player) {
    ...
    if (state.latestListone && state.latestListone.active && state.latestListone.idMap && player.fantacalcioId) {
      ...
      if (byId && listoneMatchesProtectedPessinaV767(player, byId)) return byId;
    }
    ...
    for (let c = 0; c < candidates.length; c += 1) {
      ...
      for (let k = 0; k < keys.length; k += 1) {
        ...
        for (let t = 0; t < teams.length; t += 1) {
          ...
          if (acceptedListoneCandidateV767(player, byTeamRole)) return byTeamRole;
          const byTeam = uniqueListoneCandidate(state.latestListone.map.get(nameKey + '::' + team), player);
          if (acceptedListoneCandidateV767(player, byTeam)) return byTeam;
        }
        if (!item.teamOnly) {
          const byGlobalRole = uniqueListoneCandidate(state.latestListone.globalMap.get(nameKey + '::' + role), player);
          if (acceptedListoneCandidateV767(player, byGlobalRole)) return byGlobalRole;
          const globalList = state.latestListone.globalMap.get(nameKey) || [];
          if (globalList.length === 1 && acceptedListoneCandidateV767(player, globalList[0])) return globalList[0];
        }
      }
    }
    return acceptedListoneCandidateV767(player, looseListoneForPlayer(player));
  }
  ...
  function setupLeagueChrome() {
    ...
    document.title = 'ioSudo V767 - ' + league.name;
  }
  ...
  const PROTECTED_PESSINA_V767 = Object.freeze({
  ...
  function normalizedPessinaIdentityV767(item) {
    ...
    const direct = ids.map(function (id) { return PROTECTED_PESSINA_V767[id]; }).find(Boolean);
    if (direct) return direct;
    ...
    if (team === 'bol') return PROTECTED_PESSINA_V767['bologna-pessina-mas'];
    if (team === 'mon') return PROTECTED_PESSINA_V767['monza-pessina-mas'];
    return null;
  }


  function protectedPessinaIdentityForPlayerV767(player) {
    return normalizedPessinaIdentityV767(player);
  }

  function listoneMatchesProtectedPessinaV767(player, entry) {
    const identity = protectedPessinaIdentityForPlayerV767(player);
  ...
  function acceptedListoneCandidateV767(player, entry) {
    return entry && listoneMatchesProtectedPessinaV767(player, entry) ? entry : null;
  }

  function protectedRoleForPlayerV767(player) {
    const identity = protectedPessinaIdentityForPlayerV767(player);
    return identity ? identity.role : '';
  }

  function enforceProtectedPessinaIdentitiesV767(root) {
    ...
    while (stack.length) {
      ...
      seen.add(item);
      inspected += 1;
      const identity = normalizedPessinaIdentityV767(item);
      if (identity) {
        ...
        if (item.id && PROTECTED_PESSINA_V767[item.id]) item.id = identity.id;
        ...
        item.identityOverrideV767 = identity.teamName + ': identita protetta per nome completo, squadra e ruolo';
  ...
  function buildPlayerCatalogIndexesV757() {
    ...
    window.ioSudoDiagnosticsV767 = Object.freeze(Object.assign({ version: 'V767', sourceBadgeMode: 'catalog-membership-listone-first', anagraficaVisible: false, identityMode: 'full-name-team-scoped', musahCurrentTeam: 'Milan', giovaneProtected: true, pessinaProtected: true, protectedIdentityAuditV767: state.protectedIdentityAuditV767 || null }, state.catalogStatsV757));
  }
  ...
  function displayRoleForPlayer(player) {
    if (!player) return '-';
    const protectedRole = protectedRoleForPlayerV767(player);
  ...
  function sameListonePerson(listone, player) {
    ...
    if (!listoneMatchesProtectedPessinaV767(player, listone)) return false;
  ...
  function applyListoneToPlayer(player, item) {
    ...
    if (!listoneMatchesProtectedPessinaV767(player, item)) return;
    player.listone = item;
  ...
  function renderPlayerDetail(playerId, teamId) {
    ...
    try {
      ...
      console.error('[ioSudo V767] errore dettaglio giocatore', error);
      if (els.focus) {
  ...
  async function loadData() {
    ...
    const protectedIdentityAuditV767 = enforceProtectedPessinaIdentitiesV767(state.data);
    state.protectedIdentityAuditV767 = protectedIdentityAuditV767;
...
/* V767: ruolo Pessina protetto prima di qualsiasi fallback da listone. */

/* V767: fix strutturale matching listone e header/versione sincronizzati. */
