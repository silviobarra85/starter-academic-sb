(function () {
  ...
  async function loadLeagueRosters() {
    ...
    try {
      ...
      console.warn('[ioSudo V649] Rose live non caricate: uso snapshot Sudatori incorporato.', error);
    }
  }
  ...
  async function loadLatestListone() {
    ...
    try {
      ...
      console.warn('[ioSudo V649] Listone piu recente non caricato.', error);
    }
  }
  ...
  function teamFriendlies(teamId) {
    ...
    return ((state.data && state.data.friendliesByTeam && state.data.friendliesByTeam[teamId]) || []).filter(function (item) { return isRealFriendlyMatch(item, team); });
  }
  ...
  function applyListoneToPlayer(player, item) {
    ...
    if (listTeam && !officialCurrentTeamForPlayer(player)) {
      player.realTeamOriginal = player.realTeamOriginal || listTeam;
      player.realTeam = player.realTeam || listTeam;
      player.teamName = player.teamName || listTeam;
    }
  }
  ...
  function playerRowsForView() {
    if (state.playerRowsCache && !state.playerRowsCache.dirty) return state.playerRowsCache.rows || [];
    const rows = collectPlayerRows().map(preparePlayerRowForCache);
    state.playerRowsCache = { dirty: false, rows: rows };
    return rows;
  }
  ...
  function scheduleRenderResults() {
    ...
    state.renderTimer = window.setTimeout(function () {
      state.renderTimer = 0;
      renderResults();
    }, state.quickView === 'players' ? 180 : 80);
  }
  ...
  function compactSourceLabel(value) {
    ...
    if (blob.indexOf('gianlucadimarzio') !== -1 || blob.indexOf('di marzio') !== -1) return 'Di Marzio';
  ...
  function globalPlayerItem(row) {
    ...
    const sourceText = playerSourceText(player, row);
    return '<article class="iosudo-list-row iosudo-compact-row iosudo-player-global-row ' + escapeHtml(roleClass(displayRole)) + '">'
  ...
  function renderGlobalView(q) {
    ...
    const visibleRows = state.quickView === 'players' ? rows.slice(0, 220) : rows;
    const capNote = state.quickView === 'players' && totalRows > visibleRows.length ? ' · mostro le prime ' + visibleRows.length + ', usa la ricerca per restringere' : '';
    els.results.innerHTML = '<section class="iosudo-global-view"><div class="iosudo-global-head">'
      ...
      + '</div>'
      + (visibleRows.length ? '<div class="iosudo-list iosudo-global-list">' + visibleRows.map(mapper).join('') + '</div>' : '<p class="iosudo-empty">' + escapeHtml(empty) + '</p>')
      + '</section>';
    bindCards();
  }
  ...
  function renderTeamPanel(teamId) {
    ...
    const teamTabs = new Set(['xi', 'mercato', 'sos', 'amichevoli']);
    ...
    els.focus.innerHTML = '<div class="iosudo-panel-header"><div>'
      ...
      + tabButton('xi', 'XI') + tabButton('mercato', 'Mercato') + tabButton('sos', 'SOS') + tabButton('amichevoli', 'Amichevoli')
      + '</div>' + tabContent;
    els.focus.classList.remove('hidden');
    bindTeamPanel();
  ...
  function bindCards() {
    // V649: niente listener per-card. I click su squadre/giocatori sono gestiti una sola volta in init().
  }
