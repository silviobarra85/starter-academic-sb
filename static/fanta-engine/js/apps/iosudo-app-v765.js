(function () {
  ...
  function setupLeagueChrome() {
    ...
    document.title = 'ioSudo V765 - ' + league.name;
  }
  ...
  const PROTECTED_PESSINA_V765 = Object.freeze({
  ...
  function normalizedPessinaIdentityV765(item) {
    ...
    const direct = ids.map(function (id) { return PROTECTED_PESSINA_V765[id]; }).find(Boolean);
    if (direct) return direct;
    ...
    if (team === 'bol') return PROTECTED_PESSINA_V765['bologna-pessina-mas'];
    if (team === 'mon') return PROTECTED_PESSINA_V765['monza-pessina-mas'];
    return null;
  }

  function enforceProtectedPessinaIdentitiesV765(root) {
    ...
    while (stack.length) {
      ...
      seen.add(item);
      inspected += 1;
      const identity = normalizedPessinaIdentityV765(item);
      if (identity) {
        ...
        if (item.id && PROTECTED_PESSINA_V765[item.id]) item.id = identity.id;
        ...
        item.identityOverrideV765 = identity.teamName + ': identita protetta per nome completo, squadra e ruolo';
  ...
  function buildPlayerCatalogIndexesV757() {
    ...
    window.ioSudoDiagnosticsV765 = Object.freeze(Object.assign({ version: 'V765', sourceBadgeMode: 'catalog-membership-listone-first', anagraficaVisible: false, identityMode: 'full-name-team-scoped', musahCurrentTeam: 'Milan', giovaneProtected: true, pessinaProtected: true, protectedIdentityAuditV765: state.protectedIdentityAuditV765 || null }, state.catalogStatsV757));
  }
  ...
  function renderPlayerDetail(playerId, teamId) {
    ...
    try {
      ...
      console.error('[ioSudo V765] errore dettaglio giocatore', error);
      if (els.focus) {
  ...
  async function loadData() {
    ...
    const protectedIdentityAuditV765 = enforceProtectedPessinaIdentitiesV765(state.data);
    state.protectedIdentityAuditV765 = protectedIdentityAuditV765;
...
/* V765: identita Pessina protette e versione visibile nell header. */

/* V765: payload V150, header/versione sincronizzati e identità protette. */
