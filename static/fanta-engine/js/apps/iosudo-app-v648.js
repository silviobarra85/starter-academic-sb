(function () {
  ...
  async function loadLeagueRosters() {
    ...
    try {
      ...
      console.warn('[ioSudo V646] Rose live non caricate: uso snapshot Sudatori incorporato.', error);
    }
  }
  ...
  async function loadLatestListone() {
    ...
    try {
      ...
      console.warn('[ioSudo V646] Listone piu recente non caricato.', error);
    }
  }
  ...
  function excelSerialDate(value) {
    ...
    if (!text) return null;
    const serialMatch = text.match(/^0*(\d{5})(?:\.0+)?$/) || text.match(/^\+0*(\d{5})(?:-01(?:-01)?)?$/);
  ...
  function prewarmPlayerRowsCache() {
    if (!state.data || !state.latestListone || !state.latestListone.loaded) return;
    const build = function () {
      if (!state.playerRowsCache || state.playerRowsCache.dirty) playerRowsForView();
    };
    if ('requestIdleCallback' in window) window.requestIdleCallback(build, { timeout: 1200 });
    else window.setTimeout(build, 300);
  }
  ...
  function scheduleRenderResults() {
    ...
    state.renderTimer = window.setTimeout(function () {
      state.renderTimer = 0;
      renderResults();
    }, state.quickView === 'players' ? 120 : 60);
  }
