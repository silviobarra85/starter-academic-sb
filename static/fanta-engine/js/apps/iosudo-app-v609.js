(function () {
  ...
  function teamPlayersList(teamId) {
    return teamPlayers(teamId).slice().sort(function (a, b) {
      return String(a.role || '').localeCompare(String(b.role || '')) || String(a.playerName || '').localeCompare(String(b.playerName || ''));
    });
  }
  ...
  function init() {
    ...
    if (backLink && leagueParam === 'fantapetillomantramanager') backLink.href = '../fantapetillomantramanager/';
    if (backLink && leagueParam === 'zonaorientale') backLink.href = '../zonaorientale/';
    bindInstall();
    if (els.search) {
