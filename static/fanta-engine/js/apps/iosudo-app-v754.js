(function () {
  ...
  function playerIdentityHtml(player, fallbackName) {
    ...
    return roleBadgeHtml(role) + '<span class="iosudo-player-name-text">' + escapeHtml(upperPlayerName(player && player.playerName || fallbackName)) + '</span>' + sourceBadgeHtml(playerNameSourceKind(player));
  }

  function itemIdentityHtml(item, sourceKind) {
    return roleBadgeHtml(item && (item.role || item.roleBrief)) + '<span class="iosudo-player-name-text">' + escapeHtml(upperPlayerName(item && (item.playerName || item.target) || 'Giocatore')) + '</span>' + sourceBadgeHtml(sourceKind);
  }
  ...
  function liveSummaryCount(key) {
    ...
    if (key === 'players') {
      // V754: allinea il KPI alla vista GIOCATORI effettivamente renderizzata.
  ...
  function collectFastPlayerRows() {
    ...
    rows.sort(function (a, b) {
      // V754: i giocatori con rumor/notizie/SOS/ufficialità più recenti devono stare più in alto.
  ...
  function teamPlayersList(teamId) {
    return teamPlayers(teamId).slice().sort(function (a, b) {
      // V754: nelle rose il ruolo e il criterio principale: P, D, C, A.
...
/* V754: Excel V126 integrato; Palma rinnovo Udinese; dedupe Atalanta-Atalanta U23; Kostić/Kostic disambiguati. */

/* V754: Excel V128 integrato; SOS Napoli aggiornati; trattative non ufficiali V128; dedupe amichevoli confermata. */
