(function () {
  ...
  function playerNameCandidates(player) {
    ...
    (player && player.nameAliasesV756 || []).forEach(function (value) { names.push({ value: value, teamOnly: true }); });
  ...
  function emptyPlayerSourceBucketV756() {
    return { talks: [], officialIn: [], officialOut: [], injuries: [], formations: [], friendlies: [] };
  }

  function canonicalIdsForItemV756(item) {
    if (!item) return [];
  ...
  function sourceBucketForPlayerV756(player) {
    const id = String(player && player.id || '');
    return id && state.playerSourceIndex ? state.playerSourceIndex.get(id) || null : null;
  }

  function addIndexedSourceV756(playerId, key, item) {
    const id = String(playerId || '');
    if (!id || !state.playerByIdIndex.has(id) || !item) return;
    const bucket = state.playerSourceIndex.get(id) || emptyPlayerSourceBucketV756();
  ...
  function indexItemForCanonicalPlayersV756(item, key) {
    canonicalIdsForItemV756(item).forEach(function (playerId) {
      addIndexedSourceV756(playerId, key, item);
    });
  }

  function buildPlayerCatalogIndexesV756() {
    ...
    allPlayers.forEach(function (player) {
      state.playerSourceIndex.set(String(player.id), emptyPlayerSourceBucketV756());
    });

    Object.values(state.data.marketSummaryByTeam || {}).forEach(function (summary) {
      (summary.officialIncoming || []).forEach(function (item) { indexItemForCanonicalPlayersV756(item, 'officialIn'); });
      (summary.officialOutgoing || []).forEach(function (item) { indexItemForCanonicalPlayersV756(item, 'officialOut'); });
      (summary.officialRenewals || []).forEach(function (item) { indexItemForCanonicalPlayersV756(item, 'officialIn'); });
      (summary.talksIncoming || []).forEach(function (item) { indexItemForCanonicalPlayersV756(item, 'talks'); });
      (summary.talksOutgoing || []).forEach(function (item) { indexItemForCanonicalPlayersV756(item, 'talks'); });
    });
    Object.values(state.data.injuriesByTeam || {}).forEach(function (items) {
      (items || []).forEach(function (item) { indexItemForCanonicalPlayersV756(item, 'injuries'); });
    });
    Object.values(state.data.formationsByTeam || {}).forEach(function (items) {
      (items || []).forEach(function (item) { indexItemForCanonicalPlayersV756(item, 'formations'); });
    });
    Object.entries(state.data.friendlyPlayerStatsByMatch || {}).forEach(function (entry) {
      ...
      (match.players || []).forEach(function (row) {
        ...
        indexItemForCanonicalPlayersV756(item, 'friendlies');
      });
    });

    state.catalogStatsV756 = {
    ...
    window.ioSudoDiagnosticsV756 = Object.freeze(Object.assign({ version: 'V756' }, state.catalogStatsV756));
  }
  ...
  function renderFriendlyDetail(matchKey, teamId) {
    ...
    navigateHashV756('friendly=' + encodeURIComponent(key) + (state.activeTeamId ? '&team=' + encodeURIComponent(state.activeTeamId) : ''));
  }
  ...
  function friendlyStatsForPlayer(player) {
    const bucket = sourceBucketForPlayerV756(player);
  ...
  function normalizeNameSource(value) {
    ...
    if (text.indexOf('anagraf') !== -1 || text.indexOf('alias') !== -1) return 'ANAGRAFICA';
    return 'ROSA';
  }

  function playerNameSourceKind(player) {
    if (!player) return 'ROSA';
    if (player.nameSource) return normalizeNameSource(player.nameSource);
    if (player.virtualMarketPlayer) return marketRowsAreOfficial(player.virtualMarketRows) ? 'UFFICIALITÀ' : 'TRATTATIVA';
    if (player.listoneOnlyPlayer || player.listone || player.fantacalcioId) return 'LISTONE';
    return normalizeNameSource(player.rosterSource || 'ROSA');
  }
  ...
  function liveSummaryCount(key) {
    ...
    if (key === 'players') {
      // V756: allinea il KPI alla vista GIOCATORI effettivamente renderizzata.
  ...
  function teamPlayersList(teamId) {
    return teamPlayers(teamId).slice().sort(function (a, b) {
      // V756: nelle rose il ruolo e il criterio principale: P, D, C, A.
  ...
  function playerDetailModel(player) {
    ...
    const bucket = sourceBucketForPlayerV756(player) || emptyPlayerSourceBucketV756();
    const model = {
  ...
  function renderPlayerDetailUnsafeV756(playerId, teamId) {
    ...
    if (els.app) els.app.classList.add('is-team-open');
    bindPlayerDetail();
    navigateHashV756('team=' + encodeURIComponent(state.activeTeamId) + '&player=' + encodeURIComponent(player.id));
  }
  ...
  function renderPlayerDetail(playerId, teamId) {
    ...
    try {
      return renderPlayerDetailUnsafeV756(playerId, teamId);
    } catch (error) {
      console.error('[ioSudo V756] errore dettaglio giocatore', error);
      if (els.focus) {
  ...
  function renderTeamPanel(teamId) {
    ...
    els.focus.classList.remove('hidden');
    bindTeamPanel();
    navigateHashV756('team=' + encodeURIComponent(teamId) + '&tab=' + encodeURIComponent(tab));
  }
  ...
  function navigateHashV756(value, replace) {
...
/* V756: Excel V126 integrato; Palma rinnovo Udinese; dedupe Atalanta-Atalanta U23; Kostić/Kostic disambiguati. */

/* V756: Excel V128 integrato; SOS Napoli aggiornati; trattative non ufficiali V128; dedupe amichevoli confermata. */
