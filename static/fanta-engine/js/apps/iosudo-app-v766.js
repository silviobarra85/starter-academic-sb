(function () {
  ...
  function latestListoneFor(player) {
    ...
    if (state.latestListone && state.latestListone.active && state.latestListone.idMap && player.fantacalcioId) {
      ...
      if (byId && listoneMatchesProtectedPessinaV766(player, byId)) return byId;
    }
    ...
    for (let c = 0; c < candidates.length; c += 1) {
      ...
      for (let k = 0; k < keys.length; k += 1) {
        ...
        for (let t = 0; t < teams.length; t += 1) {
          ...
          if (acceptedListoneCandidateV766(player, byTeamRole)) return byTeamRole;
          const byTeam = uniqueListoneCandidate(state.latestListone.map.get(nameKey + '::' + team), player);
          if (acceptedListoneCandidateV766(player, byTeam)) return byTeam;
        }
        if (!item.teamOnly) {
          const byGlobalRole = uniqueListoneCandidate(state.latestListone.globalMap.get(nameKey + '::' + role), player);
          if (acceptedListoneCandidateV766(player, byGlobalRole)) return byGlobalRole;
          const globalList = state.latestListone.globalMap.get(nameKey) || [];
          if (globalList.length === 1 && acceptedListoneCandidateV766(player, globalList[0])) return globalList[0];
        }
      }
    }
    return acceptedListoneCandidateV766(player, looseListoneForPlayer(player));
  }
  ...
  function setupLeagueChrome() {
    ...
    document.title = 'ioSudo V766 - ' + league.name;
  }
  ...
  const PROTECTED_PESSINA_V766 = Object.freeze({
  ...
  function normalizedPessinaIdentityV766(item) {
    ...
    const direct = ids.map(function (id) { return PROTECTED_PESSINA_V766[id]; }).find(Boolean);
    if (direct) return direct;
    ...
    if (team === 'bol') return PROTECTED_PESSINA_V766['bologna-pessina-mas'];
    if (team === 'mon') return PROTECTED_PESSINA_V766['monza-pessina-mas'];
    return null;
  }


  function protectedPessinaIdentityForPlayerV766(player) {
    return normalizedPessinaIdentityV766(player);
  }

  function listoneMatchesProtectedPessinaV766(player, entry) {
    const identity = protectedPessinaIdentityForPlayerV766(player);
  ...
  function acceptedListoneCandidateV766(player, entry) {
    return entry && listoneMatchesProtectedPessinaV766(player, entry) ? entry : null;
  }

  function protectedRoleForPlayerV766(player) {
    const identity = protectedPessinaIdentityForPlayerV766(player);
    return identity ? identity.role : '';
  }

  function enforceProtectedPessinaIdentitiesV766(root) {
    ...
    while (stack.length) {
      ...
      seen.add(item);
      inspected += 1;
      const identity = normalizedPessinaIdentityV766(item);
      if (identity) {
        ...
        if (item.id && PROTECTED_PESSINA_V766[item.id]) item.id = identity.id;
        ...
        item.identityOverrideV766 = identity.teamName + ': identita protetta per nome completo, squadra e ruolo';
  ...
  function buildPlayerCatalogIndexesV757() {
    ...
    window.ioSudoDiagnosticsV766 = Object.freeze(Object.assign({ version: 'V766', sourceBadgeMode: 'catalog-membership-listone-first', anagraficaVisible: false, identityMode: 'full-name-team-scoped', musahCurrentTeam: 'Milan', giovaneProtected: true, pessinaProtected: true, protectedIdentityAuditV766: state.protectedIdentityAuditV766 || null }, state.catalogStatsV757));
  }
  ...
  function displayRoleForPlayer(player) {
    if (!player) return '-';
    const protectedRole = protectedRoleForPlayerV766(player);
  ...
  function sameListonePerson(listone, player) {
    ...
    if (!listoneMatchesProtectedPessinaV766(player, listone)) return false;
  ...
  function applyListoneToPlayer(player, item) {
    ...
    if (!listoneMatchesProtectedPessinaV766(player, item)) return;
    player.listone = item;
  ...
  function renderPlayerDetail(playerId, teamId) {
    ...
    try {
      ...
      console.error('[ioSudo V766] errore dettaglio giocatore', error);
      if (els.focus) {
  ...
  async function loadData() {
    ...
    const protectedIdentityAuditV766 = enforceProtectedPessinaIdentitiesV766(state.data);
    state.protectedIdentityAuditV766 = protectedIdentityAuditV766;
...
/* V766: ruolo Pessina protetto prima di qualsiasi fallback da listone. */

/* V766: fix strutturale matching listone e header/versione sincronizzati. */
