(function () {
  ...
  const state = {
    ...
    latestListone: { loaded: false, active: false, entries: 0, map: new Map(), globalMap: new Map(), source: '', label: '' },
  ...
  function teamKey(value) {
    ...
    return TEAM_ALIAS_V623.get(key) || key;
  }
  ...
  async function loadLeagueRosters() {
    ...
    try {
      ...
      console.warn('[ioSudo V623] Rose live non caricate: uso snapshot Sudatori incorporato.', error);
    }
  }
  ...
  async function loadLatestListone() {
    state.latestListone = { loaded: false, active: false, entries: 0, map: new Map(), globalMap: new Map(), source: '', label: '' };
    try {
      ...
      state.latestListone = { loaded: true, active: players.length > 0, entries: players.length, map: map, globalMap: globalMap, source: entry.file, label: entry.label || payload?.meta?.label || entry.id || '' };
    } catch (error) {
      console.warn('[ioSudo V623] Listone piu recente non caricato.', error);
    }
  }
  ...
  function sourceHref(item) {
    ...
    if (provider === 'Bwin') return direct || 'https://www.bwin.it/it/news/calcio/amichevoli-estive-udinese-2026';
    if (provider === 'Udinese.it') return direct || 'https://www.udinese.it/';
    if (provider === "Calcio d'Angolo") return direct || 'https://calciodangolo.com/';
    if (provider === 'Pazzi di Fanta') return direct || 'https://www.pazzidifanta.com/';
    return direct || '';
  }
  ...
  function talksForPlayer(player) {
    if (player && player.virtualMarketRows) {
      return player.virtualMarketRows.filter(function (row) { return row && /^talks/.test(row.key || ''); }).map(function (row) { return row.item; });
    }
  ...
  function officialIncomingForPlayer(player) {
    if (player && player.virtualMarketRows) {
      return player.virtualMarketRows.filter(function (row) { return row && row.key === 'officialIncoming'; }).map(function (row) { return row.item; });
    }
    return (teamSummary(player && player.teamId).officialIncoming || [])
      .filter(function (item) { return itemMatchesPlayer(item, player); });
  }

  function officialOutgoingForPlayer(player) {
    if (player && player.virtualMarketRows) {
      return player.virtualMarketRows.filter(function (row) { return row && row.key === 'officialOutgoing'; }).map(function (row) { return row.item; });
    }
    return (teamSummary(player && player.teamId).officialOutgoing || [])
      .filter(function (item) { return itemMatchesPlayer(item, player); });
  }
  ...
  function marketPlayerRole(row) {
    const item = row && row.item || row || {};
    return String(item.role || item.classicRole || item.fantasyRole || item.position || item.playerRole || '').trim();
  }

  function marketPlayerKey(row) {
    const team = row && row.team || {};
    return [team.id || teamKey(team.name || ''), canonName(marketPlayerName(row)), roleKeyForRoster(marketPlayerRole(row))].filter(Boolean).join('::');
  }
  ...
  function findPlayerForMarketRow(row) {
    ...
    const candidates = teamId ? teamPlayers(teamId) : state.allPlayers;
    const local = candidates.find(function (player) {
      if (!itemMatchesPlayer(row.item, player)) return false;
      if (role && roleKeyForRoster(player.role || '') && roleKeyForRoster(player.role || '') !== role) return false;
      return true;
    });
  ...
  function makeVirtualPlayer(row) {
    ...
    const player = {
      ...
      teamName: team.name || item.teamName || '',
      realTeam: team.name || item.teamName || '',
  ...
  function playerLastUpdateTime(player) {
    ...
    teamInjuries(player.teamId).filter(function (item) { return itemMatchesPlayer(item, player); }).forEach(function (item) { values.push(itemUpdatedTime(item)); });
  ...
  function collectPlayerRows() {
    ...
    [].concat(collectMarketRows('rumor'), collectMarketRows('official')).forEach(function (row) {
      const name = marketPlayerName(row);
      if (!name) return;
    ...
    return Array.from(rowsByKey.values()).sort(function (a, b) {
  ...
  function globalPlayerItem(row) {
    ...
    const listone = playerInLatestListone(player);
    const listoneBadge = '<span class="iosudo-pill ' + (listone.present ? 'iosudo-listone-ok' : 'iosudo-listone-missing') + '">Listone: ' + (listone.present ? 'SI' : 'NO') + '</span>';
    const fantasy = fantasyRosterText(player) || '-';
    const updated = playerLastUpdateText(player);
    return '<article class="iosudo-list-row iosudo-compact-row iosudo-player-global-row ' + escapeHtml(roleClass(player.role)) + '">'
      ...
      + '<p><strong>' + escapeHtml(player.role || '-') + '</strong> · ' + escapeHtml(player.teamName || '-') + ' · Fantasy: ' + escapeHtml(fantasy) + '</p>'
  ...
  async function loadData() {
    ...
    setStatus('Dati aggiornati al ' + formatDate(updated) + ' - versione ' + (state.manifest.version || state.data.meta.version || 'corrente') + (state.liveRoster && state.liveRoster.active ? ' - rose live ' + state.liveRoster.source : '') + (state.latestListone && state.latestListone.active ? ' - listone ' + state.latestListone.source : ''));
    renderSummary();
    renderResults();
    parseHash();
  }
