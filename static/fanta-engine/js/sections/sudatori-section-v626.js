(() => {
  ...
  function teamKey(value) { const key = normalize(value); return TEAM_ALIAS_V626.get(key) || key; }
...
function roleKeyForRoster(role) {
  ...
  async function loadLeagueRosters() {
    ...
    try {
      ...
      console.warn('[Sudatori V626] Rose live non caricate: uso snapshot Sudatori incorporato.', err);
    }
  }
  ...
  async function loadListone() {
    ...
    try {
      ...
      console.warn('[Sudatori V626] Listone live non caricato: uso snapshot incorporato se disponibile.', err);
    }
  }
  ...
  function ensureSection() {
    ...
    section.className = 'app-page sudatori-page-v626';
    section.dataset.page = PAGE;
    section.setAttribute('aria-labelledby', 'sudatoriTitleV626');
    section.innerHTML = `<section class="panel sudatori-shell-v626"><div class="sudatori-hero-v626"><div><p class="eyebrow">Sezione standalone</p><h2 id="sudatoriTitleV626">Per i SUDATORI</h2><p>Rose Serie A, schede giocatore, ritiri, allenatori, moduli e amichevoli estive.</p><small class="muted">Modulo isolato: legge dati statici e non modifica Firebase.</small></div></div><div class="sudatori-status-v626" data-sudatori-status>Caricamento dati Sudatori...</div></section>`;
  ...
  function renderMarketSignal(player) {
    ...
    return `<span class="sudatori-market-flag-v626 ${badge.cls}" title="${escapeHtml(badge.title)}">${escapeHtml(badge.label)}</span>`;
  }

  function renderKpis(data) {
    return `<div class="sudatori-kpis-v626" aria-label="Indicatori Sudatori"><div class="sudatori-kpi-v626"><span>Squadre Serie A</span><strong>${escapeHtml(data.meta?.teams || data.teams?.length || '-')}</strong></div><div class="sudatori-kpi-v626"><span>Giocatori monitorati</span><strong>${escapeHtml(data.meta?.players || '-')}</strong></div><div class="sudatori-kpi-v626"><span>Amichevoli</span><strong>${escapeHtml(data.meta?.friendlies || '-')}</strong></div><div class="sudatori-kpi-v626"><span>Note mercato</span><strong>${escapeHtml(data.meta?.marketNotes || '0')}</strong></div><div class="sudatori-kpi-v626"><span>Trattative squadre</span><strong>${escapeHtml(data.meta?.teamTransferTalks || '0')}</strong></div><div class="sudatori-kpi-v626"><span>Infortunati / SOS</span><strong>${escapeHtml(data.meta?.injuries || '0')}</strong></div><div class="sudatori-kpi-v626"><span>Moduli allineati</span><strong>${escapeHtml(data.meta?.formationTeams || '0')}</strong></div></div>`;
  }
  ...
  function renderTeamCards(data) {
    ...
    return `<div id="sudatoriTeamsV626" class="sudatori-grid-v626">${teams.map((t) => {
      const players = getPlayers(t.id).slice(0, 10);
      return `<button class="sudatori-team-card-v626 ${t.id === state.selectedTeamId ? 'is-selected' : ''}" type="button" data-sudatori-team="${escapeHtml(t.id)}"><div class="sudatori-team-card-head-v626"><h3>${escapeHtml(t.name)}</h3><span class="sudatori-badge-v626">${escapeHtml(t.abbr || t.playersCount || '')}</span></div><div class="sudatori-card-meta-v626"><span>Allenatore<br><strong>${escapeHtml(t.coach || '-')}</strong></span><span>Modulo<br><strong>${escapeHtml(t.formationModule || t.module || '-')}</strong></span><span>Ritiro<br><strong>${escapeHtml(t.retreatPlace || 'Da confermare')}</strong></span><span>Amichevoli<br><strong>${escapeHtml(t.friendliesCount || getFriendlies(t.id).length || 0)}</strong></span><span>Trattative<br><strong>${escapeHtml(t.transferTalksCount || getTransferTalks(t.id).length || 0)}</strong></span><span>SOS Fisico<br><strong>${escapeHtml(t.injuriesCount || getInjuries(t.id).length || 0)}</strong></span></div><div class="sudatori-roster-preview-v626" aria-label="Anteprima rosa">${players.map((p) => `<span class="sudatori-chip-v626" data-role="${escapeHtml(p.role)}">${escapeHtml(p.role || '-')} · ${escapeHtml(p.playerName)}</span>`).join('')}${getPlayers(t.id).length > players.length ? `<span class="sudatori-chip-v626">+${getPlayers(t.id).length - players.length}</span>` : ''}</div></button>`;
    }).join('')}</div>`;
  }
  ...
  function physicalBadgeFromText(text) {
    const value = String(text || '').trim();
    if (!value || /^OK\b/i.test(value)) return '<span class="sudatori-physical-v626">OK</span>';
    const cls = /infortun|stop|operat|out|salta/i.test(value) ? 'is-out' : 'is-risk';
    return `<span class="sudatori-physical-v626 ${cls}">${escapeHtml(value)}</span>`;
  }
  function physicalPitchBadgeFromText(text) {
    const value = String(text || '').trim();
    if (!hasPhysicalSignal(value)) return '';
    return `<span class="sudatori-physical-v626 is-field-alert">${escapeHtml(value)}</span>`;
  }
  ...
  function renderPitch(team) {
    ...
    if (!xi.length) return '<div class="sudatori-pitch-empty-v626">Probabile formazione non disponibile per questa squadra.</div>';
    ...
    const renderPlayer = (p) => {
      ...
      if (p.playerId && rosterPlayer) return `<button class="sudatori-pitch-player-v626" type="button" data-sudatori-player="${escapeHtml(p.playerId)}" title="${escapeHtml(p.playerName)}">${body}</button>`;
      return `<span class="sudatori-pitch-player-v626 is-external" title="${escapeHtml(p.playerName)}">${body}</span>`;
    };
    const renderLine = (key, label) => groups[key].length ? `<div class="sudatori-pitch-line-v626 sudatori-pitch-line-${key}-v626" aria-label="${escapeHtml(label)}">${sortPitchLine(groups[key]).map(renderPlayer).join('')}</div>` : '';
    const adjusted = xi.some((p) => p.moduleAdjusted);
    const sourceUsed = xi.find((p) => p.moduleSourceUsed)?.moduleSourceUsed || '';
    const moduleNote = adjusted ? `<div class="sudatori-pitch-module-note-v626">Forma campetto allineata al modulo dichiarato ${escapeHtml(team.formationModule || team.module || '-')}; fonte formazione originale: ${escapeHtml(sourceUsed || '-')}.</div>` : '';
    return `<div class="sudatori-pitch-card-v626" aria-label="Probabile formazione ${escapeHtml(team.name)}"><div class="sudatori-pitch-head-v626"><div><h4>Probabile formazione</h4><p>${escapeHtml(team.formationModule || team.module || 'Modulo da confermare')} · ${escapeHtml(team.coach || '-')}</p>${moduleNote}</div><span>${escapeHtml(xi.length)} giocatori</span></div><div class="sudatori-pitch-v626"><div class="sudatori-pitch-marking sudatori-pitch-marking-center"></div><div class="sudatori-pitch-marking sudatori-pitch-marking-box-a"></div><div class="sudatori-pitch-marking sudatori-pitch-marking-box-b"></div>${renderLine('attack', 'Attacco')}${renderLine('attackingMidfield', 'Trequarti')}${renderLine('midfield', 'Centrocampo')}${renderLine('defense', 'Difesa')}${renderLine('goalkeeper', 'Portiere')}${renderLine('bench', 'Altri probabili')}</div></div>`;
  }
  ...
  function renderRosterTable(team) {
    ...
    const rows = players.map((p) => {
      ...
      return `<tr data-role="${escapeHtml(p.role || '')}"><td><button type="button" class="sudatori-player-button-v626" data-sudatori-player="${escapeHtml(p.id)}">${escapeHtml(p.playerName)}</button></td><td><span class="sudatori-role-v626" data-role="${escapeHtml(p.role || '')}">${escapeHtml(p.role || '-')}</span></td><td>${escapeHtml(fantasyRoster)}</td><td>${escapeHtml(params || '-')}</td><td>${mercato}</td><td>${physical}${injury}</td></tr>`;
    }).join('');
    return `<div class="sudatori-table-wrap-v626"><table class="sudatori-table-v626"><thead><tr><th>Giocatore</th><th>Ruolo</th><th>Rosa fantacalcio</th><th>Parametri listone</th><th>Mercato</th><th>Stato fisico</th></tr></thead><tbody>${rows}</tbody></table></div>`;
  }

  function renderInjuries(team) {
    const injuries = getInjuries(team.id);
    if (!injuries.length) {
      return '<section class="sudatori-injuries-v626" aria-labelledby="sudatoriInjuriesTitleV626"><header><div><h4 id="sudatoriInjuriesTitleV626">Infortunati / SOS Fanta</h4><p>Nessuna segnalazione fisica SOS Fanta nel file caricato.</p></div></header></section>';
    }
    const cards = injuries.map((item) => {
      ...
      const body = `<div class="sudatori-injury-head-v626"><strong>${escapeHtml(item.playerName || '-')}</strong>${badge}</div><p>${escapeHtml(item.injury || 'Dettaglio non indicato')}</p><small>Rientro: ${escapeHtml(item.potentialReturn || 'Da valutare')} · Impatto asta: ${escapeHtml(item.auctionImpact || '-')} · ${source}</small>${item.note ? `<div class="sudatori-injury-note-v626">${escapeHtml(item.note)}</div>` : ''}`;
      if (item.playerId) return `<article class="sudatori-injury-card-v626"><button type="button" class="sudatori-player-button-v626" data-sudatori-player="${escapeHtml(item.playerId)}">${body}</button></article>`;
      return `<article class="sudatori-injury-card-v626">${body}</article>`;
    }).join('');
    return `<section class="sudatori-injuries-v626" aria-labelledby="sudatoriInjuriesTitleV626"><header><div><h4 id="sudatoriInjuriesTitleV626">Infortunati / SOS Fanta ${escapeHtml(team.name)}</h4><p>Entità, rientri e note operative per asta/gestione.</p></div><span class="sudatori-badge-v626">${escapeHtml(injuries.length)} segnalazioni</span></header><div class="sudatori-injury-grid-v626">${cards}</div></section>`;
  }
  ...
  function renderMarketMiniItem(item, kind) {
    ...
    return `<li class="sudatori-market-summary-item-v626" data-kind="${escapeHtml(kind)}"><strong>${escapeHtml(name)}</strong><small>${escapeHtml(role)}${origin ? ` · ${escapeHtml(origin)}` : ''}</small><span class="sudatori-market-summary-meta-v626">${status}${date}${source}</span></li>`;
  }

  function renderMarketSummaryList(title, items, kind) {
    const safeItems = items || [];
    const body = safeItems.length ? `<ul>${safeItems.slice(0, 8).map((item) => renderMarketMiniItem(item, kind)).join('')}</ul>${safeItems.length > 8 ? `<div class="sudatori-market-summary-more-v626">+${escapeHtml(safeItems.length - 8)} altri nomi nel file</div>` : ''}` : '<p class="muted">Nessun nome nel file.</p>';
    return `<article class="sudatori-market-summary-block-v626" data-kind="${escapeHtml(kind)}"><h5>${escapeHtml(title)} <span>${escapeHtml(safeItems.length)}</span></h5>${body}</article>`;
  }

  function renderMarketOverview(team) {
    const summary = getMarketSummary(team.id);
    return `<section class="sudatori-market-overview-v626" aria-labelledby="sudatoriMarketOverviewTitleV626"><header><div><h4 id="sudatoriMarketOverviewTitleV626">Riepilogo mercato ${escapeHtml(team.name)}</h4><p>Ufficialità e trattative separate fra entrata e uscita.</p></div></header><div class="sudatori-market-summary-grid-v626">${renderMarketSummaryList('Ufficialità in entrata', summary.officialIncoming, 'official-in')}${renderMarketSummaryList('Ufficialità in uscita', summary.officialOutgoing, 'official-out')}${renderMarketSummaryList('Trattative in entrata', summary.talksIncoming, 'talk-in')}${renderMarketSummaryList('Trattative in uscita', summary.talksOutgoing, 'talk-out')}</div></section>`;
  }

  function renderTransferTalks(team) {
    const talks = getTransferTalks(team.id);
    if (!talks.length) {
      return '<section class="sudatori-transfer-talks-v626" aria-labelledby="sudatoriTalksTitleV626"><header><div><h4 id="sudatoriTalksTitleV626">Trattative in corso</h4><p>Nessuna trattativa in corso disponibile nel file caricato.</p></div></header></section>';
    }
    const cards = talks.map((talk) => {
      ...
      return `<article class="sudatori-transfer-card-v626" data-role="${escapeHtml(talk.roleBrief || '')}"><div class="sudatori-transfer-head-v626"><span class="sudatori-role-v626" data-role="${escapeHtml(talk.roleBrief || '')}">${escapeHtml(talk.roleBrief || '-')}</span><strong>${escapeHtml(talk.target || '-')}</strong></div><p><span class="sudatori-transfer-direction-v626">${escapeHtml(direction)}</span> · ${escapeHtml(talk.role || 'Ruolo non indicato')}${talk.origin ? ` · ${escapeHtml(talk.origin)}` : ''}</p><small>${escapeHtml(talk.reliability || 'Affidabilità non indicata')}${talk.updatedAt ? ` · ${escapeHtml(formatDate(talk.updatedAt))}` : ''} · ${escapeHtml(sources.length)} fonti/notizie</small>${sourceLinks ? `<ul class="sudatori-transfer-sources-v626">${sourceLinks}</ul>` : ''}${noteText ? `<div class="sudatori-transfer-note-v626">${escapeHtml(noteText)}</div>` : ''}</article>`;
    }).join('');
    return `<section class="sudatori-transfer-talks-v626" aria-labelledby="sudatoriTalksTitleV626"><header><div><h4 id="sudatoriTalksTitleV626">Trattative in corso ${escapeHtml(team.name)}</h4><p>Una sola card per giocatore; eventuali fonti multiple sono aggregate nella stessa scheda. Le ufficialità non vengono mostrate qui.</p></div><span class="sudatori-badge-v626">${escapeHtml(talks.length)} nomi</span></header><div class="sudatori-transfer-grid-v626">${cards}</div></section>`;
  }

  function renderFriendlies(team) {
    const items = getFriendlies(team.id);
    if (!items.length) return '<div class="sudatori-friendlies-v626"><p class="muted">Nessuna amichevole disponibile.</p></div>';
    return `<div id="sudatoriFriendliesV626" class="sudatori-friendlies-v626">${items.map((f) => `<div class="sudatori-friendly-v626"><strong>${escapeHtml(f.event || '-')}</strong><small>${escapeHtml(formatDate(f.date) || 'Data da definire')}${f.time ? ` · ${escapeHtml(f.time)}` : ''}${f.venue ? ` · ${escapeHtml(f.venue)}` : ''}</small><div>${escapeHtml(f.status || '')}</div></div>`).join('')}</div>`;
  }
  function renderPlayerPanel(player) {
    ...
    const formationNote = player.formationPosition ? `<div class="sudatori-note-v626"><strong>Probabile formazione:</strong><br>${escapeHtml(player.formationPosition)} nel ${escapeHtml(player.module || team.formationModule || team.module || '-')} · affidabilità ${escapeHtml(player.formationReliability || '-')}<br>${escapeHtml(player.formationNote || '')}</div>` : '';
    const injuryNote = player.sosFantaFlag ? `<div class="sudatori-note-v626"><strong>Stato fisico SOS Fanta:</strong><br>${physicalBadgeFromText(player.physicalStatus || player.injuryStatus)} ${escapeHtml(player.injuryDetail || '')}<br><strong>Rientro:</strong> ${escapeHtml(player.injuryReturn || 'Da valutare')} · <strong>Impatto asta:</strong> ${escapeHtml(player.injuryImpact || '-')}<br>${player.injurySource ? `<a href="${escapeHtml(player.injurySource)}" target="_blank" rel="noopener">Fonte SOS Fanta</a>` : ''}${player.injuryNote ? `<div class="sudatori-injury-note-v626">${escapeHtml(player.injuryNote)}</div>` : ''}</div>` : (player.formationPhysicalStatus && !/^OK\b/i.test(player.formationPhysicalStatus) ? `<div class="sudatori-note-v626"><strong>Monitoraggio fisico formazione:</strong><br>${physicalBadgeFromText(player.formationPhysicalStatus)} ${escapeHtml(player.formationPotentialReturn || '')}${player.formationSosSource ? `<br><a href="${escapeHtml(player.formationSosSource)}" target="_blank" rel="noopener">Fonte SOS Fanta</a>` : ''}</div>` : '');
    return `<div class="sudatori-player-body-v626"><div><p class="eyebrow">Scheda giocatore</p><h3>${escapeHtml(player.playerName)}</h3><p>${escapeHtml(player.role || '-')} · ${escapeHtml(player.teamName || '-')} · Rosa fantacalcio: <strong>${escapeHtml(fantasyRoster)}</strong></p><p>${escapeHtml(team.coach || '-')} · ${escapeHtml(team.formationModule || team.module || '-')}</p></div><div class="sudatori-param-grid-v626"><div class="sudatori-param-v626"><span>Qt.A</span><strong>${escapeHtml(extra.quotationCurrent ?? '-')}</strong></div><div class="sudatori-param-v626"><span>FVM</span><strong>${escapeHtml(extra.fvm ?? '-')}</strong></div><div class="sudatori-param-v626"><span>Media</span><strong>${escapeHtml(extra.average ?? extra.sourceExtra?.average ?? '-')}</strong></div><div class="sudatori-param-v626"><span>Fantamedia</span><strong>${escapeHtml(extra.fantasyAverage ?? extra.sourceExtra?.fantasyAverage ?? '-')}</strong></div><div class="sudatori-param-v626"><span>Presenze</span><strong>${escapeHtml(extra.played ?? extra.sourceExtra?.played ?? '-')}</strong></div><div class="sudatori-param-v626"><span>Stato listone</span><strong>${escapeHtml(extra.status || 'Non trovato')}</strong></div></div>${url ? `<a class="button button-primary" href="${escapeHtml(url)}" target="_blank" rel="noopener">Apri scheda Fantacalcio.it</a>` : '<div class="sudatori-note-v626">Link Fantacalcio.it non disponibile: manca id nel listone.</div>'}${injuryNote}<div class="sudatori-note-v626"><strong>Mercato:</strong><br>${escapeHtml(player.marketStatus || 'In rosa')} · ${escapeHtml(player.marketDetail || 'Nessuna segnalazione mercato rilevante nelle fonti consultate.')} ${player.marketSource ? `<br><a href="${escapeHtml(player.marketSource)}" target="_blank" rel="noopener">Fonte mercato</a>` : ''}</div>${formationNote}<div class="sudatori-note-v626"><strong>Nota rosa:</strong><br>${escapeHtml(player.note || 'Nessuna nota specifica disponibile.')}</div><div><h4>Mercato / rapporti / contesto</h4>${notes.length ? notes.map((n) => `<div class="sudatori-market-note-v626"><strong>${escapeHtml(n.type || n.status || 'Nota mercato')}</strong><p>${escapeHtml(n.note || n.status || 'Aggiornamento disponibile.')}</p><small>${escapeHtml(n.team || '')}${n.source ? ` · ${escapeHtml(n.source)}` : ''}</small></div>`).join('') : '<p class="muted">Nessuna informazione mercato/rapporti disponibile nel file caricato.</p>'}</div><div><h4>Amichevoli squadra</h4>${getFriendlies(player.teamId).slice(0, 5).map((f) => `<div class="sudatori-friendly-v626"><strong>${escapeHtml(f.event)}</strong><small>${escapeHtml(formatDate(f.date) || 'Data da definire')}${f.time ? ` · ${escapeHtml(f.time)}` : ''}</small></div>`).join('') || '<p class="muted">Nessuna amichevole disponibile.</p>'}</div><div class="sudatori-sources-v626"><strong>Fonte rosa:</strong> ${player.rosterSource ? `<a href="${escapeHtml(player.rosterSource)}" target="_blank" rel="noopener">Transfermarkt / fonte rosa</a>` : 'non indicata'}</div></div>`;
  }
  ...
  function renderSources(data) {
    ...
    return `<details class="sudatori-sources-v626"><summary>Fonti dati</summary><ul>${sources.map((s) => `<li>${s.url ? `<a href="${escapeHtml(s.url)}" target="_blank" rel="noopener">${escapeHtml(s.name || s.url)}</a>` : escapeHtml(s.name || '-') } <span>${escapeHtml(s.use || '')}</span></li>`).join('')}</ul></details>`;
  }
  function render() {
    ...
    if (!team) return;
    section.innerHTML = `<section class="panel sudatori-shell-v626"><div class="sudatori-hero-v626"><div><p class="eyebrow">Sezione standalone · ${escapeHtml(data.meta?.version || VERSION)}</p><h2 id="sudatoriTitleV626">Per i SUDATORI</h2><p>Rose del campionato, schede giocatore, parametri dal listone, mercato, trattative, infortunati SOS Fanta, allenatori, moduli, ritiri e amichevoli.</p><small class="muted">Aggiornato al ${escapeHtml(formatDate(data.meta?.updatedAt))} · fonte: ${escapeHtml(data.meta?.sourceFile || '-')}</small></div><div class="sudatori-hero-actions-v626"><button class="button button-primary" type="button" data-sudatori-scroll="teams">Rose campionato</button><button class="button button-secondary" type="button" data-sudatori-scroll="injuries">Infortunati</button><button class="button button-secondary" type="button" data-sudatori-scroll="talks">Trattative</button><button class="button button-secondary" type="button" data-sudatori-scroll="friendlies">Amichevoli</button></div></div>${renderKpis(data)}${renderToolbar(data)}<section aria-labelledby="sudatoriRoseCampionatoTitleV626"><h3 id="sudatoriRoseCampionatoTitleV626">Card rose del campionato</h3><p class="muted">Ogni card mostra allenatore, modulo, ritiro, numero amichevoli e anteprima rosa.</p>${renderTeamCards(data)}</section><div class="sudatori-layout-v626"><section class="sudatori-section-card-v626" aria-labelledby="sudatoriRosterTitleV626"><header><div><h3 id="sudatoriRosterTitleV626">Rosa ${escapeHtml(team.name)}</h3><p>${escapeHtml(team.coach || '-')} · ${escapeHtml(team.formationModule || team.module || '-')}</p></div><span class="sudatori-badge-v626">${escapeHtml(getPlayers(team.id).length)} giocatori</span></header>${renderRosterTable(team)}${renderPitch(team)}${renderInjuries(team)}${renderMarketOverview(team)}${renderTransferTalks(team)}</section><section class="sudatori-section-card-v626 sudatori-player-panel-v626" aria-labelledby="sudatoriPlayerTitleV626"><header><div><h3 id="sudatoriPlayerTitleV626">Scheda giocatore</h3><p>Parametri e contesto dal file Sudatori + listone.</p></div></header>${renderPlayerPanel(state.selectedPlayer)}</section></div><section class="sudatori-section-card-v626" aria-labelledby="sudatoriFriendliesTitleV626" style="margin-top:1rem;"><header><div><h3 id="sudatoriFriendliesTitleV626">Ritiri e amichevoli ${escapeHtml(team.name)}</h3><p>${escapeHtml(team.retreatPlace || 'Ritiro da confermare')} ${team.retreatStart ? `· ${escapeHtml(formatDate(team.retreatStart))}` : ''}</p></div><span class="sudatori-badge-v626">${escapeHtml(getFriendlies(team.id).length)} eventi</span></header>${renderFriendlies(team)}</section>${renderSources(data)}</section>`;
  ...
  function bindSectionEvents(section) {
    ...
    section.querySelectorAll('[data-sudatori-team]').forEach((card) => card.addEventListener('click', () => { state.selectedTeamId = card.dataset.sudatoriTeam || state.selectedTeamId; state.selectedPlayer = null; render(); section.querySelector('#sudatoriRosterTitleV626')?.scrollIntoView({ behavior: 'smooth', block: 'start' }); }));
    section.querySelectorAll('[data-sudatori-player]').forEach((button) => button.addEventListener('click', () => { const id = button.dataset.sudatoriPlayer; state.selectedPlayer = getPlayers(state.selectedTeamId).find((p) => p.id === id) || null; render(); section.querySelector('#sudatoriPlayerTitleV626')?.scrollIntoView({ behavior: 'smooth', block: 'start' }); }));
    section.querySelectorAll('[data-sudatori-scroll]').forEach((button) => button.addEventListener('click', () => { const target = button.dataset.sudatoriScroll === 'friendlies' ? '#sudatoriFriendliesTitleV626' : (button.dataset.sudatoriScroll === 'injuries' ? '#sudatoriInjuriesTitleV626' : (button.dataset.sudatoriScroll === 'talks' ? '#sudatoriTalksTitleV626' : '#sudatoriTeamsV626'));  section.querySelector(target)?.scrollIntoView({ behavior: 'smooth', block: 'start' }); }));
  }
  async function activateSudatori() {
    ensureNav(); ensureSection(); setActivePage(PAGE);
    try { await loadData(); render(); }
    catch (err) { console.error('[Sudatori V626] Errore caricamento', err); const status = ensureSection().querySelector('[data-sudatori-status]'); if (status) status.innerHTML = `<div class="notice notice-error">Errore caricamento sezione Sudatori: ${escapeHtml(err.message || err)}</div>`; }
  }
  ...
  window.FantaSudatoriV626 = { activate: activateSudatori, reload: () => { state.loaded = false; state.data = null; return activateSudatori(); } };
})();
