(() => {
  ...
  const DATA_MANIFEST = `${ENGINE_BASE}/data/sudatori/current/manifest.json?v=644`;
  ...
  const LISTONE_MANIFEST = `${ENGINE_BASE}/data/shared-assets/current/assets/listoni/manifest.json?v=644`;
  ...
  function teamKey(value) { const key = normalize(value); return TEAM_ALIAS_V644.get(key) || key; }
...
function roleKeyForRoster(role) {
  ...
  async function loadLeagueRosters() {
    ...
    try {
      ...
      console.warn('[Sudatori V644] Rose live non caricate: uso snapshot Sudatori incorporato.', err);
    }
  }
  ...
  async function loadData() {
    ...
    const data = await fetchJson(`${ENGINE_BASE}/data/sudatori/current/${dataFile}?v=644`);
    state.data = data;
  ...
  async function loadListone() {
    ...
    try {
      ...
      if (!pick) return;
      const json = await fetchJson(`${ENGINE_BASE}/data/shared-assets/current/assets/listoni/${pick.file}?v=644`);
      ...
      console.warn('[Sudatori V644] Listone live non caricato: uso snapshot incorporato se disponibile.', err);
    }
  }
  ...
  function renderTeamCards(data) {
    ...
    return `<div id="sudatoriTeamsV644" class="sudatori-grid-v631">${teams.map((t) => {
  ...
  function renderInjuries(team) {
    ...
    if (!injuries.length) {
      return '<section class="sudatori-injuries-v631" aria-labelledby="sudatoriInjuriesTitleV644"><header><div><h4 id="sudatoriInjuriesTitleV644">Infortunati / SOS Fanta</h4><p>Nessuna segnalazione fisica SOS Fanta nel file caricato.</p></div></header></section>';
    }
    const cards = injuries.map((item) => {
      const badge = physicalBadgeFromText(item.status);
      const injuryHref = itemArticleUrlV644(item);
      const source = injuryHref ? `<a href="${escapeHtml(injuryHref)}" target="_blank" rel="noopener">${escapeHtml(itemSourceLabelV644(item, 'Articolo SOS Fanta'))}</a>` : 'Fonte non indicata';
    ...
    return `<section class="sudatori-injuries-v631" aria-labelledby="sudatoriInjuriesTitleV644"><header><div><h4 id="sudatoriInjuriesTitleV644">Infortunati / SOS Fanta ${escapeHtml(team.name)}</h4><p>Entità, rientri e note operative per asta/gestione.</p></div><span class="sudatori-badge-v631">${escapeHtml(injuries.length)} segnalazioni</span></header><div class="sudatori-injury-grid-v631">${cards}</div></section>`;
  }
  ...
  function renderMarketMiniItem(item, kind) {
    ...
    const itemHref = itemArticleUrlV644(item);
    const source = itemHref ? `<a href="${escapeHtml(itemHref)}" target="_blank" rel="noopener">${escapeHtml(itemSourceLabelV644(item, 'Articolo'))}</a>` : '';
  ...
  function renderTransferTalks(team) {
    ...
    const cards = talks.map((talk) => {
      ...
      const sourceLinks = sources.slice(0, 4).map((src, index) => {
        ...
        const srcHref = itemArticleUrlV644(src);
  ...
  function renderFriendlies(team) {
    ...
    return `<div id="sudatoriFriendliesV644" class="sudatori-friendlies-v631">${items.map((f) => `<div class="sudatori-friendly-v631"><strong>${escapeHtml(f.event || '-')}</strong><small>${escapeHtml(formatDate(f.date) || 'Data da definire')}${f.time ? ` · ${escapeHtml(f.time)}` : ''}${f.venue ? ` · ${escapeHtml(f.venue)}` : ''}</small><div>${escapeHtml(f.status || '')}</div></div>`).join('')}</div>`;
  }
  ...
  function render() {
    ...
    if (!team) return;
    section.innerHTML = `<section class="panel sudatori-shell-v631"><div class="sudatori-hero-v631"><div><p class="eyebrow">Sezione standalone · ${escapeHtml(data.meta?.version || VERSION)}</p><h2 id="sudatoriTitleV644">Per i SUDATORI</h2><p>Rose del campionato, schede giocatore, parametri dal listone, mercato, trattative, infortunati SOS Fanta, allenatori, moduli, ritiri e amichevoli.</p><small class="muted">Aggiornato al ${escapeHtml(formatDate(data.meta?.updatedAt))} · fonte: ${escapeHtml(data.meta?.sourceFile || '-')}</small></div><div class="sudatori-hero-actions-v631"><button class="button button-primary" type="button" data-sudatori-scroll="teams">Rose campionato</button><button class="button button-secondary" type="button" data-sudatori-scroll="injuries">Infortunati</button><button class="button button-secondary" type="button" data-sudatori-scroll="talks">Trattative</button><button class="button button-secondary" type="button" data-sudatori-scroll="friendlies">Amichevoli</button></div></div>${renderKpis(data)}${renderToolbar(data)}<section aria-labelledby="sudatoriRoseCampionatoTitleV644"><h3 id="sudatoriRoseCampionatoTitleV644">Card rose del campionato</h3><p class="muted">Ogni card mostra allenatore, modulo, ritiro, numero amichevoli e anteprima rosa.</p>${renderTeamCards(data)}</section><div class="sudatori-layout-v631"><section class="sudatori-section-card-v631" aria-labelledby="sudatoriRosterTitleV644"><header><div><h3 id="sudatoriRosterTitleV644">Rosa ${escapeHtml(team.name)}</h3><p>${escapeHtml(team.coach || '-')} · ${escapeHtml(team.formationModule || team.module || '-')}</p></div><span class="sudatori-badge-v631">${escapeHtml(getPlayers(team.id).length)} giocatori</span></header>${renderRosterTable(team)}${renderPitch(team)}${renderInjuries(team)}${renderMarketOverview(team)}${renderTransferTalks(team)}</section><section class="sudatori-section-card-v631 sudatori-player-panel-v631" aria-labelledby="sudatoriPlayerTitleV644"><header><div><h3 id="sudatoriPlayerTitleV644">Scheda giocatore</h3><p>Parametri e contesto dal file Sudatori + listone.</p></div></header>${renderPlayerPanel(state.selectedPlayer)}</section></div><section class="sudatori-section-card-v631" aria-labelledby="sudatoriFriendliesTitleV644" style="margin-top:1rem;"><header><div><h3 id="sudatoriFriendliesTitleV644">Ritiri e amichevoli ${escapeHtml(team.name)}</h3><p>${escapeHtml(team.retreatPlace || 'Ritiro da confermare')} ${team.retreatStart ? `· ${escapeHtml(formatDate(team.retreatStart))}` : ''}</p></div><span class="sudatori-badge-v631">${escapeHtml(getFriendlies(team.id).length)} eventi</span></header>${renderFriendlies(team)}</section>${renderSources(data)}</section>`;
  ...
  function bindSectionEvents(section) {
    ...
    section.querySelectorAll('[data-sudatori-team]').forEach((card) => card.addEventListener('click', () => { state.selectedTeamId = card.dataset.sudatoriTeam || state.selectedTeamId; state.selectedPlayer = null; render(); section.querySelector('#sudatoriRosterTitleV644')?.scrollIntoView({ behavior: 'smooth', block: 'start' }); }));
    section.querySelectorAll('[data-sudatori-player]').forEach((button) => button.addEventListener('click', () => { const id = button.dataset.sudatoriPlayer; state.selectedPlayer = getPlayers(state.selectedTeamId).find((p) => p.id === id) || null; render(); section.querySelector('#sudatoriPlayerTitleV644')?.scrollIntoView({ behavior: 'smooth', block: 'start' }); }));
    section.querySelectorAll('[data-sudatori-scroll]').forEach((button) => button.addEventListener('click', () => { const target = button.dataset.sudatoriScroll === 'friendlies' ? '#sudatoriFriendliesTitleV644' : (button.dataset.sudatoriScroll === 'injuries' ? '#sudatoriInjuriesTitleV644' : (button.dataset.sudatoriScroll === 'talks' ? '#sudatoriTalksTitleV644' : '#sudatoriTeamsV644'));  section.querySelector(target)?.scrollIntoView({ behavior: 'smooth', block: 'start' }); }));
  }
  async function activateSudatori() {
    ensureNav(); ensureSection(); setActivePage(PAGE);
    try { await loadData(); render(); }
    catch (err) { console.error('[Sudatori V644] Errore caricamento', err); const status = ensureSection().querySelector('[data-sudatori-status]'); if (status) status.innerHTML = `<div class="notice notice-error">Errore caricamento sezione Sudatori: ${escapeHtml(err.message || err)}</div>`; }
  }
  ...
  window.FantaSudatoriV644 = { activate: activateSudatori, reload: () => { state.loaded = false; state.data = null; return activateSudatori(); } };
})();
