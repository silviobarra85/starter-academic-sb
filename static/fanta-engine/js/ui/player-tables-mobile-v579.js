(function () {
  ...
  var VERSION = 'v579';
  var scheduled = false;
  var ROLE_CLASSES = ['fpt-v579-role-p', 'fpt-v579-role-d', 'fpt-v579-role-c', 'fpt-v579-role-a'];
  var TYPE_CLASSES = ['fanta-player-table-v579-teamarea', 'fanta-player-table-v579-rose', 'fanta-player-table-v579-listone'];
  ...
  function ensureWrapper(table, type) {
    ...
    if (wrapper) {
      wrapper.classList.add('fanta-player-table-wrap-v579', 'fanta-player-table-wrap-v579-' + type);
      wrapper.setAttribute('data-player-table-wrap-v579', type);
    }
    return wrapper;
  }
  ...
  function roleFromRow(row, roleIndex) {
    ...
    var explicit = row.querySelector('.fpt-v579-col-role, .roster-col-role, .team-profile-role-cell, .listone-col-classicRole, [data-label="R (RM)"], [data-label="Ruolo"], [data-label="R"]');
  ...
  function applyRole(row, role) {
    ...
    if (role) {
      row.classList.add('fpt-v579-role-' + role);
      row.setAttribute('data-fpt-v579-role', role);
    } else {
      row.removeAttribute('data-fpt-v579-role');
    }
  }
  ...
  function markColumns(table) {
    ...
    addColumnClassByIndex(table, playerIndex >= 0 ? playerIndex : 0, 'fpt-v579-col-player');
    addColumnClassByIndex(table, roleIndex, 'fpt-v579-col-role');
    addColumnClassByIndex(table, statusIndex, 'fpt-v579-col-status');
    addColumnClassByIndex(table, teamIndex, 'fpt-v579-col-team');
    addColumnClassByIndex(table, costIndex, 'fpt-v579-col-cost');
    addColumnClassByIndex(table, qtaIndex, 'fpt-v579-col-qta');
    addColumnClassByIndex(table, marketIndex, 'fpt-v579-col-market');
  ...
  function roleColor(role) {
    ...
    if (dark) {
      if (role === 'p') return 'rgba(245, 158, 11, 0.24)';
      if (role === 'd') return 'rgba(34, 197, 94, 0.22)';
      if (role === 'c') return 'rgba(56, 189, 248, 0.22)';
      if (role === 'a') return 'rgba(248, 113, 113, 0.22)';
      return '#111827';
    }
    if (role === 'p') return 'rgba(245, 158, 11, 0.16)';
    if (role === 'd') return 'rgba(34, 197, 94, 0.14)';
    if (role === 'c') return 'rgba(56, 189, 248, 0.14)';
    if (role === 'a') return 'rgba(248, 113, 113, 0.14)';
    return '#ffffff';
  }

  function roleLine(role) {
    if (role === 'p') return 'rgba(245, 158, 11, 0.82)';
    if (role === 'd') return 'rgba(34, 197, 94, 0.78)';
    if (role === 'c') return 'rgba(56, 189, 248, 0.78)';
    if (role === 'a') return 'rgba(248, 113, 113, 0.78)';
    return 'rgba(148, 163, 184, 0.5)';
  }


  function roleFirstBackground(role) {
    var dark = document.documentElement.getAttribute('data-theme') === 'dark';
    if (dark) {
      if (role === 'p') return 'linear-gradient(90deg, rgba(245, 158, 11, 0.42), rgba(245, 158, 11, 0.20))';
      if (role === 'd') return 'linear-gradient(90deg, rgba(34, 197, 94, 0.40), rgba(34, 197, 94, 0.18))';
      if (role === 'c') return 'linear-gradient(90deg, rgba(56, 189, 248, 0.40), rgba(59, 130, 246, 0.18))';
      if (role === 'a') return 'linear-gradient(90deg, rgba(248, 113, 113, 0.40), rgba(239, 68, 68, 0.18))';
      return '#111827';
    }
    if (role === 'p') return 'linear-gradient(90deg, rgba(245, 158, 11, 0.34), rgba(245, 158, 11, 0.16))';
    if (role === 'd') return 'linear-gradient(90deg, rgba(34, 197, 94, 0.32), rgba(34, 197, 94, 0.14))';
    if (role === 'c') return 'linear-gradient(90deg, rgba(56, 189, 248, 0.32), rgba(59, 130, 246, 0.14))';
    if (role === 'a') return 'linear-gradient(90deg, rgba(248, 113, 113, 0.32), rgba(239, 68, 68, 0.14))';
    return '#ffffff';
  }
  ...
  function widthForType(type) {
    if (type === 'listone') {
      return {
        player: 'clamp(8.5rem, 46vw, 13.5rem)',
        playerMax: 'clamp(10rem, 54vw, 15rem)',
        tableMin: '100%'
      };
    }
    return {
  ...
  function applyInlineMobileStyles(table, type) {
    ...
    var wrapper = table.closest('[data-player-table-wrap-v579], .team-profile-roster-wrap, .roster-inline-table-wrap, .listone-table-wrap, .mobile-tabular-wrap, .table-wrap');
    if (wrapper) {
    ...
    Array.prototype.forEach.call(table.querySelectorAll('.fpt-v579-col-player, th:first-child, td:first-child'), function (cell) {
      important(cell, 'position', 'sticky');
    ...
    Array.prototype.forEach.call(table.querySelectorAll('thead .fpt-v579-col-player, thead th:first-child'), function (cell) {
      important(cell, 'background-color', 'rgba(15, 23, 42, 0.99)');
      important(cell, 'color', '#f8fafc');
    });

    Array.prototype.forEach.call(table.querySelectorAll('.fpt-v579-col-role, .roster-col-role, .team-profile-role-cell, .listone-col-classicRole'), function (cell) {
      important(cell, 'width', '5.375rem');
      important(cell, 'min-width', '5.375rem');
      important(cell, 'max-width', '5.375rem');
    });
    Array.prototype.forEach.call(table.querySelectorAll('.fpt-v579-col-status, .roster-col-status, .team-profile-status-cell, .listone-col-status'), function (cell) {
      important(cell, 'width', '3.75rem');
      important(cell, 'min-width', '3.75rem');
      important(cell, 'max-width', '3.75rem');
      important(cell, 'overflow-wrap', 'anywhere');
    });
    Array.prototype.forEach.call(table.querySelectorAll('.fpt-v579-col-team, .roster-col-team, .team-profile-team-cell, .listone-col-realTeam'), function (cell) {
      important(cell, 'width', '3.125rem');
      important(cell, 'min-width', '3.125rem');
      important(cell, 'max-width', '3.125rem');
    });
    Array.prototype.forEach.call(table.querySelectorAll('.fpt-v579-col-cost, .fpt-v579-col-qta, .roster-col-cost, .roster-col-qta, .team-profile-cost-cell, .team-profile-qta-cell, .listone-col-quotationCurrent, .listone-col-rosterCost, .listone-col-fvm'), function (cell) {
      important(cell, 'width', '3.75rem');
      important(cell, 'min-width', '3.75rem');
      important(cell, 'max-width', '3.75rem');
      important(cell, 'text-align', 'left');
    });
    Array.prototype.forEach.call(table.querySelectorAll('.fpt-v579-col-market, .roster-col-market, .team-profile-market-cell'), function (cell) {
    ...
    Array.prototype.forEach.call(table.querySelectorAll('tbody tr'), function (row) {
      var role = row.getAttribute('data-fpt-v579-role') || '';
      var color = roleColor(role);
      var line = roleLine(role);
      var firstBackground = roleFirstBackground(role);
      Array.prototype.forEach.call(row.children || [], function (cell, index) {
        ...
        if (index === 0 || cell.classList.contains('fpt-v579-col-player')) {
          important(cell, 'background', firstBackground);
          important(cell, 'box-shadow', 'inset 4px 0 ' + line + ', 8px 0 14px rgba(2, 6, 23, 0.42)');
        }
      });
    });

    Array.prototype.forEach.call(table.querySelectorAll('.fpt-v579-col-player a, td:first-child a, .fpt-v579-col-player strong, td:first-child strong'), function (node) {
  ...
  function clearInlineMobileStyles(table) {
    if (isMobileViewport()) return;
    Array.prototype.forEach.call(table.querySelectorAll('th, td'), function (cell) {
      ['position', 'left', 'top', 'z-index', 'width', 'min-width', 'max-width', 'background-color', 'background', 'box-shadow', 'white-space', 'overflow-wrap', 'word-break', 'text-overflow', 'overflow'].forEach(function (prop) {
  ...
  function decorateTable(table) {
    ...
    if (!type) return 0;
    table.classList.add('fanta-player-table-v579');
    TYPE_CLASSES.forEach(function (name) { table.classList.remove(name); });
    table.classList.add('fanta-player-table-v579-' + type);
    table.setAttribute('data-player-table-v579', type);
  ...
  function run() {
    scheduled = false;
    if (document.body) document.body.classList.add('player-table-mobile-v579-active');
    var selector = [
  ...
  window.FantaPlayerTablesMobileV579 = { run: run, schedule: schedule, classifyTable: classifyTable };
