(function () {
  ...
  var VERSION = 'v578';
  var scheduled = false;
  var ROLE_CLASSES = ['fpt-v578-role-p', 'fpt-v578-role-d', 'fpt-v578-role-c', 'fpt-v578-role-a'];
  var TYPE_CLASSES = ['fanta-player-table-v578-teamarea', 'fanta-player-table-v578-rose', 'fanta-player-table-v578-listone'];
  ...
  function ensureWrapper(table, type) {
    ...
    if (wrapper) {
      wrapper.classList.add('fanta-player-table-wrap-v578', 'fanta-player-table-wrap-v578-' + type);
      wrapper.setAttribute('data-player-table-wrap-v578', type);
    }
    return wrapper;
  }
  ...
  function roleFromRow(row, roleIndex) {
    ...
    var explicit = row.querySelector('.fpt-v578-col-role, .roster-col-role, .team-profile-role-cell, .listone-col-classicRole, [data-label="R (RM)"], [data-label="Ruolo"], [data-label="R"]');
  ...
  function applyRole(row, role) {
    ...
    if (role) {
      row.classList.add('fpt-v578-role-' + role);
      row.setAttribute('data-fpt-v578-role', role);
    } else {
      row.removeAttribute('data-fpt-v578-role');
    }
  }
  ...
  function markColumns(table) {
    ...
    addColumnClassByIndex(table, playerIndex >= 0 ? playerIndex : 0, 'fpt-v578-col-player');
    addColumnClassByIndex(table, roleIndex, 'fpt-v578-col-role');
    addColumnClassByIndex(table, statusIndex, 'fpt-v578-col-status');
    addColumnClassByIndex(table, teamIndex, 'fpt-v578-col-team');
    addColumnClassByIndex(table, costIndex, 'fpt-v578-col-cost');
    addColumnClassByIndex(table, qtaIndex, 'fpt-v578-col-qta');
    addColumnClassByIndex(table, marketIndex, 'fpt-v578-col-market');
  ...
  function applyInlineMobileStyles(table, type) {
    ...
    var wrapper = table.closest('[data-player-table-wrap-v578], .team-profile-roster-wrap, .roster-inline-table-wrap, .listone-table-wrap, .mobile-tabular-wrap, .table-wrap');
    if (wrapper) {
    ...
    Array.prototype.forEach.call(table.querySelectorAll('.fpt-v578-col-player, th:first-child, td:first-child'), function (cell) {
      important(cell, 'position', 'sticky');
    ...
    Array.prototype.forEach.call(table.querySelectorAll('thead .fpt-v578-col-player, thead th:first-child'), function (cell) {
      important(cell, 'background-color', 'rgba(15, 23, 42, 0.99)');
      important(cell, 'color', '#f8fafc');
    });

    Array.prototype.forEach.call(table.querySelectorAll('.fpt-v578-col-role, .roster-col-role, .team-profile-role-cell, .listone-col-classicRole'), function (cell) {
      important(cell, 'width', '5.375rem');
      important(cell, 'min-width', '5.375rem');
      important(cell, 'max-width', '5.375rem');
    });
    Array.prototype.forEach.call(table.querySelectorAll('.fpt-v578-col-status, .roster-col-status, .team-profile-status-cell, .listone-col-status'), function (cell) {
      important(cell, 'width', '3.25rem');
      important(cell, 'min-width', '3.25rem');
      important(cell, 'max-width', '3.25rem');
      important(cell, 'overflow-wrap', 'anywhere');
    });
    Array.prototype.forEach.call(table.querySelectorAll('.fpt-v578-col-team, .roster-col-team, .team-profile-team-cell, .listone-col-realTeam'), function (cell) {
      important(cell, 'width', '3.125rem');
      important(cell, 'min-width', '3.125rem');
      important(cell, 'max-width', '3.125rem');
    });
    Array.prototype.forEach.call(table.querySelectorAll('.fpt-v578-col-cost, .fpt-v578-col-qta, .roster-col-cost, .roster-col-qta, .team-profile-cost-cell, .team-profile-qta-cell, .listone-col-quotationCurrent, .listone-col-rosterCost, .listone-col-fvm'), function (cell) {
      important(cell, 'width', '3.75rem');
      important(cell, 'min-width', '3.75rem');
      important(cell, 'max-width', '3.75rem');
      important(cell, 'text-align', 'left');
    });
    Array.prototype.forEach.call(table.querySelectorAll('.fpt-v578-col-market, .roster-col-market, .team-profile-market-cell'), function (cell) {
    ...
    Array.prototype.forEach.call(table.querySelectorAll('tbody tr'), function (row) {
      var role = row.getAttribute('data-fpt-v578-role') || '';
      ...
      Array.prototype.forEach.call(row.children || [], function (cell, index) {
        ...
        if (index === 0 || cell.classList.contains('fpt-v578-col-player')) {
          important(cell, 'box-shadow', 'inset 4px 0 ' + line + ', 8px 0 14px rgba(2, 6, 23, 0.42)');
        }
      });
    });

    Array.prototype.forEach.call(table.querySelectorAll('.fpt-v578-col-player a, td:first-child a, .fpt-v578-col-player strong, td:first-child strong'), function (node) {
  ...
  function decorateTable(table) {
    ...
    if (!type) return 0;
    table.classList.add('fanta-player-table-v578');
    TYPE_CLASSES.forEach(function (name) { table.classList.remove(name); });
    table.classList.add('fanta-player-table-v578-' + type);
    table.setAttribute('data-player-table-v578', type);
  ...
  function run() {
    scheduled = false;
    if (document.body) document.body.classList.add('player-table-mobile-v578-active');
    var selector = [
  ...
  window.FantaPlayerTablesMobileV578 = { run: run, schedule: schedule, classifyTable: classifyTable };
