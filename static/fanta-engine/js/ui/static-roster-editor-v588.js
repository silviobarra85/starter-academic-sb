(function installStaticRosterEditorV588() {
  'use strict';

  const VERSION = 'V588';
  const PANEL_ID = 'adminStaticRosterEditorPanelV588';
  const ROOT_ID = 'adminStaticRosterEditorRootV588';
  const DATA = {
    manifest: null,
    rosterFileName: '',
    roster: null,
  ...
  function refreshMetaFromForm() {
    ...
    const seasonId = document.getElementById('adminStaticRosterSeasonIdV588')?.value?.trim() || DATA.roster.meta?.seasonId || '';
    const loadedAt = document.getElementById('adminStaticRosterLoadedAtV588')?.value || todayIso();
    const label = document.getElementById('adminStaticRosterLabelV588')?.value?.trim() || DATA.roster.meta?.label || `Rose ${seasonId}`;
    const id = `${seasonId}-${loadedAt}`;
    const sourceFile = DATA.roster.meta?.sourceFile || DATA.originalRoster?.meta?.sourceFile || 'admin-static-roster-editor-v588';
  ...
  function setMessage(message, isError = false) {
    ...
    const node = document.getElementById('adminStaticRosterEditorStatusV588');
    if (node) {
    ...
    const rows = sortPlayers([...(team?.players || [])]).map((player, index) => {
      ...
      return `
        <tr data-role="${escapeHtml(player.role || '')}">
          ...
          <td><button class="button button-danger button-small" type="button" data-admin-static-roster-remove-v588="${originalIndex}">Togli</button></td>
        </tr>`;
    }).join('');
  ...
  function renderEditor() {
    ...
    const fileName = `${meta.seasonId || 'stagione'}-${document.getElementById('adminStaticRosterLoadedAtV588')?.value || meta.loadedAt || todayIso()}.json`;
    const selectedTeam = getSelectedTeam();
    root.innerHTML = `
      <div class="static-roster-editor-v588-grid">
        <label>
          Rosa di partenza
          <select id="adminStaticRosterSnapshotV588" class="input">${renderSnapshotOptions()}</select>
        </label>
        <label>
          Data nuova rosa
          <input id="adminStaticRosterLoadedAtV588" class="input" type="date" value="${escapeHtml(meta.loadedAt || todayIso())}" />
        </label>
        <label>
          Stagione output
          <input id="adminStaticRosterSeasonIdV588" class="input" type="text" value="${escapeHtml(meta.seasonId || '')}" />
        </label>
        <label>
          Label
          <input id="adminStaticRosterLabelV588" class="input" type="text" value="${escapeHtml(meta.label || '')}" />
        </label>
        <label class="span-2">
          Rosa da modificare
          <select id="adminStaticRosterTeamV588" class="input">${renderTeamOptions()}</select>
        </label>
      </div>

      <div class="static-roster-editor-v588-summary">
      ...
      <details class="admin-edit-section static-roster-editor-v588-section" open>
        <summary><strong>Giocatori rosa selezionata</strong><span>${escapeHtml(selectedTeam?.players?.length || 0)}</span></summary>
        <div class="table-wrap static-roster-editor-v588-table-wrap">
          <table class="static-roster-editor-v588-table">
      ...
      <div class="static-roster-editor-v588-add-box">
        <h4>Aggiungi dal listone</h4>
        <div class="static-roster-editor-v588-grid">
          <label class="span-2">
            Giocatore listone
            <select id="adminStaticRosterPlayerV588" class="input">
          ...
          <label>
            Costo
            <input id="adminStaticRosterPlayerCostV588" class="input" type="number" step="0.5" min="0" placeholder="auto" />
          </label>
          <label class="static-roster-editor-v588-check">
            <input id="adminStaticRosterMoveDuplicateV588" type="checkbox" checked />
            Se presente in altra rosa, spostalo qui
          </label>
          <div class="form-actions span-2">
            <button id="adminStaticRosterAddPlayerV588" class="button button-primary" type="button">Aggiungi alla rosa</button>
          </div>
        </div>
      </div>

      <div class="static-roster-editor-v588-downloads">
        <div>
        ...
        <div class="form-actions">
          <button id="adminStaticRosterDownloadRosterV588" class="button button-primary" type="button">Scarica JSON rosa</button>
          <button id="adminStaticRosterDownloadManifestV588" class="button button-secondary" type="button">Scarica manifest</button>
          <button id="adminStaticRosterDownloadBothV588" class="button button-secondary" type="button">Scarica entrambi</button>
        </div>
      </div>

      <div class="static-roster-editor-v588-sync">
        <div>
          <strong>Sincronizza rosterEntries</strong>
          <small>Usa questa azione solo dopo aver caricato e pubblicato su GitHub il JSON rosa e il manifest. La visualizzazione usa gia' <code>assets/rose</code>; questo pulsante aggiorna anche Firestore in modo esplicito.</small>
        </div>
        <div class="form-actions">
          <button id="adminStaticRosterSyncFirebaseV588" class="button button-secondary" type="button">Sincronizza rosterEntries dalla rosa GitHub</button>
        </div>
      </div>
      <span id="adminStaticRosterEditorStatusV588" class="form-status">${escapeHtml(DATA.message || '')}</span>
    `;
    attachEditorEvents();
  }

  function attachEditorEvents() {
    document.getElementById('adminStaticRosterSnapshotV588')?.addEventListener('change', async (event) => {
      try {
    ...
    document.getElementById('adminStaticRosterTeamV588')?.addEventListener('change', (event) => {
      DATA.selectedTeamIndex = Number(event.target.value) || 0;
      renderEditor();
    });
    document.getElementById('adminStaticRosterSeasonIdV588')?.addEventListener('change', async (event) => {
      try {
    ...
    document.getElementById('adminStaticRosterPlayerV588')?.addEventListener('change', (event) => {
      const player = listonePlayers()[Number(event.target.value)];
      const costInput = document.getElementById('adminStaticRosterPlayerCostV588');
      if (costInput && player) costInput.value = player.rosterCost ?? player.quotationCurrent ?? '';
    });
    document.querySelectorAll('[data-admin-static-roster-remove-v588]').forEach((button) => {
      button.addEventListener('click', () => {
        const team = getSelectedTeam();
        const index = Number(button.dataset.adminStaticRosterRemoveV588);
    ...
    document.getElementById('adminStaticRosterAddPlayerV588')?.addEventListener('click', () => {
      const select = document.getElementById('adminStaticRosterPlayerV588');
      ...
      const moveDuplicate = document.getElementById('adminStaticRosterMoveDuplicateV588')?.checked !== false;
      const owner = findRosterOwner(player.playerName);
      if (owner && owner !== team) {
        if (!moveDuplicate) {
      ...
      const cost = document.getElementById('adminStaticRosterPlayerCostV588')?.value;
    ...
    document.getElementById('adminStaticRosterDownloadRosterV588')?.addEventListener('click', () => {
      const meta = refreshMetaFromForm();
      downloadJson(`${meta.id}.json`, normalizeRosterShape(DATA.roster));
      setMessage(`Scaricato ${meta.id}.json.`);
    });
    document.getElementById('adminStaticRosterDownloadManifestV588')?.addEventListener('click', () => {
      downloadJson('manifest.json', buildManifest());
      setMessage('Scaricato manifest.json aggiornato.');
    });
    document.getElementById('adminStaticRosterDownloadBothV588')?.addEventListener('click', () => {
      const meta = refreshMetaFromForm();
      downloadJson(`${meta.id}.json`, normalizeRosterShape(DATA.roster));
      window.setTimeout(() => downloadJson('manifest.json', buildManifest()), 350);
      setMessage(`Scaricati ${meta.id}.json e manifest.json.`);
    });
    document.getElementById('adminStaticRosterSyncFirebaseV588')?.addEventListener('click', async (event) => {
      const button = event.currentTarget;
      const meta = refreshMetaFromForm();
      const api = window.FantaStaticRosterGithubPrimaryV588;
      if (!api?.sync) {
        setMessage('Runtime V588 non disponibile: ricarica la pagina con cache pulita.', true);
        return;
      }
      if (DATA.dirty) {
        setMessage('Hai modifiche non ancora scaricate/pubblicate. Scarica JSON + manifest, caricali su GitHub, fai deploy e poi sincronizza.', true);
        return;
      }
      const ok = window.confirm(`Sincronizzare Firestore rosterEntries dalla rosa GitHub pubblicata per ${meta.seasonId}? Le entry attive esistenti della stagione verranno marcate REMOVED e sostituite.`);
      if (!ok) return;
      const previous = button?.textContent || 'Sincronizza';
      try {
        if (button) {
          button.disabled = true;
          button.textContent = 'Sincronizzazione...';
        }
        setMessage('Sincronizzazione rosterEntries in corso...');
        const result = await api.sync({ seasonId: meta.seasonId });
        const unmatched = result.unmatchedRosters?.length ? ` · non associate: ${result.unmatchedRosters.join(', ')}` : '';
        setMessage(`Sincronizzazione completata: ${result.written} giocatori scritti, ${result.removed} entry precedenti rimosse${unmatched}.`);
      } catch (error) {
        setMessage(`Errore sincronizzazione rosterEntries: ${error.message || error}`, true);
      } finally {
        if (button) {
    ...
    ['adminStaticRosterLoadedAtV588', 'adminStaticRosterSeasonIdV588', 'adminStaticRosterLabelV588'].forEach((id) => {
  ...
  function panelHtml() {
    return `
      <article id="${PANEL_ID}" class="panel admin-collapsible-panel static-roster-editor-v588-panel" data-feature-card-id="adminStaticRosterEditorPanelV588" data-visibility="admin" data-category="admin">
        <header class="collapsible-panel-header">
          ...
          <button class="button button-secondary button-small" type="button" data-static-roster-editor-toggle-v588>Riduci</button>
        </header>
        <div class="collapsible-panel-body" data-static-roster-editor-body-v588>
          <div id="${ROOT_ID}" class="static-roster-editor-v588-root">
  ...
  function insertPanelIfNeeded() {
    ...
    document.querySelector('[data-static-roster-editor-toggle-v588]')?.addEventListener('click', (event) => {
      const body = document.querySelector('[data-static-roster-editor-body-v588]');
      if (!body) return;
  ...
  function boot() {
    ...
    window.FantaStaticRosterEditorV588 = {
      version: VERSION,
