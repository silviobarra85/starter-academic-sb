...
## Aggiornamento V499 - Firebase adapter comune senza migrazione dati

La V499 introduce `static/fanta-engine/js/firebase/firebase-adapter-v499.js` come adapter Firebase comune. Il suo scopo è centralizzare inizializzazione e helper multi-lega senza cambiare il modello dati esistente.

Cosa è stato centralizzato:

- import Firebase SDK v10.12.2;
- `initializeApp`, `getFirestore`, `getAuth`;
- export delle primitive Firestore/Auth già usate da `assets/app.js`;
- factory `createFirebaseLeagueRuntimeV499()`;
- helper `collectionRef`, `docRef`, `leagueWhere`, `seasonWhere`, `withLeagueMetadata`, `runtimeInfo`;
- metadata runtime per leagueId, seasonId, projectId e data model.

Cosa NON è stato fatto:

- nessuna migrazione a `/leagues/{leagueId}/...`;
- nessuna modifica alle Firestore rules;
- nessuna modifica ai nomi collection esistenti;
- nessuna modifica ai flussi Admin/Presidente;
- nessuna modifica a Proposte regolamento, Svincola, Comunicato scambio o Area Squadra.

I wrapper locali restano obbligatori:

- `static/zonaorientale/assets/firebase.js` contiene il progetto Firebase ZonaOrientale;
- `static/fantapetillomantramanager/assets/firebase.js` contiene il progetto Firebase FantaMantraManager;
- entrambi espongono ancora `db`, `auth`, `collection`, `doc`, `getDoc`, ecc.

Roadmap aggiornata dopo V499:

1. V500 - Dashboard cards engine basato sul registry V497.
2. V501 - Tool engine comune, partendo da Sorteggio giornate.
3. V502 - Template nuova lega.
4. V503 - Test browser Playwright.
5. Fase futura opzionale - migrazione dati Firebase a namespace `/leagues/{leagueId}` solo dopo backup/rules/test.

Guardrail: la V500 potrà usare registry e adapter, ma non deve migrare path Firestore.
